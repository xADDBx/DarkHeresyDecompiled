...
public class AiAreaScanner
{
	...
	public static async Task<PathData> FindAllReachableNodesAsync(BaseUnitEntity unit, Vector3 pos, float maxPathLen, AiThreatsHandlingStrategy threatsHandlingStrategy)
	{
		...
		Dictionary<GraphNode, WarhammerPathAiCell> dictionary = await PathfindingService.Instance.FindAllReachableTiles_Delayed_Task(unit.View.MovementAgent, startNode, (int)maxPathLen, threateningAreaCells, threatsHandlingStrategy);
		if (dictionary == null)
		{
