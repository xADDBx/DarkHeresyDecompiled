...
public class ContextActionMoveOutsideZone : ContextAction
{
	...
	{
		GridNodeBase startPoint = base.TargetEntity.Position.GetNearestNodeXZ();
		GridNodeBase nearestNodeXZ = m_TargetPoint.GetValue().GetNearestNodeXZ();
		int num = int.MaxValue;
		GridNodeBase randomCell = null;
		GridNodeBase gridNodeBase = (GridNodeBase)AstarPath.active.GetNearest(base.TargetEntity.Position).node;
		if (base.TargetEntity.MaybeMovementAgent != null && base.TargetEntity is BaseUnitEntity baseUnitEntity)
		{
			...
			{
				if (graphNode2 != null && graphNode2 != gridNodeBase && warhammerPathPlayerCell2.IsCanStand && (!(graphNode2 is GridNodeBase node) || nearestNodeXZ == null || !m_Zone.GetOriented(nearestNodeXZ, nearestNodeXZ.Vector3Position() - base.TargetEntity.Position).Contains(node)))
				{
			...
			randomCell = (GridNodeBase)list[baseUnitEntity.Random.Range(0, list.Count)];
		}
		if (m_UseForceMove)
		{
