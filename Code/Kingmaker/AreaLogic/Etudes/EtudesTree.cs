...
public class EtudesTree : EntityFactsProcessor<Etude>
{
	...
	{
		if (system.EtudeIsCompleted(etude.Blueprint) || !base.RawFacts.HasItem((Etude x) => x.Blueprint == etude.Blueprint))
		{
			return;
		}
	...
	{
		if (etude.Blueprint.StartsParent && !Game.Instance.EtudesSystem.EtudeIsNotStarted(etude.Blueprint) && Game.Instance.EtudesSystem.EtudeIsNotStarted(etude.Parent?.Blueprint))
		{
	...
	private void BreakConflictingEtude(Etude etude)
	{
		foreach (BlueprintEtudeConflictingGroupReference conflictingGroup in etude.Blueprint.ConflictingGroups)
		{
			...
			{
				BlueprintEtude conflictingGroupTask = Game.Instance.EtudesSystem.GetConflictingGroupTask(conflictingGroup.Get());
