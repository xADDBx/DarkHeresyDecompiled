...
public class DirectorCameraLink : MonoBehaviour
{
	...
	{
		CameraRig instance = CameraRig.Instance;
		Transform cameraAttachPoint = instance.CameraAttachPoint;
		Transform transform = instance.Camera.transform;
		Quaternion quaternion = base.transform.rotation * Quaternion.Inverse(cameraAttachPoint.localRotation * transform.localRotation);
		Vector3 position = base.transform.position - quaternion * (cameraAttachPoint.localPosition + cameraAttachPoint.localRotation * transform.localPosition);
		if (m_DirectorCamera != null)
		{
			m_CinemachineBrain.enabled = false;
			m_DirectorCamera.gameObject.SetActive(value: false);
		...
		if (instance != null)
		{
			...
			instance.RotateToImmediately(quaternion.eulerAngles.y);
