...
public class DirectorAdapter : EntityViewBase, IUpdatable, IInterpolatable
{
	...
	private class TickLogic
	{
		...
		public void Tick(float delta)
		{
			...
			m_PreviousTime = m_CurrentTime;
			m_CurrentTime += delta;
			if ((double)m_CurrentTime > PlayableDirector.duration)
			{
				DirectorWrapMode extrapolationMode = PlayableDirector.extrapolationMode;
				if (extrapolationMode != 0)
				{
					if (extrapolationMode == DirectorWrapMode.Loop)
					{
						do
						{
							m_CurrentTime -= (float)PlayableDirector.duration;
						}
						while ((double)m_CurrentTime > PlayableDirector.duration);
					}
				}
				else
				{
	...
	public void ApplyBindings(List<CommandPlayTimeline.UnitBindingData> units = null)
	{
		...
		for (int i = 0; i < BoundTracks.Length; i++)
		{
			...
			{
				unitBinding.AnimationManager.UpdateMode = DirectorUpdateMode.Manual;
			}
			if (units != null)
			{
	...
	public void Play()
	{
		Game.Instance.Controllers.DirectorAdapterController.Add(this);
	...
	public void Stop()
	{
		...
		UnitBinding[] boundUnits = BoundUnits;
		foreach (UnitBinding unitBinding in boundUnits)
		{
			if (unitBinding.BoundUnit.AnimationManager != null)
			{
				unitBinding.BoundUnit.AnimationManager.UpdateMode = DirectorUpdateMode.GameTime;
			}
			unitBinding.BoundUnit.ControlledByDirector.Release();
			unitBinding.Reset();
		}
		if (CameraLink != null)
		{
