...
public class CutsceneControlledUnit
{
	...
	private bool CanPlayCutscene
	{
		get
		{
			if (Game.Instance.CurrentModeType == GameModeType.Cutscene || Game.Instance.GameCommandQueue.ContainsCommand((SwitchCutsceneLockCommand i) => i.Lock))
			{
				return true;
			}
	...
	private void UpdateActiveCutscene()
	{
		...
		if (CanPlayCutscene)
		{
			foreach (CutsceneEntry entry in m_Entries)
			{
				if (cutsceneEntry?.Cutscene == null || cutsceneEntry?.Cutscene?.Cutscene?.Priority < entry?.Cutscene?.Cutscene?.Priority)
				{
					cutsceneEntry = entry;
				}
			}
		...
		if (m_Active != null)
		{
			...
			{
				if (m_Active.Cutscene != null)
				{
					m_Active.PauseOrStop();
				}
				else
				{
					m_Entries.Remove(m_Active);
				}
			}
		}
	...
	public BlueprintCutscene GetCurrentlyActive()
	{
		return m_Active?.Cutscene?.Cutscene;
	}
