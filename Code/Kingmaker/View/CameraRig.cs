...
public class CameraRig : MonoBehaviour, IAreaHandler, ISubscriber, IAdditiveAreaSwitchHandler, IGameModeHandler, IDialogStartHandler, IDialogFinishHandler, IFullScreenUIHandler, IFullScreenUIHandlerWorkaround
{
	...
	[SerializeField]
	private GameObject m_CombatCameraRoot;
	...
	public Transform CameraAttachPoint => m_CameraAttachPoint;

	public GameObject CombatCameraRoot => m_CombatCameraRoot;
	...
	private bool IsSpaceGameMode
	{
		get
		{
			if (!(Game.Instance?.CurrentModeType == GameModeType.GlobalMap))
			{
				return Game.Instance?.CurrentModeType == GameModeType.StarSystem;
			}
			return true;
		}
	}
	...
	public bool IsScrollingByRoutineSynced
	{
		get
		{
			foreach (PlayerCommands<SynchronizedData> player in Game.Instance.Controllers.SynchronizedDataController.SynchronizedData.Players)
			{
				...
				{
					CameraData camera = command.camera;
	...
	public static Vector3 ViewportToScreenMainCamera(Vector3 screenPos)
	{
		Camera camera = Instance.Camera;
		if (!(camera == null))
		{
			return camera.ViewportToScreenPoint(screenPos);
		}
		return default(Vector2);
	}
	...
	{
		BlueprintCameraSettings blueprintCameraSettings = Game.Instance.CurrentlyLoadedArea?.CameraSettings ?? ((BlueprintCameraSettings)ConfigRoot.Instance.CameraRoot.GroundMapSettings);
		if (!CameraZoom)
		{
	...
	private void UpdateInternal()
	{
		...
		{
			base.transform.position = m_HardBindPosition + GetWorldOffset();
			m_WorldToClipMatrixCached = Camera.projectionMatrix * Camera.worldToCameraMatrix;
			return;
		}
		float debugTimeScale = Game.Instance.Controllers.TimeController.DebugTimeScale;
		...
		bool flag = num2 && m_WorldOffset != Vector3.zero;
		base.transform.position = Vector3.Lerp(base.transform.position, m_TargetPosition + GetWorldOffset() - m_AnchorShakeOffset, flag ? 1f : (num * ScrollRubberBand));
	...
	private Vector2 CameraDragToRotate()
	{
		...
		Vector2 result = (m_RotateDistance - vector) * (SettingsRoot.Controls.CameraRotationSpeedEdge.GetValue() * 0.05f);
	...
	public Coroutine ScrollToTimed(Vector3 position, out float targetTime, float maxTime = 0f, float maxSpeed = float.MaxValue, float speed = 0f, AnimationCurve curve = null, bool useUnscaledTime = false, Action callback = null)
	{
		...
		{
			targetTime = Game.Instance.Controllers.SynchronizedDataController.GetMinScrollTimeBySpeed(position, speed);
		}
		else
		{
			targetTime = Game.Instance.Controllers.SynchronizedDataController.GetMinScrollTimeBySpeed(position, speed, maxTime, maxSpeed);
		}
	...
	public Coroutine RotateToTimed(float toAngle, out float targetTime, float maxTime = 0f, float speed = 0f, AnimationCurve curve = null)
	{
		...
		if (maxTime == 0f && speed > 0f)
		{
			targetTime = Game.Instance.Controllers.SynchronizedDataController.GetMinRotateTimeBySpeed(toAngle, speed);
		}
		else
		{
			targetTime = Game.Instance.Controllers.SynchronizedDataController.GetMinRotateTimeBySpeed(toAngle, speed, maxTime);
		}
		if (targetTime == 0f)
		{
			m_RotateRoutine = null;
	...
	public Vector3 CalculateCameraPositionOnTarget(Vector3 position)
	{
		Vector3 pos = ClampByLevelBounds(position);
		return PlaceOnGround(pos) + base.transform.TransformDirection(CameraAttachPoint.localPosition) + GetWorldOffset();
	}

	public void SetRigPositionImmmediately(Vector3 position)
	{
		base.transform.position = position + GetWorldOffset();
	...
	{
		RotateToTimed((float)(Game.Instance.CurrentlyLoadedArea?.LocalMapRotationDeg ?? BlueprintAreaPart.LocalMapRotationDegree.Degree0) - 180f, 0.4f);
	}
