...
public class ScriptZoneCylinder : ScriptZoneShape
{
	...
	private GridNodeBase m_CenterNode;

	private NodeList m_Nodes = NodeList.Empty;

	private GridNodeBase CenterNode => m_CenterNode ?? (m_CenterNode = ScriptZoneCenter.GetNearestNodeXZUnwalkable());
	...
	public override NodeList CoveredNodes
	{
		get
		{
			...
			GridGraph gridGraph = (GridGraph)CenterNode.Graph;
			int radius = (int)((float)Radius / gridGraph.nodeSize);
			...
			m_Nodes = new NodeList(gridGraph, in pattern);
			return m_Nodes;
		}
	}
	...
	public override bool Contains(GridNodeBase node, IntRect size = default(IntRect))
	{
