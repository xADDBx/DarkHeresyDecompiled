...
public class SimpleTrapObjectView : TrapObjectView, ISimpleTrapEntityConfig, ITrapEntityConfig, IMapObjectEntityConfig, IMechanicEntityConfig, IEntityConfig
{
	...
	{
		BlueprintTrapSettingsRoot blueprintTrapSettingsRoot = ConfigRoot.Instance.BlueprintTrapSettingsRoot;
	...
	{
		return ConfigRoot.Instance.BlueprintTrapSettingsRoot.Find(blueprintArea.GetCR() + Info.AdditionalCR);
	}

	private static void AttachPerceptionComponent(GameObject gameObject, SimpleTrapObjectData data)
	{
		if (!(gameObject.GetComponent<AwarenessCheckComponent>() != null))
		{
			AwarenessCheckComponent awarenessCheckComponent = gameObject.EnsureComponent<AwarenessCheckComponent>();
			awarenessCheckComponent.Settings.CustomDifficulty = data.PerceptionDC;
			awarenessCheckComponent.Settings.Difficulty = SkillCheckDifficulty.Custom;
			awarenessCheckComponent.Settings.Radius = data.PerceptionRadius;
		}
	}
}
