...
public class UnitMovementAgentContinuous : UnitMovementAgentBase
{
	...
	private GridNodeBase m_CurrentNode;
	...
	public GridNodeBase CurrentNode => m_CurrentNode;

	public override bool IsReallyMoving
	{
		get
		{
			if (!(DirectionFromControllerMagnitude > 0.0001f) && !(m_Velocity.sqrMagnitude > 0.0001f))
			{
			...
			if ((object)unit == null || !unit.IsCommandsPreventMovement)
			{
				using (ProfileScope.New("Tick Movement Continous"))
				{
					...
					else
					{
						...
						{
							m_Speed = num;
							Vector3 position = AstarPath.active.graphs[0].GetNearest(Position).position;
							Position = position;
						}
						...
						float speed = ((Mathf.Abs(num - m_Speed) < Acceleration * deltaTime) ? num : (m_Speed + Mathf.Sign(num - m_Speed) * Acceleration * deltaTime));
						...
						directionFromController = Vector3.RotateTowards(directionFromController, vector2, base.AngularSpeedWhenMove * deltaTime * (MathF.PI / 180f), 1f);
						m_Speed = speed;
						...
						base.IsPositionChanged = Mathf.Abs(vector4.x) < 0.01f && Mathf.Abs(vector4.z) < 0.01f;
						if (base.IsPositionChanged)
						{
