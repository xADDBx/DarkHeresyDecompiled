...
public class UnitGroupView : EntityGroupViewBase
{
	[OwlPackable(OwlPackableMode.Generate)]
	public class UnitGroupData : SimpleEntity, IHashable, IOwlPackable<UnitGroupData>
	{
		...
		public IEnumerable<AbstractUnitEntity> Members => Game.Instance.EntityPools.AllUnits.Where((AbstractUnitEntity i) => i.Group == this);
		...
		{
			UnitGroupData source = new UnitGroupData();
			result = Unsafe.As<UnitGroupData, TPossiblyBase>(ref source);
		}
		...
		public override void Serialize<TFormatter>(TFormatter formatter, SerializerState state)
		{
			...
			ushort type = state.TypeLibrary.RegisterType<UnitGroupData>(OwlPackTypeInfo);
		...
		public override void Deserialize<TFormatter>(TFormatter formatter, uint objectId, DeserializerState state)
		{
			...
			TypeInfo typeInfo = state.TypeLibrary.GetTypeInfo<UnitGroupData>();
