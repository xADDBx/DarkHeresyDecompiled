...
public class UnitAnimationDecoratorManager
{
	...
	public void ShowDecorator(UnitAnimationDecoratorObject decorator, float duration, AbstractUnitEntityView unitEntity = null)
	{
		AnimationClip animationClip = m_AnimationManager.CurrentAction?.ActiveAnimation?.GetPlayableClip();
		...
		if (!m_Decorators.TryGetValue(tuple, out var decoratorEntry))
		{
			...
			foreach (DecoratorEntry decoratorEntry2 in entries)
			{
				if (!string.IsNullOrEmpty(decoratorEntry2.BoneName) && decoratorEntry2.Prefab != null)
				{
					...
					if (transform != null)
					{
						GameObject gameObject = UnityEngine.Object.Instantiate(decoratorEntry2.Prefab, transform, worldPositionStays: true);
						...
						unitEntity.Or(null)?.MarkRenderersAndCollidersAreUpdated();
					}
				}
			}
		...
		decoratorEntry.Lifetime = duration;
