...
public class RayView : MonoBehaviour, IDeactivatableComponent
{
	...
	private TimeSpan LifeTime => Game.Instance.Controllers.TimeController.GameTime - m_StartTime;
	...
	public void Update()
	{
		...
		else if (!(m_PrevTickTime == Game.Instance.Controllers.TimeController.GameTime))
		{
			m_PrevTickTime = Game.Instance.Controllers.TimeController.GameTime;
	...
	private void UpdateFadeIn(float fullDistance)
	{
		...
		float num2 = Math.Min(1f, m_FadeInShift / fullDistance);
		float ar = Math.Min(1f, num2 + num);
		m_LineRenderer.colorGradient = UpdateFade(m_ColorGradientOrigin, num2, ar, revert: false);
	}

	private void UpdateFadeOut(float fullDistance)
	{
		float num = Math.Max(0.1f, m_FadeOutDistance / fullDistance);
		float num2 = Math.Min(1f, m_FadeOutShift / fullDistance);
		float ar = Math.Min(1f, num2 + num);
		m_LineRenderer.colorGradient = UpdateFade(m_ColorGradientOrigin, num2, ar, revert: true);
	}
