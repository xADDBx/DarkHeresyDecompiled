...
namespace Kingmaker.View.Visual.Sound;
...
public class AudioConeRaycaster : MonoBehaviour
{
	...
	private void UpdateHitObjects()
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (Vector3.Angle(m_LastForward.Value, to) <= m_ConeAngle)
			{
				...
				if (component != null)
				{
					...
					{
						AkUnitySoundEngine.SetRTPCValue(m_RtpcName, m_RtpcHitValue, component.gameObject);
					}
				}
			}
			else
		...
		foreach (GameObject hitObject in m_HitObjects)
		{
			if (hitObject != null)
			{
				AkUnitySoundEngine.SetRTPCValue(m_RtpcName, m_RtpcDefaultValue, hitObject);
			}
		}
		m_HitObjects.Clear();
