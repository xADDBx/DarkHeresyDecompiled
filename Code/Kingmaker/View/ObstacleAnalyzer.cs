...
public struct ObstacleAnalyzer
{
	...
	public static float CellSize => ((GridGraph)AstarPath.active.graphs[0]).nodeSize;
	...
	static ObstacleAnalyzer()
	{
		...
		NNConstraint unwalkableXZConstraint = UnwalkableXZConstraint;
		bool distanceXZ = (DefaultXZConstraint.distanceXZ = true);
		unwalkableXZConstraint.distanceXZ = distanceXZ;
	...
	public static Vector3 TraceAlongNavmesh(Vector3 start, Vector3 end, GraphNode hint = null, WarhammerSingleNodeBlocker callerNodeBlocker = null)
	{
		...
		if (nearestNode.node.Graph is GridGraph)
		{
		...
		NavmeshBase.Linecast(nearestNode.node.Graph as NavmeshBase, start, end, nearestNode.node, out var hit2, null);
		return hit2.point;
	}

	public static Path FindPathAlongNavmesh(Vector3 start, Vector3 end, GraphNode hint = null, WarhammerSingleNodeBlocker callerNodeBlocker = null)
	{
		NNInfo nearestNode = GetNearestNode(start, hint);
		if ((start.To2D() - nearestNode.position.To2D()).sqrMagnitude > 0.0001f)
		{
			return ForcedPath.Construct(new List<Vector3> { nearestNode.position });
		}
		ForcedPath forcedPath = PathfindingService.Instance.FindPathBetweenPointsRT_Blocking(start, end);
		if (forcedPath.CompleteState == PathCompleteState.Error)
		{
			return ForcedPath.Construct(new List<Vector3> { nearestNode.position });
		}
		return forcedPath;
	...
	public static int2 GetNearestNodeCoords(Vector3 position)
	{
		...
		if (graphs[0] is GridGraph graph)
		{
			return graph.GetNearestNodeCoords(position);
		}
		Logger.Error("No valid Grid Graph");
		return default(int2);
	}
	...
	[CanBeNull]
	public static GridNode GetNearestNodeXZUnwalkable(Vector3 pos)
	{
		AstarPath active = AstarPath.active;
		if (!active)
		{
		...
		if (graphs[0] is GridGraph gg)
		{
			return gg.GetNearestDirect(pos) as GridNode;
		}
		return null;
	}
	...
	public static NNInfo GetNearestNode(Vector3 pos, GraphNode hint = null, NNConstraint constraint = null)
	{
		...
		{
			if (hint is GridNodeBase gridNodeBase && gridNodeBase.ContainsPoint(pos))
			{
				return new NNInfo(gridNodeBase, pos, (pos - gridNodeBase.Vector3Position()).sqrMagnitude);
			}
			if (AstarPath.active == null)
			{
				return default(NNInfo);
			}
			return AstarPath.active.GetNearest(pos, constraint);
		}
	}

	public static Vector3 FindClosestPointToStandOn(Vector3 pos, float corpulence, GridNodeBase hint = null)
	{
		NNInfo nearestNode = GetNearestNode(pos, hint);
		GridNode gridNode = (GridNode)nearestNode.node;
		pos = nearestNode.position;
		for (int i = 0; i < 8; i++)
		{
			if (!gridNode.HasConnectionInDirection(i))
			{
				Vector3 vector = GraphHelper.GetVector3Direction(i) * 1.35f;
				GridNode gridNode2 = (GridNode)gridNode.GetNeighbourAlongDirection(i);
				if (gridNode2 != null)
				{
					vector = (gridNode2.Vector3Position() - gridNode.Vector3Position()).To2D().To3D();
				}
				float magnitude = vector.magnitude;
				Vector3 vector2 = vector / magnitude;
				Vector3 rhs = (pos - gridNode.Vector3Position()).To2D().To3D();
