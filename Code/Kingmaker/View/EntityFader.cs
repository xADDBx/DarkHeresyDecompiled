...
public class EntityFader : LateUpdateableBehaviour
{
	...
	public bool Visible
	{
		...
		set
		{
			if (m_Visible != value)
			{
				UnitEntityView unitEntityView = Entity as UnitEntityView;
				if (Game.Instance.CurrentModeType == GameModeType.Cutscene || ((object)unitEntityView != null && !unitEntityView.Data.IsInCameraFrustum_Closer))
				{
					DisableAnimation();
				}
				else
				...
				if (m_FadeAnimation.IsFinished)
				{
					...
					m_FadeAnimation.Lifetime = ConfigRoot.Instance.Prefabs.FogOfWarDissolveSettings.Lifetime;
				}
				...
				unitEntityView?.EntityData.Wake(num);
			}
			else
			{
				ClearRoutine();
	...
	protected override void OnEnabled()
	{
		...
		DissolveSettings fogOfWarDissolveSettings = ConfigRoot.Instance.Prefabs.FogOfWarDissolveSettings;
