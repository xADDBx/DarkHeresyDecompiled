...
public class UnitEntityView : AbstractUnitEntityView, IUnitEquipmentHandler<EntitySubscriber>, IUnitEquipmentHandler, ISubscriber<IMechanicEntity>, ISubscriber, IEventTag<IUnitEquipmentHandler, EntitySubscriber>, IUnitActiveEquipmentSetHandler<EntitySubscriber>, IUnitActiveEquipmentSetHandler, ISubscriber<IBaseUnitEntity>, IEventTag<IUnitActiveEquipmentSetHandler, EntitySubscriber>, IUnitLifeStateChanged<EntitySubscriber>, IUnitLifeStateChanged, ISubscriber<IAbstractUnitEntity>, IEventTag<IUnitLifeStateChanged, EntitySubscriber>, ILootDroppedAsAttachedHandler<EntitySubscriber>, ILootDroppedAsAttachedHandler, IEventTag<ILootDroppedAsAttachedHandler, EntitySubscriber>, ICellAbilityHandler<EntitySubscriber>, ICellAbilityHandler, IEntitySubscriber, IEventTag<ICellAbilityHandler, EntitySubscriber>, IShowAoEAffectedUIHandler
{
	...
	public void UpdateCombatSwitch()
	{
		if (Data != null)
		{
			...
			AkUnitySoundEngine.SetRTPCValue("CharacterCombat", Data.IsInCombat ? 1 : 0, base.gameObject);
		}
	}

	public void UpdateIsPlayerOwned()
	{
		if (Data != null)
		{
			AkUnitySoundEngine.SetRTPCValue("IsPlayerOwned", Data.IsInPlayerParty ? 1 : 0, base.gameObject);
		}
	}
	...
	protected override void OnDidAttachToData()
	{
		...
		base.CharacterAvatar?.RequestRebuild(Character.CharacterRebuildMode.OnlyOutfit);
	...
	public override void UpdateViewActive()
	{
		...
		base.enabled = Data.IsViewActive;
		if (base.CharacterAvatar != null)
		{
			base.CharacterAvatar.enabled = Data.IsViewActive;
		}
	}
	...
	internal override void OnMovementWaypointUpdate()
	{
		base.OnMovementWaypointUpdate();
		using (ProfileScope.New("Unit move waypoint update"))
		{
			EventBus.RaiseEvent((IBaseUnitEntity)EntityData, (Action<IUnitMovementHandler>)delegate(IUnitMovementHandler h)
			{
				h.HandleWaypointUpdate();
			}, isCheckRuntime: true);
		}
	}
	...
	{
		if (base.CharacterAvatar == null || base.CharacterAvatar.IsBaked)
		{
			return;
		}
	...
	public void UpdateEquipmentColor()
	{
		...
		int? primaryRampIndex = EntityData.ViewSettings.Doll?.ClothesPrimaryIndex;
		int? secondaryRampIndex = EntityData.ViewSettings.Doll?.ClothesSecondaryIndex;
		if (!base.CharacterAvatar || base.CharacterAvatar.EquipmentEntities == null)
		{
			return;
		}
		foreach (EquipmentEntity equipmentEntity in base.CharacterAvatar.EquipmentEntities)
		{
			if (!(equipmentEntity.ColorPresets == null) && equipmentEntity.ColorPresets.HasFlag(ColorPresetFeatureFlag.ClothingPreset))
			{
				base.CharacterAvatar.SetRampIndices(equipmentEntity, primaryRampIndex, secondaryRampIndex);
			}
		}
	}
	...
	private void UpdateStealthMarks()
	{
		...
		if (active)
		{
			...
			{
				GameObject stealthEffectPrefab = ConfigRoot.Instance.Prefabs.StealthEffectPrefab;
		...
		if (num == 0 && (bool)m_StealthEffect)
		{
			...
			FxHelper.SpawnFxOnEntity(ConfigRoot.Instance.Prefabs.ExitStealthEffectPrefab, this);
		}
	}
	...
	public void UpdateLootLocalMapMark()
	{
		...
		if (isDeadAndHasLoot && m_LootLocalMapMarker == null)
		{
			...
			LocalMapModel.Add(m_LootLocalMapMarker);
			UpdateHighlight();
		}
		else if (!isDeadAndHasLoot && m_LootLocalMapMarker != null)
		{
			LocalMapModel.Remove(m_LootLocalMapMarker);
	...
	protected override void OnDoLateUpdate()
	{
		...
		foreach (Renderer renderer in Renderers)
		{
			...
			{
				skinnedMeshRenderer.updateWhenOffscreen = isInLockControlCutscene;
			}
		}
		m_ParticipateInLockControlCutscene = isInLockControlCutscene;
	}
	...
	{
		ArmorProficiencyGroup armorProficiencyGroup = ArmorProficiencyGroup.None;
	...
	public void SpawnAttachedDroppedLoot()
	{
		if (Data.IsDeadAndHasLoot && Data.Inventory.IsLootDroppedAsAttached && !(m_AttachedDroppedLoot != null))
		{
			...
			GameObject original = ConfigRoot.Instance.Prefabs.DroppedLootBagAttachedLink.Load();
