...
public class UnitViewHandSlotData
{
	...
	private void RecreateModel()
	{
		...
		if ((bool)fxPrefab)
		{
			...
			CableController componentInChildren = VisualModel.GetComponentInChildren<CableController>(includeInactive: true);
			if (componentInChildren != null)
			{
				componentInChildren.gameObject.SetActive(IsInHand);
				if (IsInHand)
				{
					if (Character.HasBonesList)
					{
						componentInChildren.Attach(Character.BonesList);
					}
					else
					{
						componentInChildren.Attach(Character.transform);
					}
				}
			}
		}
	...
	public void AttachModel()
	{
		...
		using (new NotifyOwnerRenderersChangedScope(this))
		{
			...
			{
				if (((VisualSlot == UnitEquipmentVisualSlotType.RightBack01 || VisualSlot == UnitEquipmentVisualSlotType.LeftBack01) && Character.EquipmentEntities.Any((EquipmentEntity ee) => ee.HasFeature(EquipmentFeatureFlag.IsBackpack))) || (IsOff && Owner.GetOptional<UnitPartMechadendrites>() != null))
				{
	...
	private void SetupFXSnapMap(bool isVisible)
	{
		if ((m_Equipment.IsDollRoom || !(UIDollRooms.Instance != null) || !(UIDollRooms.Instance.CharacterDollRoom != null) || UIDollRooms.Instance.CharacterDollRoom.Unit != Owner) && IsInHand && isVisible)
		{
			WeaponParticlesSnapMap fxSnapMap = ((VisualModel != null) ? VisualModel.GetComponent<WeaponParticlesSnapMap>() : null);
			Slot.FxSnapMap = fxSnapMap;
		}
	}
	...
	public void ShowItem(bool isVisible)
	{
		...
		if ((bool)VisualModel)
		{
			...
			{
				if (!visualModelRenderer.CompareTag("EditorOnly"))
				{
