...
public class UnitViewHandsEquipment
{
	...
	public WeaponType ActiveMainHandWeaponType
	{
		get
		{
			WeaponType? weaponType = m_ActiveSet?.MainHand?.Slot.MaybeWeapon?.WeaponType;
			if (!weaponType.HasValue)
			{
				if (!(Character != null))
				{
					return WeaponType.Creature;
				}
				return WeaponType.Fist;
			}
			return weaponType.GetValueOrDefault();
		}
	}

	public WeaponType ActiveOffHandWeaponType => (m_ActiveSet?.OffHand?.Slot.MaybeWeapon?.WeaponType).GetValueOrDefault();

	public WeaponAnimationStyle ActiveWeaponStyle
	{
		get
		{
			...
			if (activeSet == null)
			{
				if (!(Character != null))
				{
					return WeaponAnimationStyle.Creature;
				}
				return WeaponAnimationStyle.MeleeMelee;
			}
			return activeSet.WeaponStyle;
		}
	}
	...
	public void HandleEquipmentSlotUpdated([NotNull] HandSlot slot, ItemEntity previousItem)
	{
		using (ProfileScope.NewScope("HandleEquipmentSlotUpdated"))
		{
			...
			{
				if (InCombat && Owner.CanAct && slot.Active && !IsDollRoom)
				{
	...
	public void HandleEquipmentSetChanged()
	{
		if (Active)
		{
			if (InCombat && Owner.CanAct && !IsDollRoom)
			{
				StartCombatChangeAnimation();
			}
			else
	...
	public void StartCombatChangeAnimation()
	{
		if (!m_IsAnimatingWeaponsChange)
		{
			...
			m_Coroutine = Game.Instance.Controllers.CoroutinesController.Start(AnimateEquipmentChangeInCombat());
		}
	}
	...
	private IEnumerator AnimateEquipmentChangeInCombat()
	{
		...
		while (IsMainHandMismatched || IsOffHandMismatched)
		{
			...
			m_ActiveSet.HideWeaponModels();
			HandEquipmentHelper handEquipmentHelper = ((flag || flag2) ? EquipWeaponSet(selectedWeaponSet, unitEquipmentVisualSlotType, unitEquipmentVisualSlotType2) : null);
	...
	public void UpdateBeltPrefabs()
	{
		...
		using (ProfileScope.NewScope("UpdateBeltPrefabs"))
		{
			...
			{
				foreach (UsableSlot item in Owner.Body.QuickSlots.Where((UsableSlot s) => s.HasItem && (bool)s.Item.Blueprint.BeltItemPrefab))
				{
					...
					component = (component ? component : ConfigRoot.Instance.Prefabs.DefaultConsumableOffsets);
					if ((bool)component)
					{
	...
	private bool ShouldShowOutfit(OutfitPart part, GameObject gameObject)
	{
		if (part.HasFeature(EquipmentFeatureFlag.IsBackpack))
		{
	...
	{
		if (Game.Instance.Controllers.HandsEquipmentController != null)
		{
	...
	public bool MatchWithCurrentCombatState()
	{
		...
		if (!IsDollRoom)
		{
			m_Coroutine = Game.Instance.Controllers.CoroutinesController.Start(AnimateEquipping(InCombat));
		}
		else
		{
			m_ActiveSet.OffHand.MatchVisuals();
	...
	private IEnumerator AnimateEquipping(bool equip)
	{
		...
		if (flag)
		{
			HandEquipmentHelper handEquipmentHelper = ((mainHand.CanAnimate || offHand.CanAnimate) ? EquipWeaponSet(m_ActiveSet) : null);
			float num = (Owner.CombatState.IsInCombat ? 0.4f : 100f);
			float num2 = Mathf.Max((handEquipmentHelper?.Duration ?? num) / num, 1f);
			if ((bool)Owner.Features.IsCharging)
			{
				num2 = Mathf.Max(num2, 1.5f);
			}
			if (handEquipmentHelper != null)
			{
				handEquipmentHelper.Scale(num2);
				yield return handEquipmentHelper;
			}
		}
		if (shouldUnequip)
		{
			yield return (mainHand.CanAnimate || offHand.CanAnimate) ? UnequipWeaponSet(m_ActiveSet) : null;
		}
		if (IsDollRoom)
		{
			Character.RequestRebuild(Character.CharacterRebuildMode.OnlyOutfit);
		}
		AreHandsBusyWithAnimation.Value = false;
		m_Coroutine = default(CoroutineHandler);
		HandAnimations.Clear();
	...
	[CanBeNull]
	private HandEquipmentHelper EquipWeaponSet(WeaponSet weaponSet, UnitEquipmentVisualSlotType mainSlot = UnitEquipmentVisualSlotType.None, UnitEquipmentVisualSlotType offSlot = UnitEquipmentVisualSlotType.None)
	{
		HandEquipmentHelper handEquipmentHelper = weaponSet.Equip(AnimationManager, mainSlot, offSlot);
		if (handEquipmentHelper != null)
		{
			HandAnimations.Add(handEquipmentHelper);
		}
		return handEquipmentHelper;
	}

	[CanBeNull]
	private HandEquipmentHelper UnequipWeaponSet(WeaponSet weaponSet)
	{
		HandEquipmentHelper handEquipmentHelper = weaponSet.Unequip(AnimationManager);
		if (handEquipmentHelper != null)
		{
			HandAnimations.Add(handEquipmentHelper);
		}
