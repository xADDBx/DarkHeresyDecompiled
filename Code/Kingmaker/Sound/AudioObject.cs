...
public class AudioObject : RegisteredBehaviour
{
	...
	private void Register()
	{
		...
		AKRESULT aKRESULT = AkUnitySoundEngine.RegisterGameObj(base.gameObject, base.gameObject.name);
	...
	private void UpdatePosition()
	{
		...
		{
			AkUnitySoundEngine.SetObjectPosition(base.gameObject, transform.position, transform.forward, transform.up);
	...
	{
		if (Game.HasInstance && Game.Instance?.CurrentModeType == GameModeType.StarSystem)
		{
			AkUnitySoundEngine.SetScalingFactor(base.gameObject, ConfigRoot.Instance.Sound.StarSystemAudioScalingFactor);
		}
	}
	...
	private void Unregister()
	{
		...
		AkUnitySoundEngine.UnregisterGameObj(base.gameObject);
		foreach (AudioZone item in ObjectRegistry<AudioZone>.Instance.EmptyIfNull())
		{
