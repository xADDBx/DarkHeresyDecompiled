...
public class SubscriptionManager<TSubscriber> : IAstarPooledObject where TSubscriber : class
{
		...
		using (ContextData<StackOverflowProtection>.Request())
		{
			try
			{
				...
				try
				{
					...
					{
						if (i >= subscribersList.List.Count)
						{
							continue;
						}
	...
	private static void ExecuteAction<T>(Action<T> action, object subscriber)
	{
		...
		if (subscriber is T subscriber2)
		{
			ExecuteActionInternal(action, subscriber2);
		}
	}

	private static void ExecuteActionInternal<T>(Action<T> action, T subscriber)
	{
		using EventBusLoopGuard<T> eventBusLoopGuard = EventBusLoopGuard<T>.Request(subscriber);
		if (eventBusLoopGuard.Blocked)
		{
			return;
		}
		if (subscriber is IContextDataProvider contextDataProvider)
		{
