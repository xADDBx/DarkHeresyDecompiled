...
public class ReflectionBasedSerializer
{
	...
	private void GenericObject(ref object obj, Type fieldType = null, bool forceNeedsType = false)
	{
		bool flag = (obj == null && fieldType == null) || IsIdentifiedType(obj?.GetType() ?? fieldType) || (forceNeedsType && fieldType != null && fieldType.HasAttribute<TypeIdAttribute>());
	...
	private static object CreateObject(Type type)
	{
		...
		{
			PFLog.Default.Error("Failed to create object of type " + type.Name);
			throw;
		}
	}
			...
			else
			{
				...
				if (fieldType.IsArray)
				{
					...
					}
					{
						...
						{
							object obj5 = item3;
							GenericObject(ref obj5, elementType, field.HasAttribute<SerializeReference>());
						}
						return;
					}
				}
				if (fieldType.IsList())
				{
					...
					}
					{
						...
						{
							object obj6 = item4;
							GenericObject(ref obj6, fieldType2, field.HasAttribute<SerializeReference>());
						}
						return;
					}
				}
				if (IsIdentifiedType(fieldType) || fieldType.HasAttribute<SerializableAttribute>())
				{
					...
					GenericObject(ref obj7, fieldType, field.HasAttribute<SerializeReference>());
					return;
				}
	...
	private void ReadField(FieldInfo field, object obj)
	{
		...
		else if (fieldType.IsArray)
		{
			...
			{
				object obj5 = null;
				GenericObject(ref obj5, fieldType.GetElementType(), field.HasAttribute<SerializeReference>());
		...
		else if (fieldType.IsList())
		{
			...
			{
				object obj6 = null;
				GenericObject(ref obj6, fieldType.GenericTypeArguments[0], field.HasAttribute<SerializeReference>());
				list3.Add(obj6);
			}
