...
public class StringOrIntEnumConverter : JsonConverter
{
	...
	public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
	{
		...
		case JsonToken.String:
		{
			...
			try
			{
				return Enum.Parse(GetExactTypeToDeserialize(objectType) ?? throw new Exception($"Invalid type {objectType}"), text);
			}
			catch (Exception ex2)
			{
				object valueFormerlySerializedAs = EnumUtils.GetValueFormerlySerializedAs(objectType, text);
				if (valueFormerlySerializedAs != null)
				{
					return valueFormerlySerializedAs;
				}
