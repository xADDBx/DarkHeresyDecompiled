...
[ComponentName("Area/BlueprintAreaPart")]
public class BlueprintAreaPart : BlueprintMechanicEntityFact
{
	...
	[FormerlySerializedAs("StartVeilValueForLocation")]
	...
	public SceneReference StaticScene
	{
		get
		{
			...
			SceneReference staticScene = (Application.isPlaying ? m_SceneOverrides.FirstOrDefault((SceneOverride v) => Game.Instance.EtudesSystem.Etudes.Get(v.Etude)?.IsPlaying ?? false) : default(SceneOverride)).GetStaticScene(isConsolePlatform);
	...
	public SceneReference LightScene
	{
		...
		{
			bool isConsolePlatform = Application.isConsolePlatform;
			SceneReference lightScene = (Application.isPlaying ? m_SceneOverrides.FirstOrDefault((SceneOverride v) => Game.Instance.EtudesSystem.Etudes.Get(v.Etude)?.IsPlaying ?? false) : default(SceneOverride)).GetLightScene(isConsolePlatform);
	...
	public string MainStateName => DynamicScene?.SceneName ?? string.Empty;
