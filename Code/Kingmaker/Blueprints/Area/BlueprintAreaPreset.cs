...
[ComponentName("Area/BlueprintAreaPreset")]
public class BlueprintAreaPreset : BlueprintScriptableObject
{
	...
	public BlueprintCampaign Campaign => m_Campaign?.Get() ?? ConfigRoot.Instance.NewGameSettings.MainCampaign;
	...
	public virtual void SetupState()
	{
		...
		foreach (BlueprintEtudeReference item6 in StartEtudesNonRecursively.EmptyIfNull().NotNull())
		{
			if (!item6.IsEmpty())
			{
				Game.Instance.EtudesSystem.StartEtude(item6.Get(), "preset " + name + ", from StartEtudesNonRecursively");
			}
		}
	...
	private void StartParentEtudeRecursively(BlueprintEtude etude, string origin = "")
	{
		...
		Game.Instance.EtudesSystem.StartEtude(etude, "parent recursive start, origin: " + origin);
	}
