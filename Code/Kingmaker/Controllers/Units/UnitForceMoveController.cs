...
public class UnitForceMoveController : BaseUnitController, IUnitGetAbilityPush, ISubscriber, IGlobalRulebookHandler<RulePerformAttack>, IRulebookHandler<RulePerformAttack>, IGlobalRulebookSubscriber
{
	private struct TransitionChecker : Linecast.ICanTransitionBetweenCells
	{
		...
		public bool CanTransitionBetweenCells(GridNodeBase nodeFrom, GridNodeBase nodeTo, Vector3 transitionPosition, float distanceFactor)
		{
			...
			StoppedOnUnit = nodeTo.GetFirstUnit();
	...
	protected override void TickOnUnit(AbstractUnitEntity entity)
	{
		...
		baseUnitEntity.ForceTurnTo(active.TargetPosition + 2f * (baseUnitEntity.Position - active.TargetPosition));
	...
	{
		GridNodeBase nearestNodeXZ = unit.Position.GetNearestNodeXZ();
		...
		GridNodeBase nearestNodeXZ2 = vector.GetNearestNodeXZ();
	...
	public static void Debug_Force_Move()
	{
		...
		Vector3? virtualPosition = Game.Instance.Controllers.VirtualPositionController.VirtualPosition;
