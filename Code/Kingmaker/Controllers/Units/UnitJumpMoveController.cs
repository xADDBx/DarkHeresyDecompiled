...
public class UnitJumpMoveController : BaseUnitController, IUnitGetAbilityJump, ISubscriber<IBaseUnitEntity>, ISubscriber, IGlobalRulebookHandler<RulePerformAttack>, IRulebookHandler<RulePerformAttack>, IGlobalRulebookSubscriber
{
	...
	private static void HandleResult(MechanicEntity caster, MechanicEntity unit, Vector3 fromPoint, int distanceInCells, bool useAttack)
	{
		...
		if (nearestNodeXZ != null)
		{
			...
			GridNodeBase nearestNodeXZ2 = (unit.Position + (unit.Position - fromPoint).normalized * ((float)distanceInCells * GraphParamsMechanicsCache.GridCellSize)).GetNearestNodeXZ();
