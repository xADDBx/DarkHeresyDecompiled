...
public class UnitRoamingController : IControllerTick, IController, IRoamingTurnEndHandler, ISubscriber
{
	...
	public void Tick()
	{
		...
		{
			if (ShouldTickOnUnit(allUnit, dialogController))
			{
				try
				{
					TickOnUnit(allUnit);
	...
	private void TickOnUnit(AbstractUnitEntity unit)
	{
		...
		else
		{
			...
			BlueprintCutscene blueprintCutscene = point.SelectCutscene(unit.Random);
			if (blueprintCutscene != null)
			{
				CutscenePlayerView cutscenePlayerView = CutscenePlayerView.Play(blueprintCutscene, new ParametrizedContextSetter
				{
					AdditionalParams = { 
					{
						"Unit",
	...
	{
		return Game.Instance.EntityPools.AllBaseUnits.Where((BaseUnitEntity unit) => unit.GetOptional<UnitPartRoaming>() != null);
	}
	...
	private bool ShouldTickOnUnit(AbstractUnitEntity unit, DialogController dialogController)
	{
