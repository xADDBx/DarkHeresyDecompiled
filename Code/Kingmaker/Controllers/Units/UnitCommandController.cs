...
public class UnitCommandController : BaseUnitController
{
	...
	protected sealed override void TickOnUnit(AbstractUnitEntity unit)
	{
		...
		try
		{
			...
			if (flag)
			{
				foreach (AbilityExecutionProcess ability in Game.Instance.Controllers.AbilityExecutor.Abilities)
				{
	...
	private static void TickCommand([NotNull] AbstractUnitCommand command)
	{
		...
		if (!command.IsStarted)
		{
			...
			{
				bool turnBasedModeActive = Game.Instance.Controllers.TurnController.TurnBasedModeActive;
		...
		if (command.IsRunning)
		{
			...
			if (executor != null && !executor.CanAct)
			{
	...
	private static bool ShouldStartCommand(AbstractUnitCommand command)
	{
		...
		if ((executor != null && !executor.CanAct) || (executor.View is UnitEntityView unitEntityView && Game.Instance.Controllers.HandsEquipmentController.IsUpdateScheduledFor(unitEntityView.HandsEquipment)))
		{
			return false;
		}
	...
	private static bool IsCommandWaitingForAnimation(AbstractUnitCommand command)
	{
		...
		bool flag = unitEntityView != null && Game.Instance.Controllers.HandsEquipmentController.IsUpdateScheduledFor(unitEntityView.HandsEquipment);
