...
public class DebugDialogPlayer : MonoBehaviour, IAreaActivationHandler, ISubscriber
{
	...
	private void PlaceUnit(BlueprintUnit blueprint)
	{
		...
		{
			BaseUnitEntity baseUnitEntity = Game.Instance.EntityPools.AllBaseUnits.FirstOrDefault((BaseUnitEntity u) => u.Blueprint == blueprint);
			if (baseUnitEntity == null)
			{
				SceneEntitiesState mainState = Game.Instance.LoadedAreaState.MainState;
				baseUnitEntity = Game.Instance.Controllers.EntitySpawner.SpawnUnit(blueprint, Vector3.zero, Quaternion.identity, mainState);
			}
