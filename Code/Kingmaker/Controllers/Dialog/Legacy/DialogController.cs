...
namespace Kingmaker.Controllers.Dialog.Legacy;
...
public class DialogController : IControllerTick, IController, IControllerStart, IControllerStop, IControllerReset, IAreaHandler, ISubscriber, IDialogControllerStartScheduledDialogImmediately
{
	...
	private CueShowData m_PendingCueHistoryEntry;
	...
	[NotNull]
	public readonly List<AlignmentShift> AlignmentShifts = new List<AlignmentShift>();
	...
	public BlueprintCue CurrentCue
	{
		...
		private set
		{
			...
			CurrentSpeaker = m_CurrentCue.Speaker.GetSpeaker(m_CurrentCue) ?? FirstSpeaker;
			PlayAnimation(CurrentSpeaker, m_CurrentCue);
			AddInvolvedUnit(CurrentSpeaker);
			m_RevealedSpeaker = null;
			if (!m_CurrentCue.Speaker.NotRevealInFoW)
			{
				BaseUnitEntity currentSpeaker = CurrentSpeaker;
				if (currentSpeaker != null && currentSpeaker.IsInGame)
				{
	...
	[CanBeNull]
	public MapObjectEntity MapObject { get; set; }
	...
	public BlueprintUnit CurrentSpeakerBlueprint
	{
		get
		{
			BlueprintCue currentCue = CurrentCue;
			object obj;
			if (currentCue == null || !currentCue.Speaker.NoSpeaker)
			{
				obj = CurrentCue?.Speaker.GetSpeakerBlueprint();
				if (obj == null)
				{
	...
	private void StartScheduledDialogImmediately([NotNull] DialogData scheduled)
	{
		...
		MapObjectEntity entity = scheduled.MapObject.Entity;
		...
		if (IsSpeakerBusyInCutscene(baseUnitEntity, out var cutscene))
		{
			flag = false;
			DialogDebug.Add(dialog, $"first speaker {baseUnitEntity.Blueprint} is busy in cutscene {cutscene.Cutscene} ({cutscene.Cutscene.AssetGuid})", Color.red);
		}
		using ((baseUnitEntity != null) ? ContextData<ClickedUnitData>.Request().Setup(baseUnitEntity) : null)
		{
			using ((entity != null) ? ContextData<MechanicEntityData>.Request().Setup(entity) : null)
			{
				if (!flag)
				{
		...
		DialogPosition = GetStartPosition(Dialog, FirstSpeaker, MapObject);
		...
		Game.Instance.DialogState.ShownDialogsAdd(Dialog);
	}
	...
	{
		if (Game.Instance?.CurrentlyLoadedArea != null && !(CameraRig == null) && CurrentSpeaker != null && m_CurrentCue?.Speaker != null && m_CurrentCue.Speaker.MoveCamera)
		{
			CameraRig.SetWorldOffset(ConfigRoot.Instance.Dialog.DialogCameraCorrection);
			float valueOrDefault = (ConfigRoot.Instance?.Dialog?.GetCameraOffsetBySize(CurrentSpeaker.Size)).GetValueOrDefault();
			Vector3 position = CurrentSpeaker.Position + Vector3.up * valueOrDefault;
			CameraRig.ScrollTo(position);
		}
	}

	private static Vector3 GetStartPosition([NotNull] BlueprintDialog dialog, [CanBeNull] BaseUnitEntity firstSpeaker, [CanBeNull] MapObjectEntity mapObject)
	{
		if (dialog.StartPosition != null)
		{
			try
			{
				return dialog.StartPosition.GetValue();
			}
			catch (Exception ex)
			{
		...
		{
			return firstSpeaker.Position;
		}
		if (mapObject != null)
		{
			return mapObject.ViewPosition;
		}
		if (!Game.Instance.IsModeActive(GameModeType.GlobalMap))
		{
			return Game.Instance.Player.MainCharacter.Entity.Position;
		}
		return CameraRig.Instance.transform.position;
	}
	...
	public void StopDialog()
	{
		...
		if (instance != null)
		{
			instance.SetWorldOffset(Vector2.zero);
		}
	...
	public void SelectAnswer(BlueprintAnswer answer, BaseUnitEntity manualUnitSelection = null)
	{
		...
		Game.Instance.DialogState.SelectedAnswersAdd(answer);
		LocalSelectedAnswers.Add(answer);
		Dictionary<BlueprintDialog, List<BlueprintScriptableObject>> bookEventLog = Game.Instance.DialogState.BookEventLog;
	...
	private void AddHistoryEntry([NotNull] BlueprintAnswer answer)
	{
		if (answer.AddToHistory)
		{
			...
			AnswerShowData answerShowData = new AnswerShowData(answer, mainCharacterEntity.CharacterName, mainCharacterEntity.Blueprint);
			History.Add(answerShowData);
	...
	private void AddCueHistoryEntry()
	{
		if (m_PendingCueHistoryEntry != null)
		{
			History.Add(m_PendingCueHistoryEntry);
			EventBus.RaiseEvent(delegate(IDialogHistoryHandler h)
			{
				h.HandleOnDialogHistory(m_PendingCueHistoryEntry);
			});
	...
	private void PlayCue(BlueprintCueBase cue)
	{
		...
		Game.Instance.DialogState.ShownCuesAdd(cue);
		m_Answers.Clear();
		m_ContinueCue = null;
	...
	private void AddAnswers([NotNull] IEnumerable<BlueprintAnswerBase> answers, [CanBeNull] BlueprintCueBase continueCue)
	{
		if (continueCue == null)
		{
			foreach (BlueprintAnswerBase answer2 in answers)
			{
				...
				{
					Game.Instance.DialogState.ShownAnswerListsAdd(blueprintAnswersList);
	...
	private void PlayBasicCue(BlueprintCue cue)
	{
		...
		if (!PlayingBookPage)
		{
			...
			m_PendingCueHistoryEntry = ((!string.IsNullOrEmpty(CurrentSpeakerName)) ? new CueShowData(cue, CurrentSpeakerName, CurrentSpeakerBlueprint) : new CueShowData(cue));
		...
		else
		{
			...
			Game.Instance.DialogState.BookEventLog[Dialog].Add(cue);
		}
