...
public class DialogController : IControllerTick, IController, IControllerStart, IControllerStop, IControllerReset, IAreaHandler, ISubscriber, IDialogControllerStartScheduledDialogImmediately
{
	...
	[CanBeNull]
	public MapObjectView MapObject { get; set; }
	...
	private void AddHistoryEntry([NotNull] BlueprintAnswer answer)
	{
		...
		{
			AnswerShowData answerShowData = new AnswerShowData(answer, Game.Instance.Player.MainCharacterEntity.Blueprint);
	...
	private void PlayCheck(BlueprintCheck check)
	{
		...
		if (skillCheckResult.Passed)
		{
			...
			Experience.GainForSkillCheck(skillCheckResult.DC, skillCheckResult.ActingUnit);
	...
	{
		DialogAnimation animation = cue.Animation;
		if (!(unit?.View == null) && !(unit.View.AnimationManager == null) && animation != 0)
		{
			...
			m_CurrentSpeakerAnimation.AnimationClipWrapper = ((animation == DialogAnimation.Custom) ? cue.CustomAnimation : null);
