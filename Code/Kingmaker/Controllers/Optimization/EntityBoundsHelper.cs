...
public static class EntityBoundsHelper
{
	...
	private static PhysicsScene2D PhysicsScene => Game.Instance.Controllers.EntityBoundsController.PhysicsScene;
	...
	public static List<MechanicEntity> FindMechanicEntitiesInRange(Vector3 origin, float radius, Comparison<Entity> comparison = null)
	{
		using (ProfileScope.New("EntityBoundsHelper.FindMechanicEntitiesInRange"))
		{
			int count = OverlapCircle(origin.To2D(), radius, OverlapResults, 16777216);
			return GetDataList(OverlapResults, count, (Comparison<MechanicEntity>)comparison);
		}
	}
