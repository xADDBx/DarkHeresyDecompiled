...
public class EntityViewTrigger : MonoBehaviour
{
	...
	{
		if (EntityDataLink.GetEntity(other) is BaseUnitEntity baseUnitEntity && baseUnitEntity != Unit)
		{
			if (Unit.Vision.CanBeInRange.ContainsKey(baseUnitEntity.UniqueId))
			{
				EntityBoundsController.Logger.Error("Cant add {0} to vision of {1} - already added", baseUnitEntity, Unit);
			}
			else
			{
				Unit.Vision.CanBeInRange.Add(baseUnitEntity.UniqueId, baseUnitEntity);
			}
		}
	}
	...
	{
		if (EntityDataLink.GetEntity(other) is BaseUnitEntity baseUnitEntity && baseUnitEntity != Unit && !Unit.Vision.CanBeInRange.Remove(baseUnitEntity.UniqueId))
		{
