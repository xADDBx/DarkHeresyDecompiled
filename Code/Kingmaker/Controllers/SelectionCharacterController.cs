...
public class SelectionCharacterController : IControllerStart, IController, IControllerEnable, IControllerTick, IControllerStop, IControllerReset, IFullScreenUIHandler, ISubscriber, IFullScreenUIHandlerWorkaround, IPartyHandler, ISubscriber<IBaseUnitEntity>, ITurnBasedModeHandler, IForceShowActionBarUIHandler
{
	...
	public readonly ReactiveProperty<bool> IsMainCharacterSelected = new ReactiveProperty<bool>();

	public readonly ReactiveCommand<Unit> ActualGroupUpdated = new ReactiveCommand<Unit>();
	...
	{
		m_NeedUpdate = true;
		SelectionManagerFacade.SelectAll(Game.Instance.Player.PartyCharacters.Select((UnitReference r) => r.Entity.ToBaseUnitEntity()).Where(UtilityUnit.IsViewActiveUnit));
		if (SelectedUnits.Count == 0)
		{
			if (Game.Instance.Player.IsInCombat)
			{
				SetSelected(Game.Instance.Player.PartyAndPets.FirstOrDefault(), force: true);
			}
			else
			{
				SelectionManagerFacade.SelectAll();
			}
		}
		UpdateSelectedUnits();
		...
		m_SelectedUnitsSubscription.Add(ObservableSubscribeExtensions.Subscribe(SelectedUnits.ObserveReset(), delegate
		{
			m_NeedUpdate = true;
		}));
	...
	public void SetSelected(BaseUnitEntity unit, bool force = false, bool forceFullScreenState = false)
	{
		...
		ISelectionManager selectionManager = SelectionManagerFacade.SelectionManager;
		if (selectionManager != null)
		{
			selectionManager.SwitchSelectionUnitInGroup(unit, canAddToSelection: true, force);
		}
	...
	private void UpdateSelectedUnits()
	{
		...
		IsMainCharacterSelected.Value = SelectedUnits.FirstOrDefault((BaseUnitEntity u) => u.IsMainCharacter) != null;
	}
	...
	public void ReselectCurrentUnit()
	{
		...
		{
			SingleSelectedUnit.Value = (Game.Instance.IsControllerMouse ? FirstSelectedUnit : SelectedUnit.Value);
			SingleSelectedUnit.ForceNotify();
		}
		else
		{
			SingleSelectedUnit.Value = null;
	...
	public void HandleTurnBasedModeSwitched(bool isTurnBased)
	{
		...
		{
			SetSelected(Game.Instance.Player.PartyAndPets.FirstOrDefault());
		}
	}
