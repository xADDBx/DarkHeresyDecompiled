...
public class TimerController : IControllerEnable, IController, IControllerTick, ITimerHandler, ISubscriber
{
	...
	public void Tick()
	{
		...
		{
			if (!(m_Timers[num].runningTime > Game.Instance.Controllers.TimeController.GameTime))
			{
				try
				{
	...
	{
		m_Timers.Add((timer, Game.Instance.Controllers.TimeController.GameTime + timer.TimerTime));
	}
}
