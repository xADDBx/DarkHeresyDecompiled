...
public class FootprintsController : IControllerTick, IController, IControllerStop, IUnitFootstepAnimationEventHandler, ISubscriber<IAbstractUnitEntity>, ISubscriber
{
	...
	private static GameObject GetFootprintPrefab(AbstractUnitEntity unit, int footIndex)
	{
		...
		if (num == 0)
		{
			...
			if (name.Contains("_HM_", StringComparison.InvariantCultureIgnoreCase))
			{
				gameObject = ConfigRoot.Instance.FxRoot.DefaultHumanFootprint.Load();
			}
			else if (name.Contains("_EL_", StringComparison.InvariantCultureIgnoreCase))
			{
				gameObject = ConfigRoot.Instance.FxRoot.DefaultEldarFootprint.Load();
			}
			else if (name.Contains("marine", StringComparison.InvariantCultureIgnoreCase))
			{
				gameObject = ConfigRoot.Instance.FxRoot.DefaultSpaceMarineFootprint.Load();
			}
			else if (name.Contains("_KR_", StringComparison.InvariantCultureIgnoreCase))
			{
				gameObject = ConfigRoot.Instance.FxRoot.DefaultKrootFootprint.Load();
			}
			else
			{
				if (!name.Contains("_OG_", StringComparison.InvariantCultureIgnoreCase))
				{
					return null;
				}
				gameObject = ConfigRoot.Instance.FxRoot.DefaultOgrynFootprint.Load();
			}
		}
		else
		{
