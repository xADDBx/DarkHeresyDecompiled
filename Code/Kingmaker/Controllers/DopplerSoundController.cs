...
public class DopplerSoundController : IControllerTick, IController, IProjectileLaunchedHandler, ISubscriber, IProjectileHitHandler
{
	...
	public void Tick()
	{
		...
		foreach (KeyValuePair<Projectile, uint> projectile3 in m_Projectiles)
		{
			...
			if (projectile.Ability?.FXSettings?.SoundFXSettings != null && projectile.Ability.FXSettings.SoundFXSettings.DopplerRTPC != "" && AkUnitySoundEngine.SetRTPCValueByPlayingID(projectile.Ability.FXSettings.SoundFXSettings.DopplerRTPC, in_value, in_playingID) == AKRESULT.AK_PlayingIDNotFound)
			{
				list.Add(projectile);
			}
		}
