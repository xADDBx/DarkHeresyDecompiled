...
public class EntityDestructionController : IControllerTick, IController, IControllerStop
{
	...
	{
		if (entity == null || (entity.HoldingState == null && Game.Instance.Controllers.EntitySpawner.TryRemoveFromSpawnQueue(entity)))
		{
			entity?.Dispose();
			return;
		}
	...
	{
		if (!m_CleanupAwakeUnits && unit != null && Game.Instance.EntityPools.AllBaseAwakeUnits.HasItem(unit))
		{
