...
public class TimeController : IControllerEnable, IController, IControllerDisable, IControllerTick, IControllerStart, IControllerStop, ITimeController, InterfaceService
{
	...
	public bool IsGameDeltaTimeZero
	{
		get
		{
			if (Game.Instance.CurrentModeType == GameModeType.Default || Game.Instance.CurrentModeType == GameModeType.SpaceCombat || Game.Instance.CurrentModeType == GameModeType.Cutscene)
			{
				return Mathf.Approximately(Time.timeScale, 0f);
			}
			if (Game.Instance.CurrentModeType == GameModeType.StarSystem || Game.Instance.CurrentModeType == GameModeType.GlobalMap)
			{
	...
	public void Tick()
	{
		...
		else
		{
			...
			else if (Game.Instance.CurrentModeType == GameModeType.StarSystem || Game.Instance.CurrentModeType == GameModeType.GlobalMap)
			{
	...
	public void OnStart()
	{
		InterfaceServiceLocator.RegisterService(this, typeof(ITimeController));
	}
