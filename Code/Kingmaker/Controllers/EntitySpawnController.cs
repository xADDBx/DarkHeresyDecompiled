...
public class EntitySpawnController : IControllerTick, IController, IControllerReset
{
	...
	private Entity SpawnEntityWithView(EntityViewBase prefab, Vector3 position, Quaternion rotation, [CanBeNull] SceneEntitiesState state)
	{
		...
		entityViewBase.UniqueId = Uuid.Instance.CreateGuidForSceneObject(entityViewBase);
	...
	public Entity SpawnEntityWithView(EntityViewBase view, [CanBeNull] SceneEntitiesState state, bool moveView = true)
	{
		...
		using (ContextData<EntitySpawnData>.Request().Setup(view, state))
		{
			...
			SpawnEntity(entity, state, moveView);
	...
	public void SpawnEntity([NotNull] Entity entity, [CanBeNull] SceneEntitiesState state, bool moveView = false)
	{
		if (state == null)
		{
			state = Game.Instance.State.LoadedAreaState.MainState;
	...
	private void MoveView([NotNull] Entity entity, [NotNull] SceneEntitiesState state)
	{
		if (entity.View != null)
		{
	...
	public BaseUnitEntity RecruitNPC([CanBeNull] BaseUnitEntity npc, BlueprintUnit companionBlueprint)
	{
		...
		else
		{
			...
			{
				Game.Instance.Controllers.EntityDestroyer.Destroy(npc);
		...
		if (!baseUnitEntity.IsPet)
		{
			...
			Metrics.Recruitment.Id(companionBlueprint?.AssetGuid).RecruitState(RecruitMetricsEvent.CompanionRecruitStates.Recruit).Send();
		}
