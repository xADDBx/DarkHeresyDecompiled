...
public class SynchronizedDataController : IControllerTick, IController, IControllerReset
{
	...
	{
		float num = ((maxTime > 0f) ? maxTime : 60f);
	...
	private void FillData()
	{
		...
		synchronizedData.maxLag = Game.Instance.Controllers.TimeSpeedController.MaxLag;
		synchronizedData.stateHash = Game.Instance.Controllers.SyncStateCheckerController.StateHash;
	...
	public void PushLeftStickMovement(BaseUnitEntity unit, Vector2 moveDirection, float stickDeflection)
	{
		...
		{
			int count = Game.Instance.Controllers.SelectionCharacter.SelectedUnits.Count;
			UnitReference[] array = new UnitReference[count];
			for (int i = 0; i < count; i++)
			{
				array[i] = Game.Instance.Controllers.SelectionCharacter.SelectedUnits[i].FromBaseUnitEntity();
			}
			return array;
		}
	}
	...
	private SynchronizedData FillLeftStickData(SynchronizedData data)
	{
		if (m_HasLeftStickMovementData || m_LeftStickMovementDataFrame == Time.frameCount)
		{
			...
			Game.Instance.Controllers.MovePredictionController.PushLeftStickMovement(version, m_Unit, moveDirection, stickDeflection);
			m_HasLeftStickMovementData = false;
