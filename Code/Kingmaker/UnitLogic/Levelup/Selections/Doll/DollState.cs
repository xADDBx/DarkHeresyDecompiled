...
public class DollState : ICanConvertPropertiesToReactive
{
	...
	private ReactiveCommand<Unit> m_UpdateCommand;
	...
	public ReactiveCommand<Unit> UpdateCommand
	{
		get
		{
			if (m_UpdateCommand == null)
			{
				m_UpdateCommand = new ReactiveCommand<Unit>();
				ObservableSubscribeExtensions.Subscribe(UpdateCommandOnLateUpdate.DebounceFrame(1, UnityFrameProvider.PreLateUpdate), delegate
				{
	...
	private ReactiveCommand<Unit> UpdateCommandOnLateUpdate { get; } = new ReactiveCommand<Unit>();
	...
	{
		return UtilityChargen.GetClothesColorsProfile(Clothes)?.Ramps ?? s_EmptyRamps;
	}

	public List<Texture2D> GetOutfitRampsSecondary()
	{
		return UtilityChargen.GetClothesColorsProfile(Clothes, secondary: true)?.Ramps ?? s_EmptyRamps;
	}
	...
	private static List<Texture2D> GetRamps(EEAdapter ee, bool primary = true)
	{
		...
		return (primary ? ee.Load().PrimaryColorsProfile : ee.Load().SecondaryColorsProfile).Ramps;
	}
