		...
		if (!(selection is BlueprintSelectionFeature blueprint))
		{
			if (!(selection is BlueprintSelectionDoll blueprint2))
			{
				if (!(selection is BlueprintCharacterNameSelection blueprint3))
				{
					...
					{
						if (!(selection is BlueprintVoiceSelection blueprint5))
						{
							if (!(selection is BlueprintGenderSelection blueprint6))
							{
								if (selection is BlueprintSelectionStats blueprint7)
								{
									return new SelectionStateStats(manager, blueprint7, path, level);
								}
								throw new ArgumentOutOfRangeException(selection.GetType().ToString());
							}
							return new SelectionStateGender(manager, blueprint6, path, level);
						}
						return new SelectionStateVoice(manager, blueprint5, path, level);
					}
