...
public class AbilityTargetSelectionByDistance : AbilitySelectTarget
{
	...
	{
		IEnumerable<BaseUnitEntity> enumerable = Game.Instance.EntityPools.AllBaseUnits.Where((BaseUnitEntity p) => !p.Features.IsUntargetable && !p.LifeState.IsDead && p.IsInCombat);
