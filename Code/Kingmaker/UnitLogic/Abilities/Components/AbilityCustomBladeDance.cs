...
[ComponentName("Ability/Custom/AbilityCustomBladeDance")]
...
public class AbilityCustomBladeDance : AbilityCustomLogic
{
	...
	public override IEnumerator<AbilityDeliveryTarget> Deliver(AbilityExecutionContext context, TargetWrapper targetWrapper)
	{
		...
		List<BaseUnitEntity> possibleNodsWithDirection = Game.Instance.EntityPools.AllBaseUnits.Where((BaseUnitEntity p) => !p.Features.IsUntargetable && !p.LifeState.IsDeadOrUnconscious && p.IsInCombat && p != caster && p.InRangeInCells(caster, 1)).ToList();
		int burstCounter = 0;
		...
		while (burstCounter < Math.Min(RateOfAttack.Calculate(context), context.Ability.OverrideRateOfFire))
		{
			...
			caster.ForceTurnTo(baseUnitEntity.Position);
