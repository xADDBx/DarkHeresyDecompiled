...
public class CastAbilityOnMultipleTargetsConsequently : AbilityCustomLogic, IAbilityAoEPatternProviderHolder
{
	...
	private IEnumerable<MechanicEntity> EnumerateTargets(AbilityExecutionContext context, TargetWrapper target)
	{
		if (!IsPattern)
		{
			return Game.Instance.EntityPools.AllBaseAwakeUnits.Where((BaseUnitEntity i) => TargetRestriction.IsPassed(context, i));
		}
		Size targetSizeForPattern = context.Ability.GetTargetSizeForPattern(target);
		return from i in m_PatternSettings.GetOrientedPattern(context.Ability, context.Caster.CurrentUnwalkableNode, target.NearestNode, targetSizeForPattern, coveredTargetsOnly: true).Nodes.Select((GridNodeBase i) => i.GetFirstUnit()).NotNull()
