...
[ComponentName("Caster Restriction/AbilityResourceLogic")]
...
public class AbilityResourceLogic : BlueprintComponent, IAbilityResourceLogic, IAbilityRestriction
{
	...
	public virtual void Spend(AbilityData ability)
	{
		...
		else if (!caster.IsCheater)
		{
			...
			{
				ability.Caster.GetAbilityResourcesOptional()?.Spend(RequiredResource, CalculateCost(ability));
			}
		}
	}
	...
	public virtual int CalculateCost(AbilityData ability)
	{
		if (CostIsCustom)
		{
			...
			if (component == null)
			{
				PFLog.Default.Error(ability.Blueprint.OriginalBlueprint, $"Custom resource cost calculator is missing: {ability.Blueprint}");
				return 1;
			}
