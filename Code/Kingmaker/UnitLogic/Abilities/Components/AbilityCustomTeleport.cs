...
[ComponentName("Ability/Movement/AbilityCustomTeleport")]
...
public class AbilityCustomTeleport : AbilityCustomLogic, IAbilityTargetRestriction
{
	...
	public override IEnumerator<AbilityDeliveryTarget> Deliver(AbilityExecutionContext context, TargetWrapper target)
	{
		...
		if (FindPosition)
		{
			...
			IEnumerable<BaseUnitEntity> enemies = Game.Instance.EntityPools.AllBaseUnits.Where((BaseUnitEntity unit) => caster.IsEnemy(unit) && caster.IsConscious);
			...
			foreach (Vector2Int item in hashSet)
			{
				...
				foreach (GridNodeBase occupiedNode in caster.GetOccupiedNodes())
				{
					...
					node = gridGraph.GetNode(xCoordinateInGrid + item.x, zCoordinateInGrid + item.y);
				...
				if (flag)
				{
					int xCoordinateInGrid2 = ((GridNodeBase)caster.CurrentNode.node).XCoordinateInGrid;
					int zCoordinateInGrid2 = ((GridNodeBase)caster.CurrentNode.node).ZCoordinateInGrid;
					hashSet2.Add(((GridGraph)caster.CurrentNode.node.Graph).GetNode(xCoordinateInGrid2 + item.x, zCoordinateInGrid2 + item.y));
				}
			}
			GraphNode node2 = (RunFromEnemies ? hashSet2.MaxBy((GraphNode node) => DistanceToClosestEnemyInCells((GridNodeBase)node, caster.SizeRect, enemies)) : hashSet2.MinBy((GraphNode node) => DistanceToClosestEnemyInCells((GridNodeBase)node, caster.SizeRect, enemies)));
			settings.Targets.Clear();
			settings.Targets = new List<BaseUnitEntity> { caster };
			target = new TargetWrapper(node2.Vector3Position());
		}
		...
		bool UsableForLanding(GridNodeBase landingNode)
		{
			...
			{
				if (node.TryGetFirstUnit(out var unit2) && unit2.IsConscious)
				{
					return unit2 == caster;
				}
				return true;
	...
	private static IEnumerator<AbilityDeliveryTarget> Deliver(AbilityExecutionContext context, TeleportSettings settings, BaseUnitEntity caster, TargetWrapper target)
	{
		...
		if (target.Entity != null)
		{
			...
			point += Quaternion.Euler(0f, target.Entity.Orientation, 0f) * Vector3.forward * num;
		}
	...
	{
		return ConfigRoot.Instance.LocalizedTexts.Reasons.NotEnoughSpace;
	}
}
