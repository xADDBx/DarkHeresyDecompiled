...
public class CustomAbilityQueue : AbilityCustomLogic
{
	...
	public override IEnumerator<AbilityDeliveryTarget> Deliver(AbilityExecutionContext context, TargetWrapper target)
	{
		...
		{
			List<MechanicEntity> list = Game.Instance.EntityPools.AllBaseUnits.Where((BaseUnitEntity p) => !p.Features.IsUntargetable && !p.LifeState.IsDead && p.IsInCombat && IsConditionPassed(ConditionsOnTarget, context, p) && context.Caster.InRangeInCells(p, Range.Calculate(context))).Cast<MechanicEntity>().ToList();
		...
		while (!cmdHandle.IsFinished || !commands.Empty)
		{
			yield return null;
		}
		if (IsConditionPassed(QueConditions, context, context.Caster))
		{
			...
			while (!nextCmdHandle.IsFinished || !commands.Empty)
			{
				yield return null;
			}
		}
