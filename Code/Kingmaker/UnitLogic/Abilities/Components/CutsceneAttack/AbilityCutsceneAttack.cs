...
[ComponentName("Cutscene/AbilityCutsceneAttack")]
...
public class AbilityCutsceneAttack : AbilityCustomLogic
{
	...
	public DamageSettings DamageForFX;
	...
	{
		IntermediateDamage baseDamageOverride = (UseCustomDamageForFX ? DamageForFX.CreateDamage() : null);
		IntermediateDamage resultDamage = Rulebook.Trigger(new RuleCalculateDamage(context.Caster, target.Entity, context.Ability, null, baseDamageOverride)).ResultDamage;
		RolledDamage damage = ((context.DamagePolicy == DamagePolicyType.Default && target.Entity != null) ? Rulebook.Trigger(new RuleDealDamage(context.Caster, target.Entity, resultDamage)).ResultDamage : new RolledDamage(context.Caster, target.Entity, resultDamage, 50));
		HitFXPlayer.PlayDamageHit(context.Caster, target.Entity, context.ClickedTarget, projectile, context.Ability, damage, isCritical: false, isDot: false);
	}
}
