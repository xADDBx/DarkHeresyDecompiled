...
[Serializable]
[ComponentName("Ability/Custom/AbilityCustomDirectedFirestorm")]
...
public class AbilityCustomDirectedFirestorm : AbilityCustomLogic
{
	...
	public override IEnumerator<AbilityDeliveryTarget> Deliver(AbilityExecutionContext context, TargetWrapper targetWrapper)
	{
		...
		IEnumerable<AbstractUnitEntity> enumerable2 = Game.Instance.EntityPools.AllUnits.Where((AbstractUnitEntity p1) => !p1.Features.IsUntargetable && !p1.LifeState.IsDead && p1.IsInCombat && p1.IsAlly(caster));
		...
		ContextActionAttackNearestTarget.PriorityTargetAttackSelectType attackType = ((caster.Facts.Contains(Upgrade3Fact) && caster.Facts.Contains(Upgrade4Fact)) ? ContextActionAttackNearestTarget.PriorityTargetAttackSelectType.LowestCostButBurstOrAoE : (caster.Facts.Contains(Upgrade4Fact) ? ContextActionAttackNearestTarget.PriorityTargetAttackSelectType.LowestCostButAoE : ((!caster.Facts.Contains(Upgrade3Fact)) ? ContextActionAttackNearestTarget.PriorityTargetAttackSelectType.LowestCost : ContextActionAttackNearestTarget.PriorityTargetAttackSelectType.LowestCostButBurst)));
		List<AbstractUnitEntity> list = enumerable2.Where((AbstractUnitEntity p1) => enemies.Any((UnitEntity p2) => LosCalculations.GetWarhammerLos(p1, p2).CoverType != LosCalculations.CoverType.LosBlocker && ContextActionAttackNearestTarget.SelectAttackAbility(p1, p2, attackType, new List<BlueprintAbilityGroupReference>()) != null)).ToList();
		...
		{
			UnitEntity unitEntity2 = enemies.Where((UnitEntity p) => LosCalculations.GetWarhammerLos(ally, p).CoverType != LosCalculations.CoverType.LosBlocker && ContextActionAttackNearestTarget.SelectAttackAbility(ally, p, attackType, new List<BlueprintAbilityGroupReference>()) != null).ToList().Random(PFStatefulRandom.Mechanics);
			Ability ability = ContextActionAttackNearestTarget.SelectAttackAbility(ally, unitEntity2, attackType, new List<BlueprintAbilityGroupReference>());
			if (ability != null)
			{
