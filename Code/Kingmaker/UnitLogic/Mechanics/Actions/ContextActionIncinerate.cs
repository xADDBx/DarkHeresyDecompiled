...
public class ContextActionIncinerate : ContextAction
{
	...
	protected override void RunAction()
	{
		...
		List<BaseUnitEntity> list = Game.Instance.EntityPools.AllBaseUnits.Where((BaseUnitEntity p) => !p.Features.IsUntargetable && !p.LifeState.IsDead && p.IsInCombat && p.Facts.List.Any((EntityFact fact) => buffs.Contains(fact.Blueprint))).ToList();
