...
public class ContextActionOnRandomTargetsAround : ContextAction
{
	...
	public bool AffectAllTargets;

	[HideIf("AffectAllTargets")]
	...
	protected override void RunAction()
	{
		...
		IEnumerable<GridNodeBase> nodesSpiralAround = GridAreaHelper.GetNodesSpiralAround(base.Target.NearestNode, base.Target.SizeRect, TilesRadius);
		...
		foreach (GridNodeBase item in nodesSpiralAround)
		{
			...
			if (firstUnit != null && firstUnit.IsInCombat == isInCombat && !list.HasItem(firstUnit))
			{
				list.Add(firstUnit);
			}
		}
		...
		if (LowestWounds)
		{
			...
			{
				if (!item2.IsDeadOrUnconscious && item2.Health.HitPointsLeft < lowest)
				{
		...
		int num = (AffectAllTargets ? list.Count : NumberOfTargets);
