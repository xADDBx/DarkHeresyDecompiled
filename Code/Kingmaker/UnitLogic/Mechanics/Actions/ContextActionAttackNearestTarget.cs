...
public class ContextActionAttackNearestTarget : ContextAction
{
	...
	protected override void RunAction()
	{
		...
		List<MechanicEntity> list = Game.Instance.EntityPools.AllUnits.Where((AbstractUnitEntity p) => !p.Features.IsUntargetable && !p.LifeState.IsDead && p.IsInCombat && p.Health.HitPointsLeft > 0).Cast<MechanicEntity>().ToList();
		list.Remove(target);
		if (OnlyEnemies)
		{
