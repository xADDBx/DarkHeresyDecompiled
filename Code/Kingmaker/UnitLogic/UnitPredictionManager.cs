...
public class UnitPredictionManager : MonoBehaviour, IUnitCommandEndHandler, ISubscriber<IMechanicEntity>, ISubscriber, IPartyCombatHandler, IAbilityTargetSelectionUIHandler, IUnitDirectHoverUIHandler, ITurnStartHandler, IInterruptTurnStartHandler, IUnitPathManagerHandler, IUnitMovableAreaHandler, IAbilityTargetHoverUIHandler, IUnitActionPointsHandler
{
	...
	private Action m_UpdateAttackOfOpportunityPrediction;
	...
	[CanBeNull]
	private BaseUnitEntity CurrentUnit => Game.Instance.Controllers.TurnController.CurrentUnit as BaseUnitEntity;
	...
	private void Start()
	{
		...
		m_UpdateAttackOfOpportunityPrediction = UpdateAttackOfOpportunityPrediction;
	}
	...
	public UnitHologram GetHologram()
	{
		if (m_VirtualHologram != null)
		{
			return m_VirtualHologram;
		}
		return null;
	}
	...
	public void SetHologramPosition(BaseUnitEntity unit, Vector3 position, Vector3 direction)
	{
		...
		UpdateVirtualLoSPosition();
		EventBus.RaiseEvent(delegate(IHologramPositionChangedHandler h)
		{
			h.HandleHologramPositionChanged();
		});
		CombatHUDRenderer.Instance.UpdateLosBlocker();
	}
	...
	private Vector3? GetBestShootingPosition()
	{
		...
		Vector3 desiredPosition = Game.Instance.Controllers.VirtualPositionController.GetDesiredPosition(m_SelectedAbility.Caster);
		GridNodeBase gridNodeBase = vector?.GetNearestNodeXZUnwalkable();
		GridNodeBase nearestNodeXZUnwalkable = desiredPosition.GetNearestNodeXZUnwalkable();
		if (m_SelectedAbility.Caster.IsUnitPositionContainsNode(desiredPosition, gridNodeBase))
		{
			return nearestNodeXZUnwalkable?.Vector3Position();
		}
		if (gridNodeBase == nearestNodeXZUnwalkable)
		{
			return null;
		}
		return gridNodeBase?.Vector3Position();
	}
	...
	{
		if (Game.Instance.Controllers.VirtualPositionController == null)
		{
			return;
		}
		...
		{
			Game.Instance.Controllers.VirtualPositionController.VirtualPosition = m_VirtualHoverPosition.Value;
		}
		else if (m_VirtualHologramPosition.HasValue)
		{
			Game.Instance.Controllers.VirtualPositionController.VirtualPosition = m_VirtualHologramPosition.Value;
			if (m_VirtualHologramDirection.HasValue)
			{
				Game.Instance.Controllers.VirtualPositionController.VirtualRotation = m_VirtualHologramDirection.Value;
			}
		}
		else
		{
			Game.Instance.Controllers.VirtualPositionController.CleanVirtualPosition();
		}
	}
	...
	private void CreateHologramIfNeeded([CanBeNull] BaseUnitEntity unit)
	{
		...
		if (!((unit.Position - vector).sqrMagnitude < 1f) && (m_VirtualHologramPosition.HasValue || !unit.GetOccupiedNodes().Contains(vector.GetNearestNodeXZ())) && (m_VirtualHologram == null || m_VirtualHologram.Parent != unit))
		{
			...
			{
				m_VirtualHologram = unit.CreateHologram();
			}
		}
	}
	...
	private static void UpdateHologram(UnitHologram hologram, Vector3? position, Vector3? direction, bool threatening)
	{
		...
		{
			BaseUnitEntity baseUnitEntity = (from u in Game.Instance.EntityPools.AllBaseUnits
			...
			{
				Vector3 position2 = baseUnitEntity.View.transform.position;
				position2.y = hologram.transform.position.y;
				hologram.LookAt(position2);
			}
		}
		else
		{
	...
	private void ClearHologram()
	{
		...
		EventBus.RaiseEvent(delegate(IHologramClearHandler h)
		{
			h.HandleHologramClear();
		});
		ClearMoveCost();
	}
	...
	{
		if (CurrentUnit != null && UnitPath != null && (float)UnitPath.LengthInCells() > CurrentUnit.CombatState.MovementPoints)
		{
			UnitHelper.ClearPrediction(CurrentUnit);
		}
	}
	...
	private void DelayedUpdateAttackOfOpportunityPrediction()
	{
		...
		m_DelayedUpdateAoo = DelayedInvoker.InvokeInTime(m_UpdateAttackOfOpportunityPrediction, m_AooUpdateDelayTime);
	}
	...
	private void UpdateAttackOfOpportunityPrediction()
	{
		...
		foreach (BaseUnitEntity item in enumerable)
		{
			...
			{
				GameObject value = FxHelper.SpawnFxOnEntity(ConfigRoot.Instance.FxRoot.FXAttackOfOpportunity?.Load(), item.View);
