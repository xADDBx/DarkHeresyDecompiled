...
public class BuffApplyTrigger : UnitFactComponentDelegate, IUnitBuffHandler, ISubscriber<IBaseUnitEntity>, ISubscriber
{
	...
	public bool ForMultipleAbilityGroups;

	[ShowIf("ForMultipleAbilityGroups")]
	[SerializeField]
	private BlueprintAbilityGroupReference[] m_AbilityGroups = new BlueprintAbilityGroupReference[0];
	...
	public void HandleBuffDidAdded(Buff buff, MechanicEntity caster)
	{
		...
		if (OnlyPsychicPowerBuffs)
		{
			...
			if (maybeContext == null || maybeContext.SourceAbilityBlueprint == null || !buff.MaybeContext.SourceAbilityBlueprint.AbilityParamsSource.HasFlag(WarhammerAbilityParamsSource.PsychicPower))
			{
				return;
			}
			maybeContext = buff.MaybeContext;
			if (maybeContext != null && maybeContext.SourceAbilityBlueprint == null)
			{
				return;
			}
		}
	...
	private bool CheckBuff(Buff buff)
	{
		...
		if (ForMultipleAbilityGroups)
		{
			return buff.Blueprint.AbilityGroups.Any((BlueprintAbilityGroup p) => AbilityGroups.HasReference(p));
		}
		return true;
	}
