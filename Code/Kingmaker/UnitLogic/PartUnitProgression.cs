...
public class PartUnitProgression : BaseUnitPart, IHashable, IOwlPackable<PartUnitProgression>
{
	...
	private readonly List<DataWithSourceFact<TalentGroup>> _recommendedTalentGroupWithSources = new List<DataWithSourceFact<TalentGroup>>();

	public static readonly TypeInfo OwlPackTypeInfo = new TypeInfo
	{
		Name = "PartUnitProgression",
		OldNames = null,
		Fields = new FieldInfo[5]
		{
			new FieldInfo("m_Race", typeof(BlueprintRace)),
			new FieldInfo("CharacterLevel", typeof(int)),
			new FieldInfo("m_Selections", typeof(List<FeatureSelectionData>)),
			new FieldInfo("RespecInfo", typeof(RespecInfo)),
			new FieldInfo("Experience", typeof(int))
		}
	};
	...
	[JsonProperty]
	[OwlPackInclude]
	private List<FeatureSelectionData> m_Selections { get; set; } = new List<FeatureSelectionData>();
	...
	public TalentGroup RecommendedTalentGroup { get; private set; }

	private BlueprintStatProgression ExperienceTable => ConfigRoot.Instance.Progression.ExperienceTable;
	...
	public void AddRecommendedTalentGroup(TalentGroup group, EntityFactComponent component)
	{
		if (!_recommendedTalentGroupWithSources.HasItem((DataWithSourceFact<TalentGroup> i) => i.IsFrom(component)))
		{
			_recommendedTalentGroupWithSources.Add(new DataWithSourceFact<TalentGroup>(group, component));
			RecommendedTalentGroup |= group;
		}
	}

	public void RemoveRecommendedTalentGroup(EntityFactComponent component)
	{
		if (_recommendedTalentGroupWithSources.RemoveAll((DataWithSourceFact<TalentGroup> i) => i.IsFrom(component)) < 1)
		{
			return;
		}
		RecommendedTalentGroup = (TalentGroup)0;
		foreach (DataWithSourceFact<TalentGroup> recommendedTalentGroupWithSource in _recommendedTalentGroupWithSources)
		{
			RecommendedTalentGroup |= recommendedTalentGroupWithSource.Data;
		}
	}
	...
	public override Hash128 GetHash128()
	{
		...
		Hash128 val2 = SimpleBlueprintHasher.GetHash128(m_Race);
	...
	{
		PartUnitProgression source = new PartUnitProgression();
		result = Unsafe.As<PartUnitProgression, TPossiblyBase>(ref source);
	}
	...
	public override void Serialize<TFormatter>(TFormatter formatter, SerializerState state)
	{
		...
		ushort type = state.TypeLibrary.RegisterType<PartUnitProgression>(OwlPackTypeInfo);
		formatter.StartObject(type, OwlPackTypeInfo.Name, objectId);
		formatter.Field(0, "m_Race", ref m_Race, state);
		int value = CharacterLevel;
		formatter.UnmanagedField(1, "CharacterLevel", ref value, state);
		List<FeatureSelectionData> value2 = m_Selections;
		formatter.Field(2, "m_Selections", ref value2, state);
		RespecInfo value3 = RespecInfo;
		formatter.Field(3, "RespecInfo", ref value3, state);
		int value4 = Experience;
		formatter.UnmanagedField(4, "Experience", ref value4, state);
		formatter.EndObject();
	}
	...
	public override void Deserialize<TFormatter>(TFormatter formatter, uint objectId, DeserializerState state)
	{
		...
		TypeInfo typeInfo = state.TypeLibrary.GetTypeInfo<PartUnitProgression>();
		...
		for (int i = 0; i < typeInfo.Fields.Length; i++)
		{
			...
			case byte.MaxValue:
				...
				break;
			case 0:
				m_Race = formatter.ReadPackable<BlueprintRace>(state);
				break;
			case 1:
				CharacterLevel = formatter.ReadUnmanaged<int>(state);
				break;
			case 2:
				m_Selections = formatter.ReadPackable<List<FeatureSelectionData>>(state);
				break;
			case 3:
				RespecInfo = formatter.ReadPackable<RespecInfo>(state);
				break;
			case 4:
				Experience = formatter.ReadUnmanaged<int>(state);
				break;
			}
		}
