...
public sealed class UnitJumpToCellParams : UnitCommandParams<UnitJumpToCell>, IOwlPackable<UnitJumpToCellParams>
{
	public bool ProvokeAttackOfOpportunity = true;

	public AnimationClipWrapper JumpAnimation;
	...
	public UnitJumpToCellParams(Vector3 from, Vector3 to, AnimationClipWrapper jumpAnimation)
		: base((TargetWrapper)to)
	{
		...
		{
			TotalTransitionTime = jumpAnimation.Length;
			float startTraverseTime = jumpAnimation.GetStartTraverseTime();
			float endTraverseTime = jumpAnimation.GetEndTraverseTime();
