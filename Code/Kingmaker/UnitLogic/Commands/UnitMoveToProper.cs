...
public sealed class UnitMoveToProper : UnitCommand<UnitMoveToProperParams>
{
	...
	protected override void OnStart()
	{
		...
		MovePointsSpent = Mathf.Min(a, base.Executor.CombatState.MovementPoints);
		base.Executor.CombatState.SpendMovementPoints(MovePointsSpent);
	...
	{
		Vector3 position = base.Executor.Position;
		if (!base.Executor.IsProne && base.Result != ResultType.Fail)
		{
		...
		if (!WarhammerBlockManager.Instance.NodeContainsInvisibleAnyExcept(node, base.Executor.View.MovementAgent.Blocker))
		{
			...
			Vector3 forward = GraphHelper.AdjustDirection(base.Executor.MovementAgent.FinalDirection);
