...
public sealed class PartUnitCommands : EntityPart<AbstractUnitEntity>, IUnitFeaturesHandler, ISubscriber<IAbstractUnitEntity>, ISubscriber, IHashable, IOwlPackable<PartUnitCommands>
{
	...
	public UnitCommandHandle Run(UnitCommandParams cmdParams)
	{
		...
		if (!Game.Instance.Controllers.UnitCommandBuffer.TryAdd(base.Owner, cmdParams))
		{
	...
	private static void UpdateCombatTarget(AbstractUnitCommand cmd)
	{
		...
		if (mechanicEntity != null && mechanicEntity != cmd.Executor)
		{
			...
			if (lifeStateOptional != null && lifeStateOptional.IsConscious && baseUnitEntity.CombatGroup.CanAttack(mechanicEntity))
			{
	...
	{
		if (Game.Instance.Controllers.UnitCommandBuffer.TryAddToQueue(base.Owner, cmdParams))
		{
			return UnitCommandHandle.Request(cmdParams);
		}
	...
	{
		if (Game.Instance.Controllers.UnitCommandBuffer.TryAddToQueueFirst(base.Owner, cmd))
		{
			return UnitCommandHandle.Request(cmd);
		}
	...
	public void ClearLocalQueueAndBreakNetwork()
	{
		ClearQueue();
	}
	...
	{
		if (m_Current is T result)
		{
			return result;
		}
		foreach (UnitCommandParams item in m_Queue)
		{
			if (item is T result2)
			{
				return result2;
			}
		}
	...
	public void HandleFeatureAdded(FeatureCountableFlag feature)
	{
		...
		if ((bool)feature && feature.Type == MechanicsFeatureType.Prone && baseUnitEntity == base.ConcreteOwner)
		{
			InterruptMove();
		}
	}
	...
	{
		PartUnitCommands source = new PartUnitCommands();
		result = Unsafe.As<PartUnitCommands, TPossiblyBase>(ref source);
	}
	...
	public override void Serialize<TFormatter>(TFormatter formatter, SerializerState state)
	{
		...
		ushort type = state.TypeLibrary.RegisterType<PartUnitCommands>(OwlPackTypeInfo);
	...
	public override void Deserialize<TFormatter>(TFormatter formatter, uint objectId, DeserializerState state)
	{
		...
		TypeInfo typeInfo = state.TypeLibrary.GetTypeInfo<PartUnitCommands>();
