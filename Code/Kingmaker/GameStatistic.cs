...
public class GameStatistic : IDisposable, IUnitLifeStateChanged, ISubscriber<IAbstractUnitEntity>, ISubscriber, ITutorialTriggerFailedHandler, INewTutorialUIHandler, IUnitGainExperienceHandler, ISubscriber<IBaseUnitEntity>, ILevelUpInitiateUIHandler, ILevelUpCompleteUIHandler, IGameOverHandler, IAreaHandler, IFullScreenUIHandler
{
	...
	public class StatisticLimit
	{
		...
		public bool CanBeReported()
		{
			...
			if (Game.Instance.Controllers.TimeController.RealTime - LastReportTime > ReportInterval)
			{
				return true;
			}
			return false;
		...
		{
			LastReportTime = Game.Instance.Controllers.TimeController.RealTime;
		}
	}
	...
	public class PlayTimeStatistic : MergeableStatistic
	{
		...
		{
			GameModeType currentModeType = game.CurrentModeType;
			if (currentModeType != GameModeType.Default)
			{
				return currentModeType;
			}
			return game.CurrentModeType;
		}
	}
	...
	public class LevelupPlayerStatistic : LevelupStatistic
	{
		...
		public override string GetSerializedString()
		{
			return $"{LevelupGUID};{Level};{EndGameFakeLevelup};{Money};{Class}";
		}
	...
	public class LoadStatistic : MergeableStatistic
	{
		...
		public void Fill()
		{
			...
			bool flag3 = Game.Instance.CurrentModeType == GameModeType.GameOver;
	...
	public class MoneyFlowStatistic : MergeableStatistic
	{
		...
		public void Fill(ActionType operation, string source, string assetName, float value, int count)
		{
			...
			PlayerLevel = Game.Instance.Player.MainCharacterEntity.Progression.ExperienceLevel.ToString();
		}
	...
	private void AddLevelUp(Game gameInstance, bool endGameFake = false)
	{
		try
		{
			...
			int experienceLevel = baseUnitEntity.Progression.ExperienceLevel;
			string text = Guid.NewGuid().ToString();
			LevelupPlayerStatistic levelupPlayerStatistic = new LevelupPlayerStatistic();
			levelupPlayerStatistic.Fill(experienceLevel, baseUnitEntity, text ?? "", endGameFake);
			...
			levelupInventoryStatistic.Fill(baseUnitEntity.Body, "Player", text ?? "", experienceLevel, endGameFake);
			...
			{
				experienceLevel = activeCompanion.Progression.ExperienceLevel;
				levelupInventoryStatistic = new LevelupInventoryStatistic();
				levelupInventoryStatistic.Fill(activeCompanion.Body, activeCompanion.Blueprint.name, text ?? "", experienceLevel, endGameFake);
				AddStatisticToBuffer(levelupInventoryStatistic);
			}
		}
		catch (Exception ex)
	...
	private static MobStatistic GetBoss()
	{
		...
		{
			int cr = 0;
			UnitDifficultyType unitDifficultyType = UnitDifficultyType.Swarm;
			...
			int maxHP = 0;
			bool flag = false;
			foreach (UnitGroup item in Game.Instance.UnitGroups.Awake)
			{
				for (int i = 0; i < item.Count; i++)
				{
					BaseUnitEntity baseUnitEntity = item[i];
					if (baseUnitEntity != null && baseUnitEntity != null && (!s_CheckOnlyAliveMobs || !baseUnitEntity.LifeState.IsDead) && baseUnitEntity.CombatGroup.IsEnemy(Game.Instance.Player.MainCharacter.Entity.ToBaseUnitEntity()) && (!flag || unitDifficultyType < baseUnitEntity.Blueprint.DifficultyType))
					{
						unitDifficultyType = baseUnitEntity.Blueprint.DifficultyType;
			...
			if (flag)
			{
				mobStatistic.Fill(cr, name, characterName, lifeState, currentHP, maxHP);
			}
		}
		return mobStatistic;
	}
	...
	public static void Deserialize(SaveInfo saveInfo, string data, AppStatus appStatus)
	{
		...
		try
		{
			Game.Instance.State.Statistic = Serializer.DeserializeObject<GameStatistic>(data) ?? new GameStatistic();
		}
		catch (Exception arg)
		{
			PFLog.GameStatistics.Error($"Failed to read game statistic from save, creating new. Error: {arg}");
			Game.Instance.State.Statistic = new GameStatistic();
		}
	...
	public void HandleVendorDeal(MechanicEntity vendor, IEnumerable<ItemEntity> items)
	{
		try
		{
			...
			foreach (ItemEntity item in items)
			{
				...
				{
					float value = Game.Instance.TradeLogic.GetItemCost(item);
					dictionary[item.Blueprint.name] = new MoneyFlowStatistic();
					dictionary[item.Blueprint.name].Fill(MoneyFlowStatistic.ActionType.Buy, source, item.Blueprint.name, value, item.Count);
				}
			}
			foreach (MoneyFlowStatistic value2 in dictionary.Values)
			{
				AddStatisticToBuffer(value2);
			}
		}
		catch (Exception ex)
		{
