...
public class AchievementsManager : IReadOnlyCollection<AchievementEntity>, IEnumerable<AchievementEntity>, IEnumerable, IHashable, IOwlPackable, IOwlPackable<AchievementsManager>
{
	...
	public static readonly TypeInfo OwlPackTypeInfo = new TypeInfo
	{
		...
		{
			new FieldInfo("m_Achievements", typeof(List<AchievementEntity>))
		}
	};

	[JsonProperty]
	[OwlPackInclude]
	private List<AchievementEntity> m_Achievements { get; set; } = new List<AchievementEntity>();
	...
	public void Activate()
	{
		...
		foreach (AchievementData achData in ConfigRoot.Instance.Achievements.List)
		{
	...
	{
		AchievementsManager source = new AchievementsManager();
		result = Unsafe.As<AchievementsManager, TPossiblyBase>(ref source);
	}
	...
	public virtual void Serialize<TFormatter>(TFormatter formatter, SerializerState state) where TFormatter : IOutputFormatter
	{
		...
		ushort type = state.TypeLibrary.RegisterType<AchievementsManager>(OwlPackTypeInfo);
		formatter.StartObject(type, OwlPackTypeInfo.Name, objectId);
		List<AchievementEntity> value = m_Achievements;
		formatter.Field(0, "m_Achievements", ref value, state);
		formatter.EndObject();
	}
	...
	public virtual void Deserialize<TFormatter>(TFormatter formatter, uint objectId, DeserializerState state) where TFormatter : IInputFormatter
	{
		...
		TypeInfo typeInfo = state.TypeLibrary.GetTypeInfo<AchievementsManager>();
		...
		for (int i = 0; i < typeInfo.Fields.Length; i++)
		{
			...
			case byte.MaxValue:
				...
				break;
			case 0:
				m_Achievements = formatter.ReadPackable<List<AchievementEntity>>(state);
				break;
			}
		}
