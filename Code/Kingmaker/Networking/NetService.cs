...
public class NetService : IService, IDisposable
{
			...
			using (CollectionPool<List<GameCommand>, GameCommand>.Get(out value))
			{
				...
				using (CollectionPool<List<UnitCommandParams>, UnitCommandParams>.Get(out value2))
				{
					...
					using (CollectionPool<List<SynchronizedData>, SynchronizedData>.Get(out value3))
					{
						...
						Game.Instance.Controllers.UnitCommandBuffer.PushCommandsForPlayer(netPlayer, num, value2);
						Game.Instance.Controllers.SynchronizedDataController.PushDataForPlayer(netPlayer, num, value3);
					}
				}
			}
		}
	...
	public void CancelCurrentCommands()
	{
		...
		Game.Instance.Controllers.UnitCommandBuffer.CancelCurrentCommands();
		Game.Instance.Controllers.AbilityExecutor.DetachAll();
	}
	...
	void IDisposable.Dispose()
	{
		...
		Game.Instance.Controllers.UnitCommandBuffer.DumpScheduledCommands();
		Game.Instance.Controllers.UnitCommandBuffer.Clear();
		Game.Instance.Controllers.TimeSpeedController.Clear();
