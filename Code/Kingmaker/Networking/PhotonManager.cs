...
public class PhotonManager : MonoBehaviour, IMatchmakingCallbacks, IConnectionCallbacks, IInRoomCallbacks, IOnEventCallback
{
	...
	void IConnectionCallbacks.OnDisconnected(DisconnectCause cause)
	{
		...
		if (cause != DisconnectCause.DisconnectByClientLogic && cause != 0)
		{
			...
			UtilityNet.HandlePhotonDisconnectedError(cause, allowReconnect);
		}
	...
	void IInRoomCallbacks.OnPlayerLeftRoom(Photon.Realtime.Player otherPlayer)
	{
		...
		if (Lobby.IsPlaying)
		{
			Game.Instance.Controllers.PauseController.OnPlayerLeftRoom();
	...
	{
		StopPlaying("Kick");
		UtilityMessageBox.ShowMessageBox(UIStrings.Instance.NetLobbyTexts.KickMessage, DialogMessageBoxType.Message, delegate
		{
		});
	}
