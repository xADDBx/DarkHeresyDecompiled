...
public class GraphicsSettingsController
{
	...
	private readonly IReadOnlyList<Resolution> m_CommonResolutions = new List<Resolution>
	{
		new Resolution
		{
			width = 2560,
	...
	private readonly List<Resolution> m_RelevantResolutions = new List<Resolution>();
	...
	public IReadOnlyList<Resolution> RelevantResolutions => m_RelevantResolutions;
	...
	private void OnVolumeManagerUpdated(Camera camera)
	{
		...
		BloomEnhanced component = stack.GetComponent<BloomEnhanced>();
		if (component.IsActive() && !m_Settings.Bloom.GetValue())
		{
			component.intensity.value = 0f;
		}
		DepthOfField component2 = stack.GetComponent<DepthOfField>();
		if (component2.IsActive() && !m_Settings.DepthOfField.GetValue())
		{
			component2.mode.value = DepthOfFieldMode.Off;
		}
		ScreenSpaceReflections component3 = stack.GetComponent<ScreenSpaceReflections>();
		if (component3.IsActive())
		{
			switch (m_Settings.SSRQuality.GetValue())
			{
			case QualityOptionDisactivatable.Off:
				component3.Quality.value = ScreenSpaceReflectionsQuality.None;
				break;
			case QualityOptionDisactivatable.Medium:
				component3.Quality.value = ScreenSpaceReflectionsQuality.Half;
				break;
			case QualityOptionDisactivatable.High:
				component3.Quality.value = ScreenSpaceReflectionsQuality.Full;
				break;
			}
		}
		FilmGrain component4 = stack.GetComponent<FilmGrain>();
		if (component4.IsActive() && !m_Settings.FilmGrainEnabled.GetValue())
		{
			component4.intensity.value = 0f;
		}
	}
	...
	private void ApplyGraphicsSettings()
	{
		...
		if (TryGetRenderPipelineAsset(out var result))
		{
			...
			result.UpscalingFilter = ((m_Settings.FsrMode.GetValue() != 0) ? UpscalingFilterSelection.FSR : UpscalingFilterSelection.Auto);
			result.FsrOverrideSharpness = result.UpscalingFilter == UpscalingFilterSelection.FSR;
	...
	{
		PFLog.Settings.Log("Relevant Resolutions: " + string.Join(",", m_RelevantResolutions.Select((Resolution r) => $"{r.width}x{r.height}")));
		...
		if (!m_Settings.ScreenResolutionWasTouched.GetValue() || (currentWidth < 640 && currentHeight < 480) || !m_RelevantResolutions.Any((Resolution r) => r.width == currentWidth && r.height == currentHeight))
		{
	...
	{
		LocalizedString text = ConfigRoot.Instance.LocalizedTexts.UserInterfacesText.SettingsUI.RestartConfirmation;
		EventBus.RaiseEvent(delegate(IDialogMessageBoxUIHandler w)
		{
			w.HandleOpen(text, DialogMessageBoxType.Dialog, delegate(DialogMessageBoxButton b)
			{
	...
	private void GenerateResolutionsList()
	{
		...
		m_RelevantResolutions.Add(new Resolution
		{
			width = systemWidth,
			height = systemHeight
		});
		...
		for (int j = 0; j < resolutions.Length; j++)
		{
			...
			if ((Mathf.Approximately(a2, b2) || m_CommonResolutions.Contains((Resolution r) => r.width == resolution.width && r.height == resolution.height)) && !m_RelevantResolutions.Contains((Resolution r) => r.width == resolution.width && r.height == resolution.height))
			{
				m_RelevantResolutions.Add(resolution);
			}
		}
		m_RelevantResolutions.Sort((Resolution a, Resolution b) => (b.width * b.height).CompareTo(a.width * a.height));
	}
	...
	private int GetIndexOfResolution(int width, int height)
	{
		...
		{
			Resolution resolution = m_RelevantResolutions[i];
