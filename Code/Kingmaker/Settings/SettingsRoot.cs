...
public static class SettingsRoot
{
	...
	private static DevelopmentSettings s_Development;
	...
	public static DevelopmentSettings Development
	{
		get
		{
			EnsureInitialized();
	...
	public static void Initialize(SettingsValues settingsValues)
	{
		...
		using (Initializing.Retain())
		{
			...
			using (new SettingsKeyPrefix("settings"))
			{
				...
				using (new SettingsKeyPrefix("development"))
				{
					s_Development = new DevelopmentSettings(instance, settingsValues.SettingsDefaultValues.Development);
				}
			}
