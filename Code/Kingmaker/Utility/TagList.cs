...
public class TagList<T> : TagListBase, IEnumerable<T>, IEnumerable
{
	...
	private static int[] ValuesIntArray { get; } = System.Enum.GetValues(typeof(T)).Cast<int>().ToArray();


	private static T[] ValuesArray { get; } = System.Enum.GetValues(typeof(T)).Cast<T>().ToArray();
