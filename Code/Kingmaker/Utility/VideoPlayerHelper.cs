...
public class VideoPlayerHelper : MonoBehaviour
{
	...
	private void PrepareAudio()
	{
		...
		if (m_SoundStartEventName != null)
		{
			...
			AkUnitySoundEngine.PrepareEvent(AkPreparationType.Preparation_LoadAndDecode, new string[1] { m_SoundStartEventName }, 1u, delegate
			{
	...
	{
		if (AkUnitySoundEngine.GetSourcePlayPosition(m_SoundId, out var out_puPosition) != AKRESULT.AK_Success)
		{
			audioTime = 0.0;
			return false;
		}
	...
	private void SeekOrSpeedUpAudio(double playbackDelta, double videoTime)
	{
		...
		if (playbackDelta > 0.20000000298023224)
		{
			...
			AkUnitySoundEngine.SeekOnEvent(m_SoundStartEventName, base.gameObject, in_iPosition, in_bSeekToNearestMarker: false, m_SoundId);
		}
	}
