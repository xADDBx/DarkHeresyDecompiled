...
public static class GridPatterns
{
	...
	{
		private readonly List<GridNodeBase>[] m_Rays;
		...
		private readonly GridNodeBase m_FromNode;

		private readonly int m_Range;

		public CanTransitionBetweenCellsImpl(List<GridNodeBase>[] rays, int left1Idx, int left2Idx, int right1Idx, int right2Idx, GridNodeBase fromNode, int range)
		{
			m_Rays = rays;
			m_Left1Idx = left1Idx;
			m_Left2Idx = left2Idx;
		...
		public bool CanTransitionBetweenCells(GridNodeBase nodeFrom, GridNodeBase nodeTo, Vector3 transitionPosition, float distanceFactor)
		{
			...
			{
				gridNodeBase = nodeTo;
				gridNodeBase2 = nodeTo;
			}
			else
			{
				gridNodeBase = nodeTo.GetNeighbourAlongDirection(m_Left1Idx, checkConnectivity: false) ?? nodeTo.GetNeighbourAlongDirection(m_Left2Idx, checkConnectivity: false);
				gridNodeBase2 = nodeTo.GetNeighbourAlongDirection(m_Right1Idx, checkConnectivity: false) ?? nodeTo.GetNeighbourAlongDirection(m_Right2Idx, checkConnectivity: false);
			}
			GridNodeBase gridNodeBase3 = gridNodeBase?.GetNeighbourAlongDirection(m_Left1Idx, checkConnectivity: false) ?? gridNodeBase?.GetNeighbourAlongDirection(m_Left2Idx, checkConnectivity: false);
			GridNodeBase gridNodeBase4 = gridNodeBase2?.GetNeighbourAlongDirection(m_Right1Idx, checkConnectivity: false) ?? gridNodeBase2?.GetNeighbourAlongDirection(m_Right2Idx, checkConnectivity: false);
			if (gridNodeBase3 != null && m_FromNode.CellDistanceTo(gridNodeBase3) <= m_Range)
			{
				m_Rays[0].Add(gridNodeBase3);
			}
			if (gridNodeBase != null && m_FromNode.CellDistanceTo(gridNodeBase) <= m_Range)
			{
				m_Rays[1].Add(gridNodeBase);
			}
			if (m_FromNode.CellDistanceTo(nodeTo) <= m_Range)
			{
				m_Rays[2].Add(nodeTo);
			}
			if (gridNodeBase2 != null && m_FromNode.CellDistanceTo(gridNodeBase2) <= m_Range)
			{
				m_Rays[3].Add(gridNodeBase2);
			}
			if (gridNodeBase4 != null && m_FromNode.CellDistanceTo(gridNodeBase4) <= m_Range)
			{
				m_Rays[4].Add(gridNodeBase4);
			}
			return true;
		}
	}
	...
	public static List<GridNodeBase> CalcScatterShotMainLine(GridNodeBase fromNode, GridNodeBase toNode, int range)
	{
		return CalcScatterShot(fromNode, toNode, range)[2];
	}

	public static List<GridNodeBase>[] CalcScatterShot(GridNodeBase fromNode, GridNodeBase toNode, int range)
	{
		List<GridNodeBase>[] array = new List<GridNodeBase>[5]
		{
			new List<GridNodeBase>(),
			new List<GridNodeBase>(),
			new List<GridNodeBase>(),
			new List<GridNodeBase>(),
			new List<GridNodeBase>()
		};
		Vector3 vector = fromNode.Vector3Position();
		Vector3 vector2 = toNode.Vector3Position();
		vector2 = vector + (vector2 - vector).normalized * range.Cells().Meters;
		(int left1, int left2, int right1, int right2) neighboursIdx = GetNeighboursIdx((Int3)vector, (Int3)vector2);
		...
		Linecast.LinecastGrid2(fromNode.Graph, vector, vector2, fromNode, out var _, NNConstraint.None, ref condition);
		return array;
	}
	...
	public static PatternGridData ConstructPattern(IEnumerable<GridNodeBase> nodes)
	{
		s_BitPattern.Clear();
		BitPattern2D bitPattern2D = s_BitPattern;
		foreach (GridNodeBase node in nodes)
		{
			bitPattern2D.Add(node.CoordinatesInGrid);
		}
		return PatternGridData.Create(bitPattern2D, disposable: true);
	}
	...
	private static void AccumulateNodesInRadius(int radius, BitPattern2D nodes)
	{
		for (int i = -radius; i < radius + 1; i++)
		{
			for (int j = -radius; j < radius + 1; j++)
			{
				...
				if (!nodes.Has(vector2Int) && GraphHelper.GetWarhammerLength(vector2Int) <= radius)
				{
	...
	private static void AccumulateNodesInRadius(int radius, HashSet<Vector2Int> nodes)
	{
		for (int i = -radius; i < radius + 1; i++)
		{
			...
			{
				Vector2Int vector2Int = new Vector2Int(i, j);
				if (!nodes.Contains(vector2Int) && GraphHelper.GetWarhammerLength(vector2Int) <= radius)
				{
					nodes.Add(vector2Int);
				}
			}
	...
	public static void AddSectorNodes(BitPattern2D nodes, int radius, int degrees, Vector2 dir)
	{
		...
		for (int i = -radius; i < radius + 1; i++)
		{
			...
			{
				Vector2Int vector2Int = new Vector2Int(i, j);
				if (!nodes.Has(vector2Int) && Vector2.Dot(dir, new Vector2(vector2Int.x, vector2Int.y).normalized) >= num && GraphHelper.GetWarhammerLength(vector2Int) <= radius)
				{
					nodes.Add(vector2Int);
				}
			}
