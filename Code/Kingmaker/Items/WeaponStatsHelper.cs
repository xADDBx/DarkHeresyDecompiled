...
public static class WeaponStatsHelper
{
	private readonly struct Key : IEquatable<Key>
	{
		...
		public Key([CanBeNull] AbilityData ability, [CanBeNull] ItemEntityWeapon weapon, [CanBeNull] MechanicEntity wielder, [CanBeNull] MechanicEntity target)
		{
			m_Wielder = wielder;
			m_Weapon = weapon;
			m_Ability = ability;
		...
		{
			if (object.Equals(m_Wielder, other.m_Wielder) && object.Equals(m_Weapon, other.m_Weapon) && object.Equals(m_Ability, other.m_Ability))
			{
				return object.Equals(m_Target, other.m_Target);
			}
			return false;
		}
		...
		{
			return HashCode.Combine(m_Wielder, m_Weapon, m_Ability, m_Target);
		}
	}
	...
	public static RuleCalculateStatsWeapon GetWeaponStats([CanBeNull] AbilityData ability, [CanBeNull] ItemEntityWeapon weapon, [CanBeNull] MechanicEntity wielder, [CanBeNull] MechanicEntity target)
	{
		...
		Key key = new Key(ability, weapon, wielder, target);
