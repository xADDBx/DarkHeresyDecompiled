...
public class GameLogController : IControllerStart, IController, IControllerEnable, IControllerDisable, IControllerTick, IRulebookEventAboutToTriggerHook, IRulebookEventHook, IRulebookEventDidTriggerHook
{
	...
	void IControllerStart.OnStart()
	{
		...
		PatternCollection.Instance.AddPattern(PatternAddEventMergeEvent<GameLogRuleEvent<RulePerformSavingThrow>>.Create(PerformSavingThrowComparer.Create())).AddPattern(PatternAddEventMergeEvent<GameLogEventItemsCollection>.Create(ItemsCollectionComparer.Create())).AddPattern(PatternAddEventMergeEvent<GameLogRuleEvent<RuleCalculateCanApplyBuff>>.Create(CalculateCanApplyBuffComparer.Create()))
			.AddPattern(PatternAddEventMergeEvent<GameLogRuleEvent<RulePerformMoraleChange>>.Create(PerformMoraleChangeUnitDeathComparer.Create()))
			.AddPattern(PatternAddEventMergeEvent<GameLogRuleEvent<RuleDealDamage>>.Create(new DisperseDamageComparer()))
		...
		PatternCollection.Instance.AddPattern(PatternPostAddEventAttackEventChildrenSort.Create()).AddPattern(PatternPostAddEventGrenadeAttackEventChildrenSort.Create()).AddPattern(PatternPostAddEventSwitchApplyBuffAndDependAbility.Create())
	...
	void IControllerTick.Tick()
	{
		using (GameLogContext.Scope)
		{
			for (int j = 0; j < m_EventsQueue.Count; j++)
			{
				...
				if (gameLogEvent != null && !gameLogEvent.IsReady)
				{
					break;
				}
				m_EventsQueue[j] = null;
				if (gameLogEvent == null || (gameLogEvent != null && !gameLogEvent.IsEnabled))
				{
					continue;
				}
		...
		foreach (LogThreadBase allThread in Services.GetInstance<LogThreadService>().AllThreads)
		{
			...
			foreach (Type type in interfaces)
			{
				...
				else if (type.GetGenericTypeDefinition() == typeof(IGameLogEventHandler<>))
				{
					...
					{
						type4 = type3;
						while (type4.IsGenericType)
						{
							if (type4.GetGenericTypeDefinition() == typeof(GameLogRuleEvent<>) || type4.GetGenericTypeDefinition() == typeof(GameLogRuleEvent<, >))
							{
								type2 = type4.GetGenericArguments()[0];
								break;
							}
