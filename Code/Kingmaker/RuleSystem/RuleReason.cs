...
public readonly struct RuleReason : IUIDataProvider, IEquatable<RuleReason>
{
	...
	public AbilityData Ability
	{
		get
		{
			object obj = (Rule as RulePerformAbility)?.Ability;
			if (obj == null)
			{
				obj = (Rule as RulePerformAttack)?.Ability;
				if (obj == null)
				{
					obj = (Rule as RulePerformAttackRoll)?.Ability;
					if (obj == null)
					{
	...
	public RuleReason([NotNull] RulebookEvent rule)
	{
		...
		SourceEntity = (Caster = rule.Initiator);
		Rule = rule;
		if (rule is RulePerformAbility rulePerformAbility)
		{
			Item = rulePerformAbility.Ability.SourceItem;
