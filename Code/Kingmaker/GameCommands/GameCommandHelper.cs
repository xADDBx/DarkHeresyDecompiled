...
public static class GameCommandHelper
{
	...
	public static void AddItemToTrash(ItemEntity item)
	{
		Game.Instance.VendorsManager.AddToTrash(item);
		EventBus.RaiseEvent((IItemEntity)item, (Action<ITrashItemHandler>)delegate(ITrashItemHandler h)
		{
			h.HandleItemAddedToTrash(item);
		}, isCheckRuntime: true);
	}

	public static void RemoveItemFromTrash(ItemEntity item)
	{
		Game.Instance.VendorsManager.RemoveFromTrash(item);
		EventBus.RaiseEvent((IItemEntity)item, (Action<ITrashItemHandler>)delegate(ITrashItemHandler h)
		{
			h.HandleItemRemovedFromTrash(item);
		}, isCheckRuntime: true);
	}
	...
	public static void TrySwapSlots(ItemSlotRef fromData, ItemSlotRef toData, MechanicEntity entity, bool isLoot)
	{
		if (fromData.ItemsCollection != toData.ItemsCollection && isLoot)
		{
			...
			_ = toData.SlotIndex;
			_ = toData.ItemsCollection;
			TransferCount(toData.ItemsCollection ?? Game.Instance.PartySharedInventory.Collection, fromData.Item, item.Count);
		}
		else
		{
	...
	{
		ItemEntity item = fromData.Item;
		ItemEntity item2 = ((item == null || !item.IsFake) ? fromData.Item : null);
		item = toData.Item;
		ItemEntity itemEntity = ((item == null || !item.IsFake) ? toData.Item : null);
		if (isLoot || (!fromData.IsEquipment && !toData.IsEquipment))
		{
			int slotIndex = fromData.SlotIndex;
			int slotIndex2 = toData.SlotIndex;
			SetItem(item2, slotIndex2);
			SetItem(itemEntity, slotIndex);
			EventBus.RaiseEvent(delegate(IMoveItemHandler h)
			{
		...
		{
			if ((itemSlot2 != null && !itemSlot2.CanInsertItem(item2)) || (itemSlot != null && !itemSlot.TryRemove(autoMerge)))
			{
				EventBus.RaiseEvent(delegate(IMoveItemHandler h)
				{
					h.HandleMoveItem(fromData.IsEquipment || toData.IsEquipment);
		...
		{
			SetItem(itemEntity, fromData.SlotIndex);
		}
		if (toData.IsEquipment)
		{
			if (!TryInsertItem(itemSlot2, item2))
			{
				EventBus.RaiseEvent(delegate(IMoveItemHandler h)
				{
					h.HandleMoveItem(fromData.IsEquipment || toData.IsEquipment);
			...
			if (itemSlot != null && !itemSlot.HasItem && itemEntity != null)
			{
				TryInsertItem(itemSlot, itemEntity);
			}
		}
		else
		{
			SetItem(item2, toData.SlotIndex);
		}
		EventBus.RaiseEvent(delegate(IMoveItemHandler h)
		{
			h.HandleMoveItem(fromData.IsEquipment || toData.IsEquipment);
	...
	{
		Game.Instance.TradeLogic.RemoveFromBuy(item, count);
	}

	public static void AddForBuy(ItemEntity item, int count)
	{
		Game.Instance.TradeLogic.AddForBuy(item, count);
	}

	public static void RemoveFromSell(ItemEntity item, int count)
	{
		Game.Instance.TradeLogic.RemoveFromSell(item, count);
	}

	public static void AddForSell(ItemEntity item, int count)
	{
		Game.Instance.TradeLogic.AddForSell(item, count);
	}
	...
	public static void TryCollect(List<EntityRef<ItemEntity>> items)
	{
		...
		foreach (EntityRef<ItemEntity> item in items)
		{
			if (item.Entity != null)
			{
				...
				if (from != Game.Instance.PartySharedInventory.Collection)
				{
					from.Transfer(item, Game.Instance.PartySharedInventory.Collection);
					flag = true;
				}
			}
		}
		if (flag)
		{
			...
			{
				h.HandleCollectAll(from, Game.Instance.PartySharedInventory.Collection);
			});
		}
	}
