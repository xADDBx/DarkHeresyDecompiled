...
public class NetworkingOverlay : Overlay
{
	...
	private static OverlayElement[] CreateElements()
	{
		...
		return new OverlayElement[25]
		{
			...
			{
				int minLag = Game.Instance.Controllers.TimeSpeedController.GetMinLag(byte.MaxValue);
				int maxLag = Game.Instance.Controllers.TimeSpeedController.MaxLag;
				int minLag2 = Game.Instance.Controllers.SynchronizedDataController.SynchronizedData.GetMinLag(255);
				int maxLag2 = Game.Instance.Controllers.SynchronizedDataController.SynchronizedData.GetMaxLag();
				return $"{minLag}:{maxLag} | {minLag2}:{maxLag2}";
			}),
			new Label("Speed Mode", delegate
			{
				int currentSpeedMode = Game.Instance.Controllers.TimeSpeedController.CurrentSpeedMode;
			...
			new Label("Prediction", () => Game.Instance.IsControllerGamepad ? ((!Game.Instance.Controllers.MovePredictionController.IsActive) ? "off" : "ON") : "off (PC)")
		};
	}
