...
public class TaskUseAreaTransition : ClockworkRunnerTask
{
	...
	protected override IEnumerator Routine()
	{
		...
		UnitCommandsRunner.MoveSelectedUnitsToPointRT(transform.position, ClickGroundHandler.GetDefaultDirection(transform.position), Game.Instance.IsControllerGamepad, preview: false, ConfigRoot.Instance.Formations.MinSpaceFactor, null, delegate(BaseUnitEntity unit, MoveCommandSettings settings)
		{
			...
			else
			{
				...
				List<EntityRef<BaseUnitEntity>> units = ((IEnumerable<BaseUnitEntity>)Game.Instance.Controllers.SelectionCharacter.SelectedUnits).Select((Func<BaseUnitEntity, EntityRef<BaseUnitEntity>>)((BaseUnitEntity elem) => elem)).ToList();
