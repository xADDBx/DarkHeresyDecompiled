...
public class AreaTaskSelector
{
	...
	{
		(m_NNConstraint as NNConstraintWithTraversalProvider)?.Reset();
	...
	public ClockworkRunnerTask SelectNewTask(ClockworkRunner runner, bool checkNavMesh = true)
	{
		...
		IEnumerable<BaseUnitEntity> source = Game.Instance.EntityPools.AllBaseUnits.Where((BaseUnitEntity u) => !m_Runner.AIConfig.IsExcludedUnit(u.Blueprint));
		List<TargetAndPriority> source2 = (from i in Enumerable.Concat(second: from o in Game.Instance.EntityPools.MapObjects
	...
	private int GetInteractionPriority(BaseUnitEntity unit)
	{
		...
		if (!(Game.Instance.Controllers.TimeController.GameTime - dialogData.LastTalkTime < 10.Days()))
		{
			if (!dialogData.Seen)
			{
				return 18;
	...
	private int GetInteractionPriority(MapObjectEntity mo)
	{
		...
		int num = -1;
		foreach (AbstractInteractionPart interaction in mo.Interactions)
		{
			num++;
	...
	private bool IsTransition(Entity obj)
	{
		...
		foreach (AbstractInteractionPart interaction in mapObjectEntity.Interactions)
		{
