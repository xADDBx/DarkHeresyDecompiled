...
public class TaskSolveDialog : ClockworkRunnerTask
{
	...
	{
		DialogController controller = Game.Instance.Controllers.DialogController;
		...
		dialogData.LastTalkTime = Game.Instance.Controllers.TimeController.GameTime;
		while (Game.Instance.CurrentModeType == GameModeType.Dialog)
		{
			yield return 1f;
			...
			BlueprintAnswer sb = controller.Answers.Where((BlueprintAnswer a) => !Game.Instance.DialogState.SelectedAnswersContains(a) && Clockwork.Instance.Scenario.CanUseAnswer(a)).Random(PFStatefulRandom.Qa);
	...
	private static bool IsUntriedAnswer(BlueprintScriptableObject answer)
	{
		...
		{
			return !Game.Instance.DialogState.SelectedAnswersContains(blueprintAnswer);
		}
		return false;
	}
	...
	{
		return $"Talk [{Game.Instance.Controllers.DialogController.Dialog}]";
	}
}
