...
public class ClockworkRunner : ITeleportHandler, ISubscriber, IAreaHandler, IGameOverHandler, IGameModeHandler, IPartyCombatHandler, IUnitCombatHandler, ISubscriber<IBaseUnitEntity>
{
	...
	public BlueprintClockworkScenario TestScenario => m_TestScenario;
	...
	{
		Game.Instance.RootUIContext.CloseUI(m_CurrentTask is TaskInteractWithMapObject);
	}
	...
	private bool IsWaitingForDelay()
	{
		if (m_Delay > 0f)
		{
			m_Delay -= Game.Instance.Controllers.TimeController.DeltaTime;
	...
	{
		if (Game.Instance.CurrentModeType == GameModeType.Pause)
		{
			PFLog.Clockwork.Log("Unpausing game");
			Game.Instance.IsPaused = false;
		}
		if (Game.Instance.CurrentModeType == GameModeType.Default && GameHelper.GetPlayerCharacter().IsInCombat && !(m_CurrentTask is TaskWinCombat) && !(m_CurrentTask is TaskHeal))
		{
			PFLog.Clockwork.Log("Interrupt task to enter combat");
			InsertTaskBeforeCurrent(new TaskWinCombat(this));
			m_CurrentTask.CreateTicker();
		}
		if (Game.Instance.CurrentModeType == GameModeType.Dialog && !(LastCommand is AnswerCommand))
		{
		...
		Game.Instance.Controllers.TimeController.DebugTimeScale = ((Game.Instance.CurrentModeType == GameModeType.Cutscene) ? 3f : TimeScale);
	}
	...
	public void OnAreaBeginUnloading()
	{
		...
		areaData.LastVisitTime = Game.Instance.Controllers.TimeController.GameTime;
	}
	...
	public void HandleGameOver(Player.GameOverReasonType reason)
	{
		...
		{
			text = text + "\nEnemies: \n" + string.Join("\n", m_CurrentEnemies.Select((BaseUnitEntity x) => x.Blueprint));
		}
