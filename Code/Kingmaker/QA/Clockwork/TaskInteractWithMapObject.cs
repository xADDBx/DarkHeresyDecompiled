...
public class TaskInteractWithMapObject : ClockworkRunnerTask
{
	...
	protected override IEnumerator Routine()
	{
		if (m_MapObject == null)
		{
			m_MapObject = Game.Instance.EntityPools.MapObjects.Where((MapObjectEntity o) => o.Interactions.Any()).Random(PFStatefulRandom.Qa);
		}
		if (m_MapObject == null)
		{
			yield break;
		...
		yield return 0.5f;
		Game.Instance.RootUIContext?.MaybeCollectLoot();
		yield return null;
		AbstractInteractionPart abstractInteractionPart = command?.Interaction;
		if (abstractInteractionPart is InteractionDoorPart door)
		{
			yield return 2f;
			if (door.IsOpen)
			{
