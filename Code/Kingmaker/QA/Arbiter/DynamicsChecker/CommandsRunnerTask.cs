...
public class CommandsRunnerTask : ArbiterTask
{
	...
	private void EnableImmortality()
	{
		...
		{
			if (!selectedUnit.Buffs.Contains(ConfigRoot.Instance.Cheats.Iddqd))
			{
				GameHelper.ApplyBuff(selectedUnit, ConfigRoot.Instance.Cheats.Iddqd);
			}
		}
	}
	...
	private void HandleGameModeChange()
	{
		...
		if (Game.Instance.CurrentModeType == GameModeType.Default && GameHelper.GetPlayerCharacter().IsInCombat && !(m_CurrentTask is TaskAutoCombat))
		{
	...
	{
		bool flag = Game.Instance.Controllers.CameraController?.Follower.HasTarget ?? false;
		if (Game.Instance.CurrentModeType == GameModeType.Default && !CutsceneLock.Active && !flag)
		{
			if (GameHelper.GetPlayerCharacter().IsInCombat)
			{
				Game.Instance.Controllers.CameraController.Follower.Follow(Game.Instance.Controllers.TurnController.CurrentUnit);
			}
			else
			{
				Game.Instance.Controllers.CameraController.Follower.Follow(Game.Instance.Player.MainCharacterEntity);
			}
		}
	}
