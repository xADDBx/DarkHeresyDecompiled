...
public static class StackTraceSpamDetector
{
	...
	private static bool IsDetectionEnabled
	{
		get
		{
			if (UnitySyncContextHolder.IsInUnity && (!Application.isEditor || BuildModeUtility.ForceSpamDetectionInEditor) && !BuildModeUtility.StackTraceSpamDetectionDisabled && BuildModeUtility.IsDevelopment)
			{
	...
	private static void LogCallback(LogInfo logInfo)
	{
		...
		QAModeExceptionEvents.Instance.MaybeShowError(string.Format("{0}\nGameModeType:{1}", "Put a bug on this, because there will be lags on consolas!", Game.Instance.CurrentModeType), new SpamDetectingException(stringBuilder.ToString(), Game.Instance.CurrentModeType));
