...
public class EntityFactsManager : IDisposable, IHashable, IOwlPackable, IOwlPackable<EntityFactsManager>
{
	...
	{
		Name = "EntityFactsManager",
		OldNames = null,
		Fields = new FieldInfo[1]
		{
			new FieldInfo("m_Facts", typeof(List<EntityFact>))
		}
	};
	...
	[OwlPackInclude]
	private List<EntityFact> m_Facts { get; set; } = new List<EntityFact>();
	...
	public bool Contains([CanBeNull] EntityFact fact)
	{
		if (fact != null)
		{
	...
	public bool Contains([CanBeNull] BlueprintFact blueprint)
	{
		if (blueprint == null)
		{
			return false;
	...
	public void RemoveAll<TFact>(Predicate<TFact> pred) where TFact : EntityFact
	{
	...
	{
		EntityFactsManager source = new EntityFactsManager();
		result = Unsafe.As<EntityFactsManager, TPossiblyBase>(ref source);
	}
	...
	public virtual void Serialize<TFormatter>(TFormatter formatter, SerializerState state) where TFormatter : IOutputFormatter
	{
		...
		ushort type = state.TypeLibrary.RegisterType<EntityFactsManager>(OwlPackTypeInfo);
		formatter.StartObject(type, OwlPackTypeInfo.Name, objectId);
		List<EntityFact> value = m_Facts;
		formatter.Field(0, "m_Facts", ref value, state);
		formatter.EndObject();
	}
	...
	public virtual void Deserialize<TFormatter>(TFormatter formatter, uint objectId, DeserializerState state) where TFormatter : IInputFormatter
	{
		...
		TypeInfo typeInfo = state.TypeLibrary.GetTypeInfo<EntityFactsManager>();
		...
		for (int i = 0; i < typeInfo.Fields.Length; i++)
		{
			...
			case byte.MaxValue:
				...
				break;
			case 0:
				m_Facts = formatter.ReadPackable<List<EntityFact>>(state);
				break;
			}
		}
