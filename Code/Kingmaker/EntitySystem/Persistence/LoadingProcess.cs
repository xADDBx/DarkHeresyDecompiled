...
public class LoadingProcess : MonoBehaviour
{
	...
	private string m_CurrentProfilingName;
	...
	public bool IsAnyLoadingScreenActive
	{
		get
		{
			...
			{
				return IsManualLoadingScreenActive;
			}
			return true;
		}
	}
	...
	public bool IsFadeActive
	{
		get
		{
			if (m_LoadingScreen != null)
			{
				return m_LoadingScreen.Equals((ILoadingScreen)Game.Instance.RootUIContext.GetLoadingScreenVM());
			}
			return false;
		}
	}
	...
	public void TickLoading()
	{
		...
		using (SuperluminalPerf.BeginEvent("LoadingProcess.TickLoading", m_CurrentProfilingName, new SuperluminalPerf.ProfilerColor(byte.MaxValue, 0, 0)))
		{
			if (m_CurrentLoadingProcess.MoveNext())
			{
				return;
			}
			...
			if (m_Queue.Count == 0)
			{
				...
				{
					EnqueueLoadingProcess("AwaitingCommands", Game.AwaitingCommands(), null, currentProcessTag);
				}
			...
			if (m_Queue.Count > 0)
			{
				...
				StartLoadingProcessInternal(queuedProcess.ProfilingName, queuedProcess.Process, queuedProcess.Callback, queuedProcess.Tag);
			}
			else
			{
	...
	public void StartLoadingProcess(string profilingName, IEnumerator process, Action callback = null, LoadingProcessTag processTag = LoadingProcessTag.None)
	{
		if (m_CurrentLoadingProcess != null || m_Queue.Count > 0)
		{
			EnqueueLoadingProcess(profilingName, process, callback, processTag);
		}
		else
		{
			StartLoadingProcessInternal(profilingName, process, callback, processTag);
		}
	}

	public void StartLoadingProcess(string profilingName, Action callback, LoadingProcessTag processTag = LoadingProcessTag.None)
	{
		if (m_CurrentLoadingProcess != null || m_Queue.Count > 0)
		{
			IEnumerator<object> enumerator = Enumerable.Empty<object>().GetEnumerator();
			EnqueueLoadingProcess(profilingName, enumerator, callback, processTag);
		}
		else
		{
			callback?.Invoke();
	...
	private void EnqueueLoadingProcess(string profilingName, IEnumerator process, Action callback = null, LoadingProcessTag processTag = LoadingProcessTag.None)
	{
	...
	private void StartLoadingProcessInternal(string profilingName, IEnumerator process, Action callback = null, LoadingProcessTag processTag = LoadingProcessTag.None)
	{
		StartLoadingScreen(SelectLoadingScreen(processTag));
		m_OnLoadCallback = callback;
		m_CurrentLoadingProcess = process;
		m_CurrentProfilingName = profilingName;
	...
	private void StartLoadingScreen(ILoadingScreen loadingScreen)
	{
		...
		if (loadingScreen != null && (m_LoadingScreen == null || !m_LoadingScreen.Equals(loadingScreen)))
		{
			...
			else if (IsFadeActive && loadingScreen.Equals((ILoadingScreen)Game.Instance.RootUIContext.GetLoadingScreenVM()))
			{
				m_LoadingScreen.HideLoadingScreen();
				m_LoadingScreen = loadingScreen;
			}
	...
	private static ILoadingScreen SelectLoadingScreen(LoadingProcessTag tag)
	{
		return tag switch
		{
			LoadingProcessTag.None => (ILoadingScreen)Game.Instance.RootUIContext.GetLoadingScreenVM(), 
			LoadingProcessTag.Save => EmptyLoadingScreen.Instance, 
			LoadingProcessTag.ReloadMechanics => (ILoadingScreen)Game.Instance.RootUIContext.GetFadeVM(), 
			LoadingProcessTag.ReloadLight => (ILoadingScreen)Game.Instance.RootUIContext.GetFadeVM(), 
			LoadingProcessTag.TeleportParty => (ILoadingScreen)Game.Instance.RootUIContext.GetFadeVM(), 
			LoadingProcessTag.ShowCityBuilder => (ILoadingScreen)Game.Instance.RootUIContext.GetLoadingScreenVM(), 
			LoadingProcessTag.ExceptionReporter => EmptyLoadingScreen.Instance, 
			LoadingProcessTag.ResetUI => (ILoadingScreen)Game.Instance.RootUIContext.GetFadeVM(), 
