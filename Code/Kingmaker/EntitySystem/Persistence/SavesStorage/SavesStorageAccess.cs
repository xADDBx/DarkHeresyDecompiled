...
public class SavesStorageAccess
{
	...
	private static Task<string> UploadImpl([NotNull] SaveInfo saveInfo, [NotNull] SaveCreateDTO dto)
	{
		...
		if (!SettingsRoot.Game.Main.AskedSendGameStatistic.GetValue() || !SettingsRoot.Game.Main.SendGameStatistic.GetValue())
		{
			return null;
		}
	...
	public static void Load(string remotePath)
	{
		...
		{
			LoadingProcess.Instance.StartLoadingProcess("SaveStorageAccess.LoadProcess", LoadProcess(remotePath));
		}
	}
