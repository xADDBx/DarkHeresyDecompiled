...
public class SceneLoader
{
	...
	private HashSet<string> m_LoadedUIScenes = new HashSet<string>();
	...
	public IEnumerable<string> LoadedUIScenes => m_LoadedUIScenes;
	...
	public IEnumerator ForceReloadUIScene(bool isSmokeTest = false)
	{
		using (CodeTimer.New(Logger, "Reload UI Scenes"))
		{
		...
		IEnumerator unloadInGameUiScene = UnloadInGameUiSceneCoroutine();
		while (unloadInGameUiScene.MoveNext())
		{
			yield return null;
		}
		if (!isSmokeTest && !string.IsNullOrEmpty("UI_Common_Scene") && !SceneManager.GetSceneByName("UI_Common_Scene").isLoaded && !(BuildModeUtility.Data?.Loading.IgnoreUI ?? false))
		{
			IEnumerator loadInGameUiScene = LoadUIScene();
			while (loadInGameUiScene.MoveNext())
			{
				yield return null;
			}
		}
		IEnumerator initializeUiScene = InitializeUIScene();
		while (initializeUiScene.MoveNext())
		{
			yield return null;
		}
	}
	...
	private IEnumerator UnloadInGameUiSceneCoroutine()
	{
		...
		{
			Game.Instance.RootUIContext.Clear();
			if (SceneManager.GetSceneByName("UI_Common_Scene").isLoaded)
			{
				Task op = BundledSceneLoader.UnloadSceneAsync("UI_Common_Scene");
	...
	private static IEnumerator LoadUIScene()
	{
	...
	private static IEnumerator InitializeUIScene()
	{
		using (CodeTimer.New(Logger, "Initialize UI Scene"))
		{
			using (ProfileScope.New("Initialize UI Scene"))
			{
				...
				IEnumerator initUiScene = InitUIScene();
	...
	private static IEnumerator InitUIScene()
	{
		yield return null;
	...
	public IEnumerator<object> LoadAreaCoroutine([NotNull] BlueprintArea area, [NotNull] BlueprintAreaPart areaPart, [CanBeNull] BlueprintAreaEnterPoint enterPoint, bool isSmokeTest = false, IProgress<float> progressValueCallback = null)
	{
		...
		if (CrossSceneRoot.IsEmpty)
		{
			using (CodeTimer.New(Logger, "Load Cross Scene Entities"))
			{
				...
				{
					IEnumerator matchState = LoadSceneEntitiesCoroutine(Game.Instance.Player.CrossSceneState, Enumerable.Empty<EntityViewBase>());
					while (matchState.MoveNext())
					{
						yield return null;
					}
		...
		using (CodeTimer.New(Logger, "Destroy Unattached Views"))
		{
			using (ProfileScope.New("Destroy Unattached Views"))
			{
				...
				{
					if (!entityViewBase.IsInState && !Game.Instance.Controllers.EntitySpawner.IsViewWaitToCreate(entityViewBase))
					{
						if (TraceLoading)
						{
	...
	private static void ApplyFowStartOptions(AreaPartBounds areaPartBounds)
	{
		...
		foreach (Entity entity in Game.Instance.EntityPools.Entities)
		{
	...
	public IEnumerator<object> ReloadAreaMechanicsCoroutine()
	{
		...
		Game.Instance.Controllers.EntitySpawner.SuppressSpawn.Release();
		yield return null;
		EntitySpawnController entitySpawner = Game.Instance.Controllers.EntitySpawner;
	...
	private async Task MatchAreaStaticAndDynamicScenesCoroutine(BlueprintArea area, [CanBeNull] BlueprintAreaPart blueprintAreaPart, bool isSmokeTest = false, IProgress<float> progressValueCallback = null)
	{
		...
		Game.Instance.EtudesSystem.ForceUpdateEtudes(SimpleBlueprintExtendAsObject.Or(blueprintAreaPart, area));
	...
	private void SetSceneEnabled(Scene scene, bool enabled)
	{
		...
		foreach (GameObject gameObject in rootGameObjects)
		{
			...
			ControllableGameObject component = gameObject.GetComponent<ControllableGameObject>();
			if (component != null)
			{
				ControllableState state = component.GetState();
				if (state != null && state.Active.HasValue)
				{
					gameObject.SetActive(state.Active.Value && enabled);
				}
			}
			else
			{
	...
	private static void CreateDataForView(SceneEntitiesState state, EntityViewBase view, bool load)
	{
		try
		{
			...
			else
			{
				...
				Game.Instance.Controllers.EntitySpawner.SpawnEntityImmediately(entity, state);
			}
		}
		catch (Exception ex)
		{
	...
	public IEnumerator<object> UnloadAreaCoroutine(bool forDispose = false, bool leaveStatics = false, [CanBeNull] HashSet<string> hotScenes = null)
	{
		...
		if (!forDispose)
		{
			...
			foreach (BaseUnitEntity allCrossSceneUnit in Game.Instance.Player.AllCrossSceneUnits)
			{
				...
				{
					Game.Instance.Controllers.EntityDestroyer.Destroy(allCrossSceneUnit);
				}
			}
		}
		yield return null;
		...
		if (BuildModeUtility.ExtraOptimization)
		{
			...
			Game.Instance.RootUIContext.StartUI(showLoadingScreen: true);
	...
	public IEnumerator ActivateAdditiveArea(BlueprintAreaEnterPoint enterPoint, bool isSmokeTest = false, bool showLoadingScreen = true)
	{
		...
		Game.Instance.EtudesSystem.ForceUpdateEtudes(area);
		...
		if (CrossSceneRoot.IsEmpty)
		{
			using (CodeTimer.New(Logger, "Load Cross Scene Entities"))
			{
				using (ProfileScope.New("Load Cross Scene Entities"))
				{
					IEnumerator enumerator4 = LoadSceneEntitiesCoroutine(Game.Instance.Player.CrossSceneState, Enumerable.Empty<EntityViewBase>());
	...
	public IEnumerator<object> LoadMainMenuCoroutine(Exception exception = null)
	{
		...
		resetTask.Wait();
		BugReportCanvas bugReportCanvas = UnityEngine.Object.FindAnyObjectByType<BugReportCanvas>();
