...
{
	private static ReplaceNodeLinkPositionsModifier s_ReplaceNodeLinkPositionsModifier = new ReplaceNodeLinkPositionsModifier
	{
		TurnBased = true
	};
	...
	public static ForcedPath ConstructPathTo(GraphNode node, Dictionary<GraphNode, WarhammerPathAiCell> pathData)
	{
		...
		list.Reverse();
		list2.Reverse();
		ForcedPath forcedPath = ForcedPath.Construct(list2, list);
		s_ReplaceNodeLinkPositionsModifier.Apply(forcedPath);
		return forcedPath;
	}
}
