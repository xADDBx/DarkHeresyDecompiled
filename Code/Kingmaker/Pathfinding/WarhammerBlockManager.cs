...
public class WarhammerBlockManager : MonoBehaviour
{
	...
	public bool NodeContainsAnyExcept(GraphNode node, WarhammerSingleNodeBlocker blocker, bool enemies, bool passThroughSmallUnits)
	{
		lock (m_Blocked)
		{
			if (!m_Blocked.TryGetValue(node, out var value))
			...
			if (hashSet.Contains(blocker) ? (hashSet.Count >= 2) : (hashSet.Count >= 1))
			{
				...
				{
					if (item != blocker && (!passThroughSmallUnits || !item.IsSmallUnit) && !item.IsBlockerSoftUnit)
					{
						return true;
					}
				}
	...
	public bool CanUnitStandOnNode(MechanicEntity unit, GraphNode targetNode)
	{
		...
		foreach (GridNodeBase node in GridAreaHelper.GetNodes(targetNode, unit.SizeRect, unit.Forward))
		{
	...
	{
		foreach (GridNodeBase node in GridAreaHelper.GetNodes(targetNode, sizeRect))
		{
	...
	public void InternalBlock(GraphNode node, WarhammerSingleNodeBlocker blocker, IntRect rect, Vector3 direction, BlockType blockType)
	{
		...
		{
			m_ChangeVersion++;
			foreach (GridNodeBase node2 in GridAreaHelper.GetNodes(node, rect, direction))
			{
