...
public static class GraphHelper
{
	...
	public static WeakpointSide GetWarhammerAttackSide(Vector3 targetFwd, Vector3 casterAttack, Size targetSize)
	{
		...
		WeakpointSide result = WeakpointSide.Front;
	...
	public static bool HasConnectionBetweenNodes(GridNodeBase nodeFrom, GridNodeBase nodeTo)
	{
		...
		return ((GridNode)nodeFrom).HasConnectionInDirection(num3);
	}

	public static int GetWarhammerCellDistance(GridNodeBase a, GridNodeBase b)
	{
	...
	public static IEnumerable<GridNodeBase> TryGetAllSuitableNodesForUnitAfterLinkTraversal(GridNodeBase linkEndNode, GraphNode linkStartNode, AbstractUnitEntity unit)
	{
		if (unit.Size > Size.Large)
		{
			yield break;
		}
		if (unit.Size <= Size.Medium || !unit.IsInCombat)
		{
			yield return linkEndNode;
			yield break;
		}
		Vector3 fwd = (Vector3)(linkEndNode.position - linkStartNode.position);
		int direction = GuessDirection(fwd);
		GridNodeBase result = ((direction == 1 || direction == 2) ? linkEndNode : linkEndNode.GetNeighbourAlongDirection(direction));
		if (result != null)
		{
			if (WarhammerBlockManager.Instance.CanUnitStandOnNode(unit, result))
			{
				yield return result;
			}
			int direction2 = ((direction == 0 || direction == 2) ? 3 : 0);
			result = result.GetNeighbourAlongDirection(direction2);
			if (result != null && WarhammerBlockManager.Instance.CanUnitStandOnNode(unit, result))
			{
				yield return result;
			}
		}
