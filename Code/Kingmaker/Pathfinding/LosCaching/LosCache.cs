...
{
	protected readonly GridGraph graph;

	protected readonly IntRect bounds;

	public LosCache(GridGraph graph, IntRect bounds)
	{
		this.graph = graph;
		this.bounds = bounds;
	}

	public abstract bool CheckLos(GridNodeBase origin, IntRect originSize, GridNodeBase end, IntRect endSize);
	...
	public static bool CanConsolidateByLos(CustomGridQuadtree q1, CustomGridQuadtree q2)
	{
		for (int i = q1.Rect.xmin; i <= q1.Rect.xmax; i++)
		{
			...
			{
				GridNodeBase node = q1.Graph.GetNode(i, j);
				...
				for (int k = q2.Rect.xmin; k <= q2.Rect.xmax; k++)
				{
					...
					{
						GridNodeBase node2 = q2.Graph.GetNode(k, l);
	...
	private static bool HasTwoWayLos(GridNodeBase node1, GridNodeBase node2)
	{
