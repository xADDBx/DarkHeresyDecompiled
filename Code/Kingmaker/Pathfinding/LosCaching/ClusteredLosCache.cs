...
public class ClusteredLosCache : LosCache
{
	private class LosCluster
	{
		...
		public bool CanGrowHorizontally
		{
			get
			{
				...
				{
					GridNodeBase node = Graph.GetNode(num, i);
					...
					for (int j = Rect.xmin; j <= Rect.xmax; j++)
					{
						...
						{
							GridNodeBase node2 = Graph.GetNode(j, k);
		...
		public bool CanGrowVertically
		{
			get
			{
				...
				{
					GridNodeBase node = Graph.GetNode(i, num);
					if (node == null || !node.Walkable)
					{
						continue;
					}
		...
		public LosCluster(GridGraph graph, IntRect rect, IntRect bounds)
		{
			Graph = graph;
			Bounds = bounds;
			m_Rect = rect;
	...
	public ClusteredLosCache(GridGraph graph, IntRect bounds)
		: base(graph, bounds)
	{
	...
	private bool IsEmptyCluster(LosCluster c)
	{
		for (int i = c.Rect.xmin; i <= c.Rect.xmax; i++)
		{
			...
			{
				GridNodeBase node = graph.GetNode(i, j);
				if (node != null && node.Walkable)
				{
					return false;
				}
	...
	public bool CheckLos(GridNodeBase from, GridNodeBase to)
	{
