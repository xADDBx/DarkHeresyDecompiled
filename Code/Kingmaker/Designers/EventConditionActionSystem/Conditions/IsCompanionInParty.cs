	...
	protected override bool CheckCondition()
	{
		foreach (BaseUnitEntity allCharacter in Game.Instance.Player.AllCharacters)
		{
			if (allCharacter.Blueprint == companion || allCharacter.Blueprint.PrototypeLink == companion)
			{
				...
				if (companionState != 0 && (MatchWhenActive || companionState != CompanionState.InParty) && (MatchWhenRemote || companionState != CompanionState.Remote) && (MatchWhenDetached || companionState != CompanionState.InPartyDetached) && (MatchWhenEx || companionState != CompanionState.ExCompanion))
				{
					return true;
				}
			}
