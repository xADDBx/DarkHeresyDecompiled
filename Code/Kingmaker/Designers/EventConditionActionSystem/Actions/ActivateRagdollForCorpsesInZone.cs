...
public class ActivateRagdollForCorpsesInZone : GameAction
{
	...
	[SerializeField]
	[Tooltip("Override RagdollTime on the controller to ensure ragdoll stays active for the full duration")]
	public bool OverrideRagdollTime = true;
	...
	private IEnumerator ActivateRagdollCoroutine(RigidbodyCreatureController ragdollController, BaseUnitEntity unit)
	{
		...
		bool wasRagdollInAnyState = ragdollController.IsActive;
		float originalRagdollTime = ragdollController.RagdollTime;
		PFLog.Default.Log($"ActivateRagdollForCorpsesInZone: Starting ragdoll for {unit.CharacterName}, wasActive: {wasRagdollActive}, wasInAnyState: {wasRagdollInAnyState}");
		if (OverrideRagdollTime)
		{
			ragdollController.RagdollTime = Duration;
			PFLog.Default.Log($"ActivateRagdollForCorpsesInZone: Overriding RagdollTime for {unit.CharacterName} from {originalRagdollTime} to {ragdollController.RagdollTime}");
		}
		if ((wasRagdollActive || wasRagdollInAnyState) && ForceRestart)
		{
		...
		{
			PFLog.Default.Warning("ActivateRagdollForCorpsesInZone: Ragdoll failed to start for " + unit.CharacterName + ", retrying...");
			ragdollController.CancelRagdoll();
			yield return null;
			ragdollController.StartRagdoll();
			yield return null;
			PFLog.Default.Log($"ActivateRagdollForCorpsesInZone: Ragdoll retry for {unit.CharacterName}, working: {ragdollController.RagdollWorking}");
		}
		...
		if (ragdollController != null && ((!wasRagdollActive && !wasRagdollInAnyState) || ForceRestart))
		{
		...
		if (OverrideRagdollTime)
		{
			ragdollController.RagdollTime = originalRagdollTime;
			PFLog.Default.Log($"ActivateRagdollForCorpsesInZone: Restored RagdollTime for {unit.CharacterName} to {originalRagdollTime}");
		}
