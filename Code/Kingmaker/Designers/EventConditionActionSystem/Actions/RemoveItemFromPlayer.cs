...
public class RemoveItemFromPlayer : GameAction
{
	...
	protected override void RunAction()
	{
		using (ContextData<GameLogDisabled>.RequestIf(m_Silent))
		{
			...
			long num = (flag ? Game.Instance.Player.Money : (itemToRemove.IsActuallyStackable ? GameHelper.GetPlayerCharacter().Inventory.Sum((ItemEntity i) => (i.Blueprint != itemToRemove) ? i.Count : 0) : GameHelper.GetPlayerCharacter().Inventory.Count((ItemEntity i) => i.Blueprint == itemToRemove)));
