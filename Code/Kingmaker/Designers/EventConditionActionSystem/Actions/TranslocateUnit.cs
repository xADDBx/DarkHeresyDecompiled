...
public class TranslocateUnit : GameAction
{
	...
	protected override void RunAction()
	{
		...
		GridNodeBase nearestNodeXZ = position.GetNearestNodeXZ();
		...
		{
			nearestNodeXZ = GridAreaHelper.GetNodesSpiralAround(nearestNodeXZ, new IntRect(0, 0, 0, 0), 2).FirstOrDefault((GridNodeBase node) => WarhammerBlockManager.Instance.CanUnitStandOnNode(targetUnit, node) && node.Walkable);
			if (nearestNodeXZ == null)
			{
				return;
			}
