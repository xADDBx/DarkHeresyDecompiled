...
public class Recruit : GameAction
{
	...
	private void ShowPartyInterface()
	{
		...
		EventBus.RaiseEvent(delegate(IGroupChangerHandler h)
		{
			h.HandleCall(delegate
			{
				...
				SelectionManagerFacade.MultiSelect(Game.Instance.Player.Party.Select((BaseUnitEntity character) => character.View).ToTempList());
			}, delegate
			{
	...
	private void SwitchToCompanion(RecruitData data)
	{
		...
		else
		{
			...
			{
				Game.Instance.Controllers.EntityDestroyer.Destroy(baseUnitEntity2);
