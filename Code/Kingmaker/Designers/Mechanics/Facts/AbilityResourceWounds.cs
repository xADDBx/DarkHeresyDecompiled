...
public class AbilityResourceWounds : BlueprintComponent, IAbilityResourceLogic, IAbilityRestriction
{
	...
	public void Spend(AbilityData ability)
	{
		...
		{
			Rulebook.Trigger(RuleHealDamage.Setup(caster, caster).WithMinMax(num, num).Create());
			return;
		}
		DamageTypeSettings damageTypeSettings = new DamageTypeSettings
		{
			Type = DamageType.Direct
		};
		IntermediateDamage resultDamage = Rulebook.Trigger(new RuleCalculateDamage(caster, caster, ability, null, damageTypeSettings.CreateDamage(num, num))).ResultDamage;
