...
internal readonly struct SurfaceJobDataFactory
{
	...
	private struct ContainerAdapter : IIdentifierContainer
	{
		...
		public unsafe ContainerAdapter(int estimatedCellsCount, GridGraph graph, Allocator allocator)
		{
			m_Graph = graph;
				...
				for (int num13 = num12 + 8; i < num13; i++)
				{
					...
					for (int num14 = num11 + 8; j < num14; j++)
					{
						...
						if (j < m_GridDimensionX && i < m_GridDimensionY)
						{
							...
							GridNodeBase gridNodeBase = m_Graph.nodes[num15];
							GridMeshNode meshMode = m_AdditionalData.NodeData[num15].MeshMode;
							cellUnion.intermediateCell.indexInGrid = num15;
							cellUnion.intermediateCell.packedHeight = gridNodeBase.position.y;
							cellUnion.intermediateCell.packedCornerOffsets.packedCornerOffsetNE = meshMode.packedCornerOffsetNE;
							cellUnion.intermediateCell.packedCornerOffsets.packedCornerOffsetNW = meshMode.packedCornerOffsetNW;
							cellUnion.intermediateCell.packedCornerOffsets.packedCornerOffsetSW = meshMode.packedCornerOffsetSW;
							cellUnion.intermediateCell.packedCornerOffsets.packedCornerOffsetSE = meshMode.packedCornerOffsetSE;
	...
	public SurfaceJobDataFactory(GridGraph graph, List<AreaData> areas)
	{
		m_Graph = graph;
		m_Areas = areas;
	}
