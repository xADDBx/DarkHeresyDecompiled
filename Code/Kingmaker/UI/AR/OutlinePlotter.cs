...
internal struct OutlinePlotter
{
	...
	private void PlotTurnBackward(in OutlinePlotCommand command)
	{
		...
		float num = math.lerp(cornerHeight2, cell2.GetCornerHeight(in corner2), 0.5f);
		float3 @float = m_Basis.Forward * m_HalfCellSize;
		float3 float2 = m_CursorPosition + @float;
		float2.y = cornerHeight;
		float3 cursorPosition = m_CursorPosition;
		cursorPosition.y = num;
		if ((double)math.abs(m_CursorPosition.y - num) > 0.01)
		{
			float num2 = math.sign(cursorPosition.y - m_CursorPosition.y);
			...
			float3 float6 = num2 * math.normalize(float5 - float2);
		...
		{
			quaternion rotation3 = quaternion.LookRotation(math.normalize(cursorPosition - float2), new float3(0f, 1f, 0f));
			m_SplinePlotter.BreakLine();
			m_SplinePlotter.PushPoint(float2, 0f, 0f, masked);
			m_SplinePlotter.PushPoint(float2, rotation3, 0f, 0f, masked2);
			m_SplinePlotter.PushPoint(cursorPosition, rotation3, 0f, 0f, masked2);
		}
