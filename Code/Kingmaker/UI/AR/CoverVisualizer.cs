...
public class CoverVisualizer : MonoBehaviour, IUnitMovableAreaHandler, ISubscriber<IMechanicEntity>, ISubscriber, ITurnBasedModeHandler, ITurnBasedModeResumeHandler, ITurnStartHandler, IInterruptTurnStartHandler, IRoundStartHandler, IPreparationTurnBeginHandler, IPreparationTurnEndHandler
{
	...
	private readonly Dictionary<uint, HashSet<DestructibleEntity>> m_DestructiblesCache = new Dictionary<uint, HashSet<DestructibleEntity>>();
	...
	private void Awake()
	{
		...
		UpdatePlayerTurn();
		m_IsDeploymentPhase = Game.Instance.Controllers.TurnController.IsPreparationTurn && Game.Instance.Controllers.TurnController.IsDeploymentAllowed;
	}
	...
	private void Update()
	{
		...
		GridNodeBase gridNodeBase = (GridNodeBase)ObstacleAnalyzer.GetNearestNode(worldPosition).node;
		if (gridNodeBase != null && (gridNodeBase.NodeIndex != m_LastNodeId || flag))
		{
			m_LastNodeId = gridNodeBase.NodeIndex;
			if (!IsNodeCoverVisible(gridNodeBase))
			{
				HideCoverMeshes();
			}
			else
			{
				UpdateCoverMeshes(gridNodeBase);
			}
		}
	}

	private void UpdateCoverMeshes(GridNodeBase currentNode)
	{
		GridGraph gridGraph = (GridGraph)currentNode.Graph;
		...
		for (int i = 0; i < 5; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				GridNodeBase node = gridGraph.GetNode(xCoordinateInGrid + j - 2, zCoordinateInGrid + i - 2);
				...
				if (node != null && IsNodeCoverVisible(node) && node.Walkable)
				{
					...
					if (firstUnit == null || !firstUnit.GetOptional<PartProvidesCover>())
					{
	...
	private void UpdateDestructiblesCache()
	{
		...
		{
			foreach (GridNodeBase occupiedNode in destructibleEntity.GetOccupiedNodes())
			{
	...
	private bool IsNodeCoverVisible(GridNodeBase node)
	{
