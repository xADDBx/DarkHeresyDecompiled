...
internal struct BuildPathJob
{
	private struct ContainerAdapter : IIdentifierContainer, IDisposable
	{
		...
		private readonly AdditionalGraphData m_AdditionalData;

		private NativeList<CellUnion> m_Cells;

		private NativeList<int> m_CellIndices;

		public ContainerAdapter(int estimatedCellsCount, GridGraph graph, NativeList<CellUnion> cells, NativeList<int> cellIndices)
		{
			m_Graph = graph;
		...
		public void Push(int cellIdentifier)
		{
			...
			GridNodeBase gridNodeBase = m_Graph.nodes[cellIdentifier];
			GridMeshNode meshMode = m_AdditionalData.NodeData[cellIdentifier].MeshMode;
			value.intermediateCell.indexInGrid = cellIdentifier;
			value.intermediateCell.packedHeight = gridNodeBase.position.y;
			if (gridNodeBase.Walkable)
			{
				value.intermediateCell.flags = (IntermediateCellFlags)(gridNodeBase as GridNode).GetAllConnectionInternal();
			}
			else
			{
				value.intermediateCell.flags = (IntermediateCellFlags)0;
			}
			value.intermediateCell.packedCornerOffsets.packedCornerOffsetNE = meshMode.packedCornerOffsetNE;
			value.intermediateCell.packedCornerOffsets.packedCornerOffsetNW = meshMode.packedCornerOffsetNW;
			value.intermediateCell.packedCornerOffsets.packedCornerOffsetSW = meshMode.packedCornerOffsetSW;
			value.intermediateCell.packedCornerOffsets.packedCornerOffsetSE = meshMode.packedCornerOffsetSE;
	...
	public BuildPathJob(GridSettings gridSettings, int materialId, PathLineSettings pathLineSettings, GridGraph graph, IAreaSource pathSource, ProceduralMesh<LineVertex, uint> proceduralMesh, float3 positionOffset, NativeList<ApproximatePathSegment> approximatePath)
	{
	...
	public JobHandle Schedule(JobHandle dependsOn = default(JobHandle))
	{
		...
		{
			PFLog.Pathfinding.Error("Trying to draw single-point path. This is impossible.");
			nativeList.Dispose();
			nativeList2.Dispose();
			return dependsOn;
		}
