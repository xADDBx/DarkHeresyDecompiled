...
public class ConsoleCursor : BaseCursor, IRewiredCursorController, ITurnBasedModeHandler, ISubscriber, ITurnBasedModeResumeHandler, IInteractionHighlightUIHandler
{
	...
	public ReactiveProperty<bool> IsActiveProperty = new ReactiveProperty<bool>(value: false);

	public ReactiveProperty<bool> IsNotActiveProperty = new ReactiveProperty<bool>(value: true);
	...
	{
		InteractionHighlightController instance = InteractionHighlightController.Instance;
		if ((instance == null || !instance.IsGlobalHighlighting) && !PointerController.InGui && (!base.NoTexts || m_CursorImage.gameObject.activeInHierarchy))
		{
			m_CursorTransform.anchoredPosition = UnitPathManager.Instance.PointerCellDecalCornersPositions.Select((Vector3 p) => UtilityRect.ObjectPixelPositionInRect(p, m_CursorTransform)).MaxBy((Vector2 p) => p.x);
		}
	}
	...
	private void SwitchRotate(bool rotate)
	{
		...
		m_NoMoveGroup.transform.rotation = identity;
	}
