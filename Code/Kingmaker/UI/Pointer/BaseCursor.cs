...
public abstract class BaseCursor : MonoBehaviour, IDisposable
{
	...
	[Header("Ability")]
	[SerializeField]
	protected CanvasGroup m_AbilityGroup;
	...
	[Header("Texts")]
	[SerializeField]
	protected CanvasGroup m_TextsGroup;
	...
	[Header("Notification")]
	[SerializeField]
	protected CanvasGroup m_NotificationGroup;
	...
	[Header("Additional")]
	[SerializeField]
	protected CanvasGroup m_NoMoveGroup;
	...
	public bool NoTexts { get; private set; }
	...
	{
		if (UIVisibilityState.VisibilityPreset.CurrentValue.HasFlag(UIVisibilityFlags.Pointer))
		{
			return false;
		}
	...
	public void SetCursor(CursorType type)
	{
		if (m_CurrentType != type)
		{
			...
			m_CursorImage.sprite = ConfigRoot.Instance.Cursors.GetSprite(type);
			...
			Cursor.SetCursor(ConfigRoot.Instance.Cursors.GetTexture(type), hotspot, CursorMode.Auto);
			m_AbilityGroup.alpha = (m_CastMode ? 1f : 0f);
			m_CurrentType = type;
			OnSetCursor(type);
		}
	}
	...
	{
		NoTexts = string.IsNullOrEmpty(upperText) && string.IsNullOrEmpty(lowerText);
		m_TextsGroup.alpha = (NoTexts ? 0f : 1f);
		OnSetText(!NoTexts);
		if (!NoTexts)
		{
	...
	{
		bool flag = !string.IsNullOrEmpty(notificationText);
		m_NotificationGroup.alpha = (flag ? 1f : 0f);
	...
	{
		m_NoMoveGroup.alpha = (noMove ? 1f : 0f);
	}

	public void ClearComponents()
	{
		m_AbilityImage.sprite = null;
		m_AbilityGroup.alpha = 0f;
		SetTexts(null, null);
