...
{
	[SerializeField]
	protected float defaultFov = 30f;

	[SerializeField]
	protected Vector3 defaultPosition = new Vector3(0.018005f, 1.52002f, -0.65f);
	...
	public void SetCameraFovAndYpos(BaseUnitEntity unit)
	{
		float dollRoomCameraFovOverride = unit.Blueprint.VisualSettings.DollRoomCameraFovOverride;
		m_Camera.fieldOfView = ((dollRoomCameraFovOverride == 0f) ? defaultFov : dollRoomCameraFovOverride);
		float dollRoomCameraYposOverride = unit.Blueprint.VisualSettings.DollRoomCameraYposOverride;
		m_Camera.transform.localPosition = ((dollRoomCameraYposOverride == 0f) ? defaultPosition : new Vector3(defaultPosition.x, dollRoomCameraYposOverride, defaultPosition.z));
		Start();
	}
