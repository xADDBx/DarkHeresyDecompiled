...
public class DollRoomBase : MonoBehaviour, ISaveManagerHandler, ISubscriber
{
	...
	[SerializeField]
	private int m_RenderTextureResolutionMultiplier = 1;

	[SerializeField]
	private int m_RenderTextureAntiAliasing;

	[SerializeField]
	private FilterMode m_RenderTextureFilterMode = FilterMode.Bilinear;
	...
	private CustomPostProcess m_DollPostProcess;
	...
	public virtual void Initialize(DollRoomTargetController characterController)
	{
		...
		{
			m_RenderTextureSize = characterController.GetRawImageSize();
			m_TargetController.SetTarget(m_TargetPlaceholder);
		}
		EnsureRenderTexture();
		EnsureBackground();
	...
	private CustomPostProcess GetCustomPostProcess(Volume postProcessVolume)
	{
		foreach (VolumeComponent component in postProcessVolume.profile.components)
		{
			if (component.GetType() == typeof(CustomPostProcess))
			{
				return (CustomPostProcess)component;
			}
		}
		return null;
	}
	...
	private void SetupInventoryDollPostProcessAndAnimation(Volume targetVolume)
	{
		...
		{
			m_DollDissolveProperty = m_DollPostProcess.CompositeParameterList.Value[0].Parameters.FirstOrDefault((ShaderPropertyParameter p) => p.Property.Name == "_Dissolve")?.Property;
		}
		if (m_DollDissolveProperty != null)
		{
	...
	private void RenderBackgroundForDollRoom()
	{
		...
		if (m_DollPostProcess == null)
		{
			PFLog.TechArt.Error(base.gameObject, "Can't find post process for doll room");
			return;
		}
		if (m_DollBackgroundProperty == null)
		{
			m_DollBackgroundProperty = m_DollPostProcess.CompositeParameterList.Value[0].Parameters.FirstOrDefault((ShaderPropertyParameter p) => p.Property.Name == "_MainTex")?.Property;
		}
		if (m_DollBackgroundProperty == null)
		{
			PFLog.TechArt.Error(base.gameObject, "Can't find DollBackground property in post process for doll room");
		}
		else
		{
