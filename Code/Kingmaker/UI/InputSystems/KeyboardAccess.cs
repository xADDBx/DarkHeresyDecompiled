...
public class KeyboardAccess : IFocusHandler, ISubscriber, IService, IDisposable
{
	...
	public class Binding
	{
		...
		public readonly bool SplitPressTrigger;
		...
		public Binding(string name, KeyCode key, GameModeType gameMode, bool isCtrlDown, bool isAltDown, bool isShiftDown, TriggerType triggerType, ModificationSide side = ModificationSide.Any, bool worksWhenUIPaused = true, bool splitPressTrigger = false)
		{
			Name = name;
			Key = key;
			GameMode = gameMode;
			...
			SplitPressTrigger = splitPressTrigger;
		}
	...
	public void RegisterBinding(string name, KeyCode key, IEnumerable<GameModeType> gameModes, bool worksWhenUIPaused = true, bool splitPressTrigger = false)
	{
		foreach (GameModeType gameMode in gameModes)
		{
			RegisterBinding(name, key, gameMode, ctrl: false, alt: false, shift: false, TriggerType.KeyDown, ModificationSide.Any, worksWhenUIPaused, splitPressTrigger);
		}
	}

	public void RegisterBinding(string name, KeyCode key, IEnumerable<GameModeType> gameModes, bool ctrl, bool alt, bool shift, TriggerType release = TriggerType.KeyDown, ModificationSide side = ModificationSide.Any, bool worksWhenUIPaused = true, bool splitPressTrigger = false)
	{
		foreach (GameModeType gameMode in gameModes)
		{
			RegisterBinding(name, key, gameMode, ctrl, alt, shift, release, side, worksWhenUIPaused, splitPressTrigger);
		}
	}

	private void RegisterBinding(string name, KeyCode key, GameModeType gameMode, bool ctrl, bool alt, bool shift, TriggerType release, ModificationSide side, bool worksWhenUIPaused, bool splitPressTrigger)
	{
		foreach (Binding binding in m_Bindings)
		{
		...
		Binding item = new Binding(name, key, gameMode, ctrl, alt, shift, release, side, worksWhenUIPaused, splitPressTrigger);
		m_Bindings.Add(item);
	}
	...
	public void RegisterBinding(string name, KeyBindingData data, GameModesGroup group, bool isTriggerOnHold, bool isHoldTrigger = false)
	{
		if (data.Key != 0)
		{
			...
			{
				RegisterBinding(name + UIConsts.SuffixOn, data.Key, GetGameModesArray(group), data.IsCtrlDown, data.IsAltDown, data.IsShiftDown, TriggerType.KeyDown, ModificationSide.Any, worksWhenUIPaused, splitPressTrigger: true);
				RegisterBinding(name + UIConsts.SuffixOff, data.Key, GetGameModesArray(group), data.IsCtrlDown, data.IsAltDown, data.IsShiftDown, TriggerType.KeyUp, ModificationSide.Any, worksWhenUIPaused, splitPressTrigger: true);
			}
		}
	}
	...
	public void RegisterBuiltinBindings()
	{
		...
		if (BuildModeUtility.IsDevelopment)
		{
			...
			RegisterBinding("ReloadUI", KeyCode.R, gameModesArray, ctrl: false, alt: true, shift: false);
			RegisterBinding("ReloadUIScene", KeyCode.R, gameModesArray, ctrl: false, alt: true, shift: true);
