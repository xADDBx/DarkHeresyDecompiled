...
public class CheatsJira
{
	...
	private static void CollectGameInfo()
	{
		...
		try
		{
			...
			text2 = ReportingRaycaster.Instance.GetUnderMouseBlueprintName();
		}
		catch
		{
		}
	...
	internal static void RapidBug(Action action, bool b)
	{
		...
		LoadingProcess.Instance.StartLoadingProcess("BugActionProcess", BugActionProcess(action));
	}
	...
	internal static void RapidLoreBug(bool saves)
	{
		...
		BlueprintCue currentCue = Game.Instance.Controllers.DialogController.CurrentCue;
		string text4 = "";
		if (currentCue != null)
		{
	...
	private static string GetDescription(bool saves, BlueprintAreaPart areaPart = null)
	{
		...
		text += $"%0AMain Character: {blueprintName}/{gender}";
	...
	public static string GetCutscenesInfo()
	{
		...
		List<CutscenePlayerData> list = new List<CutscenePlayerData>(Game.Instance.EntityPools.Cutscenes);
		if (list.Count > 0)
		{
