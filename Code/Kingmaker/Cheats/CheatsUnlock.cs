...
public class CheatsUnlock
{
	...
	{
		Game.Instance.EtudesSystem.MarkConditionsDirty();
	}
	...
	private static void StartEtude(string parameters)
	{
		...
		{
			Game.Instance.EtudesSystem.StartEtude(blueprint, "cheats");
		}
	}
	...
	private static void CompleteEtude(string parameters)
	{
		...
		{
			Game.Instance.EtudesSystem.MarkEtudeCompleted(blueprint, "cheat");
		}
	}
	...
	private static void CheckEtude(string parameters)
	{
		...
		{
			PFLog.SmartConsole.Log(Game.Instance.EtudesSystem.GetDebugInfo(blueprint));
		}
	}

	public static void UnlockCompanionAllStories(string parameters)
	{
	...
	public static void DebugObjectivePrevious()
	{
		if (s_Quest == null)
		{
			UtilityMessageBox.SendWarning("Quest is null");
			return;
		}
		...
		UtilityMessageBox.SendWarning($"QuestObjective: {Utilities.GetBlueprintName(s_QuestObjective.Blueprint)} ({s_QuestObjective.Blueprint.Title} {s_QuestObjective.State})");
	}
	...
	{
		List<Quest> list = new List<Quest>(Game.Instance.QuestBook.Quests);
		if (list.Count == 0)
		{
			UtilityMessageBox.SendWarning("Quest count is 0");
			return;
		}
		int i;
		...
		s_Quest = list[i];
		UtilityMessageBox.SendWarning($"Quest: {Utilities.GetBlueprintName(s_Quest.Blueprint)} ({s_Quest.Blueprint.Title})");
	}
	...
	private static void FailObjective(string parameters)
	{
		...
		foreach (Quest quest in Game.Instance.QuestBook.Quests)
		{
			foreach (QuestObjective objective in quest.Objectives)
			{
				...
				{
					Game.Instance.QuestBook.FailObjective(objective.Blueprint);
	...
	public static void RemoveFeature(BlueprintUnitFact blueprint)
	{
		if (blueprint != null)
		{
			(Utilities.GetUnitUnderMouse() ?? GameHelper.GetPlayerCharacter()).Facts.Remove(blueprint);
		}
	}
	...
	private static void CreateAllItems(string parameters)
	{
		foreach (BlueprintItem scriptableObject in Utilities.GetScriptableObjects<BlueprintItem>())
		{
			try
			{
				if (!Utilities.GetBlueprintPath(scriptableObject).Contains("Legacy"))
				{
	...
	private static void RecruitCompanion(string parameters)
	{
		...
		if (!paramBool.HasValue || paramBool.Value)
		{
			...
			Game.Instance.Controllers.EntitySpawner.SpawnEntityImmediately(baseUnitEntity, crossSceneState);
		...
		BaseUnitEntity baseUnitEntity2 = Game.Instance.Controllers.EntitySpawner.SpawnUnit(blueprint, position, Quaternion.identity, crossSceneState);
	...
	private static void CompleteQuests(string parameters)
	{
		foreach (Quest quest in Game.Instance.QuestBook.Quests)
		{
			...
			{
				QuestObjectiveState objectiveState = Game.Instance.QuestBook.GetObjectiveState(allObjective);
				if (objectiveState != QuestObjectiveState.Completed && objectiveState != QuestObjectiveState.Failed)
				{
					Game.Instance.QuestBook.GiveObjective(allObjective);
					Game.Instance.QuestBook.CompleteObjective(allObjective);
