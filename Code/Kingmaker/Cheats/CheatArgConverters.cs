...
public static class CheatArgConverters
{
	...
	internal static (bool success, object result) UnitConverter(string value, Type targetType)
	{
		...
		IEnumerable<AbstractUnitEntity> source = Game.Instance.EntityPools.AllUnits.Where((AbstractUnitEntity v) => v.UniqueId == value);
		if (source.Any())
		{
		...
		IEnumerable<AbstractUnitEntity> source2 = Game.Instance.EntityPools.AllUnits.Where((AbstractUnitEntity v) => v.Name == value);
		if (source2.Any())
		{
	...
	internal static (bool success, object result) MechanicEntityConverter(string value, Type targetType)
	{
		...
		IEnumerable<MechanicEntity> source = Game.Instance.EntityPools.MechanicEntities.Where((MechanicEntity v) => v.UniqueId == value);
		if (source.Any())
		{
			if (!source.Skip(1).Any())
			{
		...
		IEnumerable<MechanicEntity> source2 = Game.Instance.EntityPools.MechanicEntities.Where((MechanicEntity v) => v.Name == value);
		if (source2.Any())
		{
			if (!source2.Skip(1).Any())
			{
