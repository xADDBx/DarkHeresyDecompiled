...
public class Utilities
{
	...
	{
		BlueprintList.Entry entry = GetAllBlueprints().Entries.Where((BlueprintList.Entry e) => !e.IsLegacy && e.Type != null && (e.Type == typeof(T) || e.Type.IsSubclassOf(typeof(T)))).FirstOrDefault((BlueprintList.Entry e) => e.Guid == name || e.Name == name);
		if (entry != null)
		{
	...
	internal static BaseUnitEntity GetUnitForCheat()
	{
		...
		{
			baseUnitEntity = ((Game.Instance.Controllers.SelectionCharacter.SelectedUnits.Count <= 0) ? GameHelper.GetPlayerCharacter() : Game.Instance.Controllers.SelectionCharacter.SelectedUnits[0]);
		}
	...
	public static string GetDesigner(BlueprintArea area)
	{
		...
		string enumDescription = GetEnumDescription(area.AreaDesigner);
	...
	public static string GetEnumDescription(Enum e)
	{
		...
		if (name != null)
		{
			...
			if (field != null)
			{
				return field.GetCustomAttributes<DescriptionAttribute>(inherit: false).FirstOrDefault()?.Description;
			}
		}
		return null;
	}
	...
	[Obsolete("PF stuff, doesn't work for WH")]
