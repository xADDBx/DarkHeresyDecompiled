...
internal static class CheatArgPreprocessors
{
	...
	internal static (bool success, string result) SelectedUnits(string value, string targetType)
	{
		...
		return (success: true, result: string.Join(" ", Game.Instance.Controllers.SelectionCharacter.SelectedUnits.Select((BaseUnitEntity x) => x.UniqueId)));
	}
	...
	internal static (bool success, string result) Cursor(string value, string targetType)
	{
		...
		Vector3 worldPosition = Game.Instance.Controllers.ClickEventsController.WorldPosition;
