...
internal class CheatsDebug
{
	...
	public static void RegisterCommands(KeyboardAccess keyboard)
	{
		if (BuildModeUtility.CheatsEnabled)
		{
			...
			keyboard.Bind("ReloadUI", ReloadUI);
			keyboard.Bind("ReloadUIScene", ReloadUIScene);
	...
	{
		AKRESULT aKRESULT = AkUnitySoundEngine.StartProfilerCapture(Path.Combine(ApplicationPaths.DevelopmentDataPath, "WwiseProfilerSession.prof"));
	...
	[Cheat(Name = "reload_ui_scene", ExecutionPolicy = ExecutionPolicy.PlayMode)]
	public static void ReloadUIScene()
	{
		MonoSingleton<CoroutineRunner>.Instance.StartCoroutine(SceneLoader.Instance.ForceReloadUIScene());
	}

	[Cheat(Name = "toggle_uifx", Description = "Toggle monitor postprocess volume")]
	public static void ToggleUIFx()
	{
		Volume componentInChildren = GameObject.Find("UIAdditionalCamera").transform.GetComponentInChildren<Volume>();
	...
	{
		Game.Instance.RootUIContext.ChangeUIPlatform(nextPlatform: true);
	}

	[Cheat(Name = "change_ui_prev_platform", ExecutionPolicy = ExecutionPolicy.PlayMode)]
	public static void ChangeUIPrevPlatform()
	{
		Game.Instance.RootUIContext.ChangeUIPlatform(nextPlatform: false);
	}
	...
	private static void ShowDistance(string parameters = null)
	{
		...
		{
			if (SelectionManagerFacade.IsSelected(partyAndPet))
			{
	...
	{
		BaseDisposable.LogAllUndisposed(Game.Instance.RootUIContext.CreateLogChannelLoggerWrapper(PFLog.UI, "Undisposed"));
	}
