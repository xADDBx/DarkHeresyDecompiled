...
{
	private readonly IController[] m_ControllerCache;
	...
	{
		Type = type;
		m_ControllerCache = controllers.ToArray();
		m_Controllers = controllers.Where((IController v) => v != null).ToArray();
	...
	public void Tick()
	{
		...
		ITimeController service = InterfaceServiceLocator.GetService<ITimeController>();
		for (int i = 0; i < m_ControllersTick.Length; i++)
		{
			if (!service.CanTick(m_ControllersTick[i].controller))
			{
				continue;
			}
	...
	{
		return GetControllerOptional<T>() ?? throw new InvalidOperationException($"Controller {typeof(T).Name} is missing in game mode {Type}");
	}

	[CanBeNull]
	public T GetControllerOptional<T>() where T : class, IController
	{
		return (T)m_ControllerCache[Indexer<T>.Index];
	}
