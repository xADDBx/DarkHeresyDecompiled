...
public class WarhammerLocalMapRenderer : MonoBehaviour
{
	...
	private GridGraph m_CachedGraph;
	...
	{
		GridGraph graph = GetGraph(AstarPath.active);
	...
	private void UpdateGraphTexture()
	{
		...
		{
			GridNodeBase obj = m_CachedGraph.nodes[j];
			Vector3 vector = obj.Vector3Position();
			@float = math.min(@float, vector);
			float2 = math.max(float2, vector);
			if (obj.Walkable)
			{
				num2 = math.min(vector.y, num2);
				num3 = math.max(vector.y, num3);
			}
			list.Add(vector.y);
		}
		float x = num3 - num2;
	...
	private GridGraph GetGraph(AstarPath active)
	{
		if (!(active != null))
		{
			return null;
		}
		return active.graphs?.OfType<GridGraph>().FirstOrDefault();
	}
	...
	private DrawResults GenerateDrawResults()
	{
		...
		result.ScreenRect = CalculateScreenRect(Game.Instance.GetCamera());
