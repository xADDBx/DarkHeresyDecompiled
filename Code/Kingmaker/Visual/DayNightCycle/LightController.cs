...
[ExecuteInEditMode]
public class LightController : RegisteredBehaviour, IAreaLoadingStagesHandler, ISubscriber
{
	[Serializable]
	...
	private static readonly int FakeLightColor = Shader.PropertyToID("_FakeLightColorE");

	private static readonly int FakeLightIntensity = Shader.PropertyToID("_FakeLightIntensity");
	...
	private void SetupCamera(SceneLightConfig config)
	{
		...
		if (!(cameraRig == null))
		{
			...
			cameraRig.CharacterContourLight.enabled = (config ? config.CameraContourLightEnabled : SceneLightConfig.CameraContourLightDefaultEnabled);
			cameraRig.CharacterContourLight.color = (config ? config.ContourLightColor : SceneLightConfig.ContourLightDefaultColor);
			cameraRig.CharacterContourLight.intensity = (config ? config.ContourLightIntensity : SceneLightConfig.ContourLightDefaultIntensity);
		}
	}
	...
	private void SetupCharacterFakeLight()
	{
		Character[] array = UnityEngine.Object.FindObjectsByType<Character>(FindObjectsInactive.Include, FindObjectsSortMode.None);
		List<Renderer> list = new List<Renderer>();
		Character[] array2 = array;
		foreach (Character character in array2)
		{
			list.AddRange(character.GetComponentsInChildren<Renderer>());
		}
		foreach (Renderer item in list)
		{
			MaterialPropertyBlock materialPropertyBlock = new MaterialPropertyBlock();
			materialPropertyBlock.SetColor(FakeLightColor, m_SoloConfig.characterFakeLightColor);
			materialPropertyBlock.SetFloat(FakeLightIntensity, m_SoloConfig.characterFakeLightIntensity);
	...
	{
		SetupCharacterFakeLight();
	}

	public void ApplyFlashlightConfig(UnitFlashlight unitFlashlight)
	{
		SceneLightConfig sceneLightConfig = SelectConfig(m_CurrentTimeOfDay);
		if (!(sceneLightConfig == null))
		{
			unitFlashlight.ApplyFlashLightConfig(sceneLightConfig.FlashlightConfig);
		}
	}
}
