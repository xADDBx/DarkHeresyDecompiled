...
public class UnitDismembermentManager : MonoBehaviour
{
	...
	public class BoneDataObsolete : IOwlPackable, IOwlPackable<BoneDataObsolete>
	{
		...
		{
			BoneDataObsolete source = new BoneDataObsolete();
			result = Unsafe.As<BoneDataObsolete, TPossiblyBase>(ref source);
		}
		...
		public virtual void Serialize<TFormatter>(TFormatter formatter, SerializerState state) where TFormatter : IOutputFormatter
		{
			...
			ushort type = state.TypeLibrary.RegisterType<BoneDataObsolete>(OwlPackTypeInfo);
		...
		public virtual void Deserialize<TFormatter>(TFormatter formatter, uint objectId, DeserializerState state) where TFormatter : IInputFormatter
		{
			...
			TypeInfo typeInfo = state.TypeLibrary.GetTypeInfo<BoneDataObsolete>();
	...
	public void ApplyImpulse(Vector3 direction, float additionalMagnitude, DamageType damageType)
	{
		...
		m_LastImpulse.Time = Game.Instance.Controllers.TimeController.RealTime;
	...
	{
		if (m_LastImpulse != null && !(Game.Instance.Controllers.TimeController.RealTime > m_LastImpulse.Time + 0.2f.Seconds()) && m_LastImpulse.DamageType != DamageType.Toxic)
		{
	...
	public void StartDismemberment(StatefulRandom random, int setIndex, Vector3 impulse, GameObject set = null, bool bludgeon = false)
	{
		...
		SkinnedMeshRenderer skinnedMeshRenderer = componentsInChildren.Except(componentsInChildren2).ToArray().FirstOrDefault();
		...
		for (int m = 0; m < component.Pieces.Count; m++)
		{
			...
			for (int n = 0; n < dismembermentPieceDescriptor.ImpulseRigidBodies.Length; n++)
			{
				...
				SkinnedMeshRenderer componentInChildren2 = dismembermentPieceDescriptor.Root.GetComponentInChildren<SkinnedMeshRenderer>();
				...
				if (material != null)
				{
					Material[] sharedMaterials = componentInChildren2.sharedMaterials;
					...
					componentInChildren2.sharedMaterials = sharedMaterials;
				}
				componentInChildren2.enabled = false;
				list6.Add(componentInChildren2);
			}
		}
		foreach (KeyValuePair<string, Transform> item in new Dictionary<string, Transform>())
		{
		...
		int num5 = 0;
		for (int num6 = 0; num6 < list3.Count - 1; num6++)
		{
			for (int num7 = num5 + 1; num7 < list3.Count; num7++)
			{
				if (list3[num6].name == list3[num7].name)
				{
					list2.Add(new KeyValuePair<Rigidbody, Rigidbody>(list3[num6], list3[num7]));
					break;
				}
			}
			num5 = num6 + 1;
		...
		{
			Collider component3 = list2[num8].Key.GetComponent<Collider>();
			Collider component4 = list2[num8].Value.GetComponent<Collider>();
		...
		if (component5 != null && component5.Blueprint != null)
		{
			...
			if (material != null)
			{
				HitEntry[] hitEffects = ConfigRoot.Instance.HitSystemRoot.HitEffects;
		...
		if ((bool)componentInChildren)
		{
			...
			FxLocator[] componentsInChildren3 = componentInChildren.gameObject.GetComponentsInChildren<FxLocator>(includeInactive: true);
			for (int num9 = 0; num9 < componentsInChildren3.Length; num9++)
			{
				componentsInChildren3[num9].gameObject.SetActive(value: false);
			}
		...
		Game.Instance.Controllers.CoroutinesController.Start(PostProcess(componentInChildren.gameObject, list6.ToArray(), visualEffectPrefab, component.gameObject));
	...
	public void RestoreState()
	{
		...
		{
			for (int n = num + 1; n < list3.Count; n++)
			{
				if (list3[m].name == list3[n].name)
				{
					list2.Add(new KeyValuePair<Rigidbody, Rigidbody>(list3[m], list3[n]));
					break;
				}
			}
			num = m + 1;
		...
		{
			Collider component3 = list2[num2].Key.GetComponent<Collider>();
			Collider component4 = list2[num2].Value.GetComponent<Collider>();
			if (component3 != null && component4 != null)
			{
				Physics.IgnoreCollision(component3, component4);
			}
