...
public class IKController : MonoBehaviour
{
	...
	private void CheckState()
	{
		...
		m_EnableIK = m_EnableIK && CharacterUnitEntity.EntityData.LifeState.IsConscious && !CharacterUnitEntity.EntityData.IsProne && !CharacterUnitEntity.EntityData.LifeState.IsDead;
		if (m_EnableIK)
		{
			if ((bool)BipedIk)
			{
	...
	private void CheckStylesForIk(WeaponType mainHandWeaponType, WeaponType offHandWeaponType, UnitEntityView unitEntityView)
	{
		if (!unitEntityView)
		{
			return;
		}
		if (m_AnimationManager.CanRunIdleAction() && !m_AnimationManager.HasActedAnimation && CharacterSystem.DisplayOptions.IsInDollRoom && mainHandWeaponType.IsTwoHanded())
		{
			SetIkLeftHand();
		}
		else if (unitEntityView.HandsEquipment != null && unitEntityView.HandsEquipment.InCombat)
		{
			if (offHandWeaponType == WeaponType.Shield && Game.Instance.Controllers.TimeController.GameTime - m_LastArrowLaunchedToShieldTime < 1.Seconds())
			{
	...
	{
		m_LastArrowLaunchedToShieldTime = Game.Instance.Controllers.TimeController.GameTime;
	}
