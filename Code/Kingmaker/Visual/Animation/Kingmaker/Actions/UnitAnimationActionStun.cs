...
public class UnitAnimationActionStun : UnitAnimationAction
{
	...
	private AnimationEntry GetAnimation(UnitAnimationActionHandle handle)
	{
		...
		WeaponAnimationStyle weaponAnimationStyle = ((!(unitEntityView != null)) ? WeaponAnimationStyle.MeleeMelee : (unitEntityView.HandsEquipment?.ActiveWeaponStyle ?? WeaponAnimationStyle.MeleeMelee));
		foreach (AnimationEntry animation in m_Animations)
		{
			if (animation.IsValid() && ((animationEntry == null && animation.Style == WeaponAnimationStyle.MeleeMelee) || animation.Style == weaponAnimationStyle))
			{
