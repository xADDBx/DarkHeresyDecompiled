...
public class UnitAnimationActionHandAttack : UnitAnimationAction, IUnitAnimationActionHasVariants
{
	...
	public class WeaponStyleSettings
	{
		...
		public void Validate(ValidationContext context)
		{
			...
			{
				context.AddError(ErrorLevel.Critical, "Invalid clip wrapper in variants");
			}
			if (Burst.Any((AnimationClipWrapper i) => i == null))
			{
				context.AddError(ErrorLevel.Critical, "Invalid clip wrapper in mounted variants");
			}
		}
	}
	...
	{
		WeaponStyleSettings weaponStyleSettings = m_Settings.FirstOrDefault((WeaponStyleSettings i) => i.Style == handle.WeaponStyle);
		if (weaponStyleSettings == null)
		{
			PFLog.Default.Error(this, $"No animation for weapon style '{handle.WeaponStyle}' in action '{base.name}'");
		}
