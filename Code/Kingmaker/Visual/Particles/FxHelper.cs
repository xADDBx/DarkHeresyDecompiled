...
public static class FxHelper
{
	...
	private static Transform s_DetectiveTracesRoot;
	...
	public static Transform DetectiveTracesRoot
	{
		get
		{
			if (s_DetectiveTracesRoot == null)
			{
				GameObject gameObject = new GameObject("[Detective Traces Root]");
				Game.Instance.DynamicRoot?.Add(gameObject.transform);
				s_DetectiveTracesRoot = gameObject.transform;
			}
			return s_DetectiveTracesRoot;
		}
	}
	...
	public static GameObject SpawnFxOnGameObject(GameObject prefab, GameObject target, float snapToLocatorRaceScale = 1f, bool enableFxObject = true, Quaternion? overriddenRotationSource = null)
	{
		try
		{
			...
			if (component != null)
			{
				...
				{
					item2.Attach(component);
				}
				ListPool<SnapToLocator>.Release(list2);
			}
			List<FXCollisionRingCaster> list3 = ListPool<FXCollisionRingCaster>.Claim();
			gameObject.GetComponentsInChildren(list3);
			foreach (FXCollisionRingCaster item3 in list3)
			{
				item3.CreateAndAssignTexture();
			}
			ListPool<FXCollisionRingCaster>.Release(list3);
			ApplyMaterialAnimations(target.gameObject, gameObject);
			CheckDecals(gameObject);
			CheckBillboards(gameObject);
			List<HighlightAnimation> list4 = ListPool<HighlightAnimation>.Claim();
			gameObject.GetComponentsInChildren(list4);
			if (list4.Count > 0)
			{
				UnitMultiHighlight component2 = target.GetComponent<UnitMultiHighlight>();
				if ((bool)component2)
				{
					foreach (HighlightAnimation item4 in list4)
					{
						item4.Play(component2);
					}
				}
				else
				{
			...
			ListPool<HighlightAnimation>.Release(list4);
			...
			if ((bool)componentNonAlloc)
			{
				...
				if (!target.TryGetComponent<FxAttachmentSlot>(out var component3))
				{
					component3 = target.AddComponent<FxAttachmentSlot>();
					component3.hideFlags |= HideFlags.DontSave;
				}
				if (!gameObject.TryGetComponent<FxAttachment>(out var component4))
				{
					component4 = gameObject.AddComponent<FxAttachment>();
					component4.hideFlags |= HideFlags.DontSave;
				}
				component4.Attach(component3);
			}
			List<IFxSpawner> list5 = ListPool<IFxSpawner>.Claim();
			gameObject.GetComponentsInChildren(list5);
			list5.Sort(SortByPriority);
			foreach (IFxSpawner item5 in list5)
			{
				item5.SpawnFxOnGameObject(target);
			}
			ListPool<IFxSpawner>.Release(list5);
			List<BleedingEffectSetup> list6 = ListPool<BleedingEffectSetup>.Claim();
			gameObject.GetComponentsInChildren(list6);
			if (list6.Count > 0)
			{
				...
				if (componentNonAlloc5 != null && componentNonAlloc5.EntityData != null)
				{
					...
					GameObject bleeding = ConfigRoot.Instance.HitSystemRoot.GetBleeding(surfaceType);
					if (bleeding != null)
					{
						ListPool<BleedingEffectSetup>.Release(list6);
	...
	public static void Resnap(GameObject snapOwner, GameObject target, int snapperIndex)
	{
		...
		if (component != null)
		{
			...
			list.First((SnapToLocator s) => s.IndexForExtraSnapping == snapperIndex).Attach(component);
