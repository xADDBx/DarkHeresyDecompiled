...
public class LowResParticlesRenderer : MonoBehaviour
{
	...
	private void OnDisable()
	{
		...
		{
			Game.Instance.GetCamera().cullingMask |= CullingMask;
		}
	}
	...
	private Vector2 GetRTSize()
	{
		...
		if (Game.Instance.GetCamera() != null && Game.Instance.GetCamera().targetTexture != null)
		{
			result = new Vector2((float)Game.Instance.GetCamera().targetTexture.width * num, (float)Game.Instance.GetCamera().targetTexture.height * num);
		}
		return result;
	}
	...
	{
		if (!(cam != Game.Instance.GetCamera()))
		{
