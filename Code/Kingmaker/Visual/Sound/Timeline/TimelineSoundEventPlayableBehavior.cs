...
public class TimelineSoundEventPlayableBehavior : PlayableBehaviour
{
	...
	private void TriggerFadeIn(Playable playable)
	{
		...
		if (num > 0.0)
		{
			...
			AKRESULT result = AkUnitySoundEngine.ExecuteActionOnEvent(EventName, AkActionOnEventType.AkActionOnEventType_Pause, eventObject, 0, blendOutCurve);
			Verify(result);
			result = AkUnitySoundEngine.ExecuteActionOnEvent(EventName, AkActionOnEventType.AkActionOnEventType_Resume, eventObject, (int)(num * 1000.0), blendInCurve);
			Verify(result);
		}
	}
	...
	private void TriggerFadeOut(Playable playable)
	{
		...
		AKRESULT result = AkUnitySoundEngine.ExecuteActionOnEvent(EventName, AkActionOnEventType.AkActionOnEventType_Stop, eventObject, (int)(num * 1000.0), blendOutCurve);
		Verify(result);
	}
	...
	private void StopEvent(int transition = 0)
	{
		if (m_EventIsPlaying)
		{
			AKRESULT result = AkUnitySoundEngine.ExecuteActionOnEvent(EventName, AkActionOnEventType.AkActionOnEventType_Stop, eventObject, transition, AkCurveInterpolation.AkCurveInterpolation_Linear);
			Verify(result);
		}
	}
	...
	private float SeekToTime(Playable playable)
	{
		...
		if (m_EventIsPlaying)
		{
			AkUnitySoundEngine.SeekOnEvent(EventName, eventObject, proportionalTime);
		}
