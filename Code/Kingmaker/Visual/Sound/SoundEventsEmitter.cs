	...
	public class Info : RegisteredObjectBase
	{
		...
		private async void PostEvent(SoundFXSettings slot, CancellationToken ct)
		{
			try
			{
				...
				if (num != 0)
				{
					AkUnitySoundEngine.SetRTPCValueByPlayingID("SpellGain", slot.Gain, num);
					AkUnitySoundEngine.SetRTPCValueByPlayingID("SpellPitch", slot.Pitch, num);
		...
		private static void StopEvent(uint eventId)
		{
			try
			{
				AkUnitySoundEngine.StopPlayingID(eventId, 100);
			}
			catch (Exception ex)
			{
