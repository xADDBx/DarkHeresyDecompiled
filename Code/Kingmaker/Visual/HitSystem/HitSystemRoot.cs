...
public class HitSystemRoot : BlueprintScriptableObject
{
	...
	[Header("Cover Hit Highlight")]
	public Color CoverHitOutlineColor = Color.white;

	public float CoverHitHighlightTime = 0.25f;
	...
	private void Initialize()
	{
		...
		{
			m_Initialized = true;
			m_CachedDamageTypes = new HitCollection[EnumUtils.GetMaxValuePlusOne<DamageType>() + 1];
			m_CachedDamageToSoundSwitch = new AkSwitchReference[EnumUtils.GetMaxValuePlusOne<DamageType>() + 1];
			m_CachedBillboardBloodTypes = new HitCollection[EnumUtils.GetMaxValuePlusOne<SurfaceType>() + 1];
			m_CachedDirectionalBloodTypes = new HitCollection[EnumUtils.GetMaxValuePlusOne<SurfaceType>() + 1];
			m_CachedBillboardAdditiveBloodTypes = new HitCollection[EnumUtils.GetMaxValuePlusOne<SurfaceType>() + 1];
			m_CachedBloodPrefabsFromWeaponAnimationStyleEntries = new BloodPrefabsFromWeaponAnimationStyleEntry[EnumUtils.GetMaxValuePlusOne<WeaponAnimationStyle>() + 1];
	...
	[CanBeNull]
	public HitCollection GetDamage(DamageType damage)
	{
		Initialize();
		return m_CachedDamageTypes[(int)damage];
	}
	...
	[CanBeNull]
	public GameObject GetVitalHitFX(SurfaceType type)
	{
		Initialize();
		if (m_CachedHitEntryBySurfaceType.TryGetValue(type, out var value))
		{
			return value?.CreaturesHitEffect?.VitalFXLink?.Load();
		}
		return null;
	}
