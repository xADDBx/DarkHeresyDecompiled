...
{
	private ReactiveProperty<bool> m_CanClickConfirm = new ReactiveProperty<bool>(value: false);

	private ReactiveProperty<bool> m_CanLongClickConfirm = new ReactiveProperty<bool>(value: false);

	private ReactiveProperty<bool> m_CanClickDecline = new ReactiveProperty<bool>(value: false);

	private ReactiveProperty<bool> m_CanLongClickDecline = new ReactiveProperty<bool>(value: false);

	private ReactiveProperty<bool> m_CanClickFunc01 = new ReactiveProperty<bool>(value: false);

	private ReactiveProperty<bool> m_CanLongClickFunc01 = new ReactiveProperty<bool>(value: false);

	private ReactiveProperty<bool> m_CanClickFunc02 = new ReactiveProperty<bool>(value: false);

	private ReactiveProperty<bool> m_CanLongClickFunc02 = new ReactiveProperty<bool>(value: false);
	...
	private void SetClicksInInputLayer(InputLayer il, ReadOnlyReactiveProperty<bool> enabled, InputBindStruct ibs, List<NavigationInputEventTypeConfig> inputConfig = null)
	{
		ibs.Binds.AddRange(il.AddButton(OnConfirmClick, 8, enabled.And(m_CanClickConfirm).ToReadOnlyReactiveProperty(initialValue: false), NavigationInputEventTypeHelper.GetEventType(inputConfig, 8)).Binds);
		ibs.Binds.AddRange(il.AddButton(OnLongConfirmClick, 8, enabled.And(m_CanLongClickConfirm).ToReadOnlyReactiveProperty(initialValue: false), InputActionEventType.ButtonJustLongPressed).Binds);
		ibs.Binds.AddRange(il.AddButton(OnDeclineClick, 9, enabled.And(m_CanClickDecline).ToReadOnlyReactiveProperty(initialValue: false), NavigationInputEventTypeHelper.GetEventType(inputConfig, 9)).Binds);
		ibs.Binds.AddRange(il.AddButton(OnLongDeclineClick, 9, enabled.And(m_CanLongClickDecline).ToReadOnlyReactiveProperty(initialValue: false), InputActionEventType.ButtonJustLongPressed).Binds);
		ibs.Binds.AddRange(il.AddButton(OnFunc01Click, 10, enabled.And(m_CanClickFunc01).ToReadOnlyReactiveProperty(initialValue: false), NavigationInputEventTypeHelper.GetEventType(inputConfig, 10)).Binds);
		ibs.Binds.AddRange(il.AddButton(OnLongFunc01Click, 10, enabled.And(m_CanLongClickFunc01).ToReadOnlyReactiveProperty(initialValue: false), InputActionEventType.ButtonJustLongPressed).Binds);
		ibs.Binds.AddRange(il.AddButton(OnFunc02Click, 11, enabled.And(m_CanClickFunc02).ToReadOnlyReactiveProperty(initialValue: false), NavigationInputEventTypeHelper.GetEventType(inputConfig, 11)).Binds);
		ibs.Binds.AddRange(il.AddButton(OnLongFunc02Click, 11, enabled.And(m_CanLongClickFunc02).ToReadOnlyReactiveProperty(initialValue: false), InputActionEventType.ButtonJustLongPressed).Binds);
	}

	public Observable<Unit> OnClickAsObservable()
	{
		return Observable.FromEvent((Func<Action, UnityAction>)((Action h) => h.Invoke), (Action<UnityAction>)m_ClickEvent.AddListener, (Action<UnityAction>)m_ClickEvent.RemoveListener, default(CancellationToken));
	}
	...
	protected ConsoleNavigationBehaviour([CanBeNull] IConsoleNavigationOwner owner = null, [CanBeNull] IConsoleNavigationScroll scroll = null, bool playSoundOnSelect = false)
	{
		...
		InputBehavior inputBehavior = ReInput.players.GetPlayer(0).controllers.maps.InputBehaviors.FirstOrDefault();
	...
	public InputLayer GetInputLayer(InputLayer il = null, ReadOnlyReactiveProperty<bool> enabled = null, bool leftStick = true, bool rightStick = false, InputBindStruct ibs = null, List<NavigationInputEventTypeConfig> inputConfig = null)
	{
	...
	private void SetActive(bool active)
	{
		...
		if (active)
		{
			m_UpdateSubscription = Observable.EveryUpdate(UnityFrameProvider.PreLateUpdate).Subscribe(Tick);
	...
	private void RecursiveSubscribeToPointerDownEvent(IEnumerable<IConsoleEntity> entities)
	{
		foreach (IConsoleEntity entity in entities)
		{
			...
			else if (entity is IConsoleEntityReactiveProxy consoleEntityReactiveProxy)
			{
				consoleEntityReactiveProxy.Subscribe(delegate(IConsoleEntity e)
				{
					SubscribeToPointerLeftClick(e);
				});
			}
			else
			{
	...
	private void SubscribeToPointerLeftClick(IConsoleEntity entity)
	{
		...
		{
			m_PointerSubscriptions.Add(ObservableSubscribeExtensions.Subscribe(consolePointerLeftClickEvent.PointerLeftClickCommand, delegate
			{
	...
	{
		m_Commands.Clear();
		m_Commands.Add(new Command("keyboard:leftArrow", (Action)delegate
		{
			OnNavigate(Vector2.left);
		}));
		m_Commands.Add(new Command("keyboard:rightArrow", (Action)delegate
		{
			OnNavigate(Vector2.right);
		}));
		m_Commands.Add(new Command("keyboard:upArrow", (Action)delegate
		{
			OnNavigate(Vector2.up);
		}));
		m_Commands.Add(new Command("keyboard:downArrow", (Action)delegate
		{
