...
public class InputLayer
{
	private struct BindUpdateInfo
	{
		...
		public ReadOnlyReactiveProperty<bool> LayerBinded;
	}
	...
	public ReactiveProperty<bool> LayerBinded = new ReactiveProperty<bool>();
	...
	public InputBindStruct AddButton(Action<InputActionEventData> handler, int actionId, ReadOnlyReactiveProperty<bool> enabled, InputActionEventType eventType = InputActionEventType.ButtonJustPressed)
	{
	...
	public InputBindStruct AddButton(Action<InputActionEventData> handler, int[] actionIds, ReadOnlyReactiveProperty<bool> enabled, InputActionEventType eventType = InputActionEventType.ButtonJustPressed, bool avoidInputBlock = false)
	{
	...
	private InputBindStruct AddLongPressButton(Action<InputActionEventData> handler, int actionId, ReadOnlyReactiveProperty<bool> enabled, InputActionEventType eventType, bool avoidInputBlock = false)
	{
		if (GamePad.Instance.SwapButtonsForJapanese)
		{
			if (actionId == 8)
	...
	public InputBindStruct AddAxis(Action<InputActionEventData, float> handler, int actionId, ReadOnlyReactiveProperty<bool> enabled, bool repeat = false)
	{
	...
	public InputBindStruct AddAxis2D(Action<InputActionEventData, Vector2> handler, int actionIdX, int actionIdY, ReadOnlyReactiveProperty<bool> enabled = null, bool repeat = false)
	{
		...
		if (repeat)
		{
			...
			a.Chunk(a.DebounceFrame(1)).Subscribe(delegate(InputActionEventData[] list)
			{
	...
	public InputBindStruct AddCursor(Action<InputActionEventData, Vector2> handler, int actionIdX, int actionIdY, ReadOnlyReactiveProperty<bool> enabled = null)
	{
		Action<InputActionEventData> handler2 = delegate(InputActionEventData eventData)
		{
			handler(eventData, eventData.player.GetAxis2D(actionIdX, actionIdY));
	...
	public InputBindStruct AddCursorWithTriggerZone(Action<InputActionEventData, Vector2> handler, int actionIdX, int actionIdY, float triggerZone, ReadOnlyReactiveProperty<bool> enabled = null)
	{
	...
	private void UpdateBinds()
	{
		...
		{
			if (!update.Bind.Enabled.CurrentValue || !update.LayerBinded.CurrentValue)
			{
		...
		{
			if (update2.Bind.Enabled.CurrentValue && update2.LayerBinded.CurrentValue)
			{
	...
	private void QueueBind(BindDescription bind, ReadOnlyReactiveProperty<bool> layerBinded)
	{
		...
		{
			m_UpdateCorutine = Observable.TimerFrame(1).Subscribe(UpdateBinds);
		}
	}
	...
	private BindDescription GetBindDescription(int actionId, InputActionEventType eventType, Action<InputActionEventData> handler, ReadOnlyReactiveProperty<bool> enabled, string group, int bindId, bool avoidInputBlock = false)
	{
