...
{
	protected Dictionary<int, SortedList<ulong, (RTHandle resource, int frameIndex)>> m_ResourcePool = new Dictionary<int, SortedList<ulong, (RTHandle, int)>>();

	protected List<ulong> m_RemoveList = new List<ulong>(32);
	...
	internal bool AddResourceToPool(in TextureDesc texDesc, RTHandle resource, int currentFrameIndex)
	{
		...
		if (!m_ResourcePool.TryGetValue(hashCodeWithNameHash, out SortedList<ulong, (RTHandle, int)> value))
		{
			value = new SortedList<ulong, (RTHandle, int)>(s_StaleResourceMaxCapacity);
			m_ResourcePool.Add(hashCodeWithNameHash, value);
		}
		value.Add(GetUniqueID(resource), (resource, currentFrameIndex));
		s_CurrentStaleResourceCount++;
		return true;
	}

	private static ulong GetUniqueID(RTHandle rtHandle)
	{
		if (rtHandle.rt != null)
		{
			return (ulong)(int)rtHandle.rt.GetEntityId();
		}
		if (rtHandle.externalTexture != null)
		{
			return (ulong)(int)rtHandle.externalTexture.GetEntityId();
		}
		return (ulong)rtHandle.name.GetHashCode();
	}

	internal bool TryGetResource(in TextureDesc texDesc, out RTHandle resource, bool usepool = true)
	{
		int hashCodeWithNameHash = GetHashCodeWithNameHash(in texDesc);
		if (usepool && m_ResourcePool.TryGetValue(hashCodeWithNameHash, out SortedList<ulong, (RTHandle, int)> value) && value.Count > 0)
		{
	...
	{
		foreach (KeyValuePair<int, SortedList<ulong, (RTHandle, int)>> item in m_ResourcePool)
		{
			foreach (KeyValuePair<ulong, (RTHandle, int)> item2 in item.Value)
			{
	...
	internal void PurgeUnusedResources(int currentFrameIndex)
	{
		...
		foreach (KeyValuePair<int, SortedList<ulong, (RTHandle, int)>> item in m_ResourcePool)
		{
			SortedList<ulong, (RTHandle, int)> value = item.Value;
			IList<ulong> keys = value.Keys;
			...
			foreach (ulong remove in m_RemoveList)
			{
				value.Remove(remove);
			}
		}
	...
	{
		GraphicsFormat format = ((desc.depthStencilFormat != 0) ? desc.depthStencilFormat : desc.graphicsFormat);
