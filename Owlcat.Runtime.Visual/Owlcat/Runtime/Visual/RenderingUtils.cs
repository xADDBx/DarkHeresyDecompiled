...
public static class RenderingUtils
{
	...
	private static Dictionary<GraphicsFormat, Dictionary<GraphicsFormatUsage, bool>> m_GraphicsFormatSupport = new Dictionary<GraphicsFormat, Dictionary<GraphicsFormatUsage, bool>>();
	...
	internal static void RenderObjectsWithError(ScriptableRenderContext context, CommandBuffer cmd, ref CullingResults cullResults, Camera camera, FilteringSettings filterSettings, SortingCriteria sortFlags)
	{
		...
		{
			drawSettings.SetShaderPassName(i, s_LegacyShaderPassNames[i]);
		}
		RendererListParams param = new RendererListParams(cullResults, drawSettings, filterSettings);
	...
	public static bool SupportsGraphicsFormat(GraphicsFormat format, GraphicsFormatUsage usage)
	{
		bool value = false;
		if (!m_GraphicsFormatSupport.TryGetValue(format, out var value2))
		{
			value2 = new Dictionary<GraphicsFormatUsage, bool>();
	...
	public static Vector4 CalculateScaleBiasRt(WaaaghCameraData cameraData)
	{
		float num = (cameraData.IsCameraProjectionMatrixFlipped() ? (-1f) : 1f);
		if (!(num < 0f))
		{
			return new Vector4(num, 0f, 1f, 1f);
		}
		return new Vector4(num, 1f, -1f, 1f);
	}

	public static Vector4 GetFinalBlitScaleBias(ref TextureHandle renderTarget, WaaaghCameraData cameraData)
	{
		Vector2 vector = new Vector2(1f, 1f);
		if (cameraData.IsRenderTargetProjectionMatrixFlipped(renderTarget))
		{
	...
	internal static Vector4 GetFinalBlitScaleBias(RTHandle source, RTHandle destination, WaaaghCameraData cameraData)
	{
		...
		if (cameraData.IsRenderTargetProjectionMatrixFlipped(destination))
		{
			return new Vector4(vector.x, vector.y, 0f, 0f);
		}
		return new Vector4(vector.x, 0f - vector.y, 0f, vector.y);
	...
	public static void EnsureGraphicsBufferAllocated(ref GraphicsBuffer buffer, GraphicsBuffer.Target target, int minCount, int stride, string debugName)
	{
		if (buffer != null && buffer.IsValid())
		{
			if (buffer.count >= minCount)
			{
				return;
			}
			buffer.Release();
