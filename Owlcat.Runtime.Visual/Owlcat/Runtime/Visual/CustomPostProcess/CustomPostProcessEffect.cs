...
public sealed class CustomPostProcessEffect : ScriptableObject, IEquatable<CustomPostProcessEffect>, ISerializationCallbackReceiver
{
	...
	internal bool Has(ShaderPropertyDescriptor propertyDescriptor)
	{
		return (from pass in Passes
			from property in pass.Properties
			where property.Name == propertyDescriptor.Name && property.Type == propertyDescriptor.Type
			select property).Any();
	}
