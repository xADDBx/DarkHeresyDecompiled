...
internal class NativeShadowCasterPass : ScriptableRenderPass<NativeShadowCasterPass.PassData>
{
	...
	private static readonly ProfilerMarker s_ShadowCacheAtlasRenderMarkerFrameDebugger = new ProfilerMarker("Shadow Cache Atlas - ALWAYS DRAW (because FrameDebugger)");
	...
	private protected sealed override WaaaghProfileId? ProfileId => WaaaghProfileId.NativeShadowCasterPass;
	...
	protected override void Setup(RenderGraphBuilder builder, PassData data, ContextContainer frameData)
	{
		...
		TextureHandle input = waaaghResourceData.Shadowmap;
		data.ShadowMapAtlasTexture = builder.WriteTexture(in input);
		ShadowManager shadowManager = waaaghShadowData.ShadowManager;
		...
		data.ShadowQuality = waaaghShadowData.ShadowQuality;
		data.ShadowDistance = waaaghCameraData.maxShadowDistance;
		data.DirectionalCascadesCount = waaaghShadowData.DirectionalLightCascades.Count;
		data.ShadowManager = waaaghShadowData.ShadowManager;
		data.CacheEnabled = waaaghShadowData.StaticShadowsCacheEnabled;
		data.ReceiverNormalBias = waaaghShadowData.ReceiverNormalBias;
		data.ShadowAtlasSize = (int)waaaghShadowData.AtlasSize;
		if (data.CacheEnabled)
		{
			input = waaaghResourceData.CachedShadowmap;
	...
	protected override void Render(PassData data, RenderGraphContext context)
	{
		...
		if (data.CacheEnabled)
		{
			...
			{
				_ = s_ShadowCacheAtlasRenderMarker;
				if (FrameDebugger.enabled)
				{
					_ = s_ShadowCacheAtlasRenderMarkerFrameDebugger;
				}
