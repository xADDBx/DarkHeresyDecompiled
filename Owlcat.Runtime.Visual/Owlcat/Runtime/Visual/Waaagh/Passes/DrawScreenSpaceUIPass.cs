...
public class DrawScreenSpaceUIPass : ScriptableRenderPass
{
	...
	public override void RecordRenderGraph(ContextContainer frameData)
	{
		...
		renderGraph.BeginProfilingSampler(base.ProfilingSampler, ".\\Library\\PackageCache\\com.owlcat.visual@f3d4bf622f68\\Runtime\\Waaagh\\Passes\\DrawScreenSpaceUIPass.cs", 49);
		if ((m_Subset & UISubset.UIToolkit_UGUI) != 0)
		{
			PassData passData2;
			using IRasterRenderGraphBuilder rasterRenderGraphBuilder = renderGraph.AddRasterRenderPass<PassData>("Draw UGUI Overlay", out passData2, ".\\Library\\PackageCache\\com.owlcat.visual@f3d4bf622f68\\Runtime\\Waaagh\\Passes\\DrawScreenSpaceUIPass.cs", 54);
		...
		if ((m_Subset & UISubset.LowLevel) != 0)
		{
			...
			using IUnsafeRenderGraphBuilder unsafeRenderGraphBuilder = renderGraph.AddUnsafePass<UnsafePassData>("Draw IMGUI Overlay", out passData4, ".\\Library\\PackageCache\\com.owlcat.visual@f3d4bf622f68\\Runtime\\Waaagh\\Passes\\DrawScreenSpaceUIPass.cs", 75);
		...
		renderGraph.EndProfilingSampler(base.ProfilingSampler, ".\\Library\\PackageCache\\com.owlcat.visual@f3d4bf622f68\\Runtime\\Waaagh\\Passes\\DrawScreenSpaceUIPass.cs", 91);
	}
}
