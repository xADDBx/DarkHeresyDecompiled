...
public class SetCameraShaderVariablesPass : ScriptableRenderPass<SetCameraShaderVariablesPassData>
{
	...
	protected override void Setup(RenderGraphBuilder builder, SetCameraShaderVariablesPassData data, ContextContainer frameData)
	{
		WaaaghCameraData waaaghCameraData = frameData.Get<WaaaghCameraData>();
		Camera camera = waaaghCameraData.camera;
		float num = waaaghCameraData.cameraTargetDescriptor.width;
		float num2 = waaaghCameraData.cameraTargetDescriptor.height;
		...
		data.CameraRenderType = waaaghCameraData.renderType;
		...
		data.OrthoParams = new Vector4(camera.orthographicSize * waaaghCameraData.aspectRatio, camera.orthographicSize, 0f, w);
		data.WorldSpaceCameraPos = waaaghCameraData.worldSpaceCameraPos;
		...
		float num9 = ((waaaghCameraData.CameraRenderTargetBufferType == CameraRenderTargetType.Scaled) ? Math.Min((float)(0.0 - Math.Log(1f / waaaghCameraData.renderScale, 2.0)), 0f) : 0f);
