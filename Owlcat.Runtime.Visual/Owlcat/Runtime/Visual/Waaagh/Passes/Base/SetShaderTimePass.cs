...
public class SetShaderTimePass : ScriptableRenderPass<SetShaderTimePassData>
{
	...
	protected override void Setup(RenderGraphBuilder builder, SetShaderTimePassData data, ContextContainer frameData)
	{
		WaaaghRenderingData waaaghRenderingData = frameData.Get<WaaaghRenderingData>();
		float time = waaaghRenderingData.TimeData.Time;
		float deltaTime = waaaghRenderingData.TimeData.DeltaTime;
		float smoothDeltaTime = waaaghRenderingData.TimeData.SmoothDeltaTime;
		float unscaledTime = waaaghRenderingData.TimeData.UnscaledTime;
		float f = time / 8f;
		float f2 = time / 4f;
		float f3 = time / 2f;
