...
public class SetupLightDataPass : ScriptableRenderPass<SetupLightDataPassData>
{
	...
	protected override void Setup(RenderGraphBuilder builder, SetupLightDataPassData data, ContextContainer frameData)
	{
		WaaaghResourceData waaaghResourceData = frameData.Get<WaaaghResourceData>();
		data.LightDataConstantBufferHandle = builder.WriteBuffer(in waaaghResourceData.LightDataConstantBuffer);
		data.LightVolumeDataConstantBufferHandle = builder.WriteBuffer(in waaaghResourceData.LightVolumeDataConstantBuffer);
		data.ZBinsConstantBufferHandle = builder.WriteBuffer(in waaaghResourceData.ZBinsConstantBuffer);
