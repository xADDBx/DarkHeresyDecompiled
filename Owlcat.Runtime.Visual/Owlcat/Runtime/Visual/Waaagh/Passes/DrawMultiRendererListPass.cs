...
public abstract class DrawMultiRendererListPass<T> : ScriptableRenderPass where T : DrawMultiRendererListPassData, new()
{
	...
	public override void RecordRenderGraph(ContextContainer frameData)
	{
		...
		T passData;
		using RenderGraphBuilder builder = waaaghRenderingData.RenderGraph.AddRenderPass<T>(Name, out passData, ProfilingSamplerStorage<WaaaghProfileId>.Get(Name, ProfileId), ".\\Library\\PackageCache\\com.owlcat.visual@f3d4bf622f68\\Runtime\\Waaagh\\Passes\\DrawRendererListPass.cs", 108);
