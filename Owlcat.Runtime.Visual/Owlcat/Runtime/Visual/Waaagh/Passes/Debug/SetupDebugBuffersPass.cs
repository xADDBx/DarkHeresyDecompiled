...
public class SetupDebugBuffersPass : ScriptableRenderPass<SetupDebugBuffersPass.PassData>
{
	...
	private const int kClearCellsPerThread = 1024;
		...
		: base(RenderPassEvent.BeforeRendering)
	{
		...
		m_ClearDebugBuffersShader = debugData.Resources.ClearDebugBuffersCS;
	...
	protected override void Setup(RenderGraphBuilder builder, PassData data, ContextContainer frameData)
	{
		...
		int2 fullScreenDebugBufferDimensions = new int2(waaaghCameraData.scaledWidth, waaaghCameraData.scaledHeight);
		...
		m_Resources.FullScreenDebugBuffer = waaaghRenderingData.RenderGraph.CreateBuffer(in desc);
		...
		data.ClearDebugBuffersThreadGroupsX = Mathf.CeilToInt((float)num / (float)(m_ClearDebugBuffersShaderMainKernelThreadGroupSizeX * 1024));
