...
public class FullscreenDebugPass : ScriptableRenderPass<FullscreenDebugPassData>
{
	...
	protected override void Setup(RenderGraphBuilder builder, FullscreenDebugPassData data, ContextContainer frameData)
	{
		if (!(m_DebugData == null) && !(m_Material == null))
		{
			...
			if (m_DebugData.LightingDebug.DebugClustersMode == DebugClustersMode.DeferredLightingComplexity)
			{
				...
				data.CameraDepthRT = builder.ReadTexture(in input);
			}
			if ((m_DebugData.LightingDebug.DebugClustersMode == DebugClustersMode.DeferredFeatureVariants || m_DebugData.LightingDebug.DebugClustersMode == DebugClustersMode.DeferredFeatureTileCoherency) && waaaghResourceData.DeferredLightingFeatureTilesBuffer.IsValid())
			{
				data.FeatureTilesBuffer = builder.ReadBuffer(in waaaghResourceData.DeferredLightingFeatureTilesBuffer);
			}
			if (m_DebugData.StencilDebug.StencilDebugType != 0)
			{
				...
				data.CameraDepthRT = builder.WriteTexture(in input);
				TextureDesc textureDesc = new TextureDesc(waaaghCameraData.cameraTargetDescriptor.width, waaaghCameraData.cameraTargetDescriptor.height);
	...
	protected override void Render(FullscreenDebugPassData data, RenderGraphContext context)
	{
		...
		case DebugClustersMode.DeferredFeatureVariants:
			if (data.FeatureTilesBuffer.IsValid())
			{
				context.cmd.SetGlobalBuffer("_FeatureTiles", data.FeatureTilesBuffer);
				context.cmd.DrawProcedural(Matrix4x4.identity, data.Material, data.Material.FindPass("DEFERRED_FEATURE_VARIANTS"), MeshTopology.Triangles, 3);
			}
			break;
		case DebugClustersMode.DeferredFeatureTileCoherency:
			...
			{
				context.cmd.SetGlobalBuffer("_FeatureTiles", data.FeatureTilesBuffer);
				context.cmd.DrawProcedural(Matrix4x4.identity, data.Material, data.Material.FindPass("DEFERRED_FEATURE_TILE_COHERENCY"), MeshTopology.Triangles, 3);
			}
			break;
		}
