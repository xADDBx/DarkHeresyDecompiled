...
public class ComputeTilesMinMaxZPass : ScriptableRenderPass<ComputeTilesMinMaxZPassData>
{
	...
	protected override void Setup(RenderGraphBuilder builder, ComputeTilesMinMaxZPassData data, ContextContainer frameData)
	{
		...
		data.CameraDepthCopyTexture = builder.ReadTexture(in waaaghResourceData.CameraDepthCopyRT);
		...
		waaaghResourceData.TilesMinMaxZTexture = renderGraph.CreateTexture(in desc);
		data.TilesMinMaxZTexture = builder.WriteTexture(in waaaghResourceData.TilesMinMaxZTexture);
