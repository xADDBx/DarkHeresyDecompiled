...
public class FullscreenBlurPass : ScriptableRenderPass<FullscreenBlurPassData>
{
	...
	protected override void Setup(RenderGraphBuilder builder, FullscreenBlurPassData data, ContextContainer frameData)
	{
		...
		data.CameraColorRT = builder.ReadWriteTexture(in input);
		float num = 1f / (float)m_Feature.Downsample;
		TextureDesc desc = RenderingUtils.CreateTextureDesc("BlurRT0", waaaghCameraData.cameraTargetDescriptor);
