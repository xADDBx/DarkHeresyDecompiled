...
public class FogOfWarSetupPass : ScriptableRenderPass<FogOfWarSetupPassData>
{
	...
	protected override void Setup(RenderGraphBuilder builder, FogOfWarSetupPassData data, ContextContainer frameData)
	{
		data.Area = m_Area;
	...
	protected override void Render(FogOfWarSetupPassData data, RenderGraphContext context)
	{
		if (data.Area == null || !data.Area.isActiveAndEnabled)
		{
			...
			context.cmd.SetGlobalTexture(FogOfWarConstantBuffer._FogOfWarMask, data.DefaultFogOfWarMask);
			return;
		}
