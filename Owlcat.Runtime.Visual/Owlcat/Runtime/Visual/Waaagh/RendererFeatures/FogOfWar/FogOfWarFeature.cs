...
{
	private FogOfWarFeatureResources m_Resources;
	...
	public override void AddRenderPasses(ScriptableRenderer renderer, ContextContainer frameData)
	{
		...
		CameraType cameraType = waaaghCameraData.camera.cameraType;
	...
	{
		m_Resources = GraphicsSettings.GetRenderPipelineSettings<FogOfWarFeatureResources>();
		if (m_Settings != null)
		{
			m_Settings.InitSingleton(m_Settings);
		}
		m_FowMat = CoreUtils.CreateEngineMaterial(m_Resources.FogOfWarShader);
		m_ScreenSpaceMat = CoreUtils.CreateEngineMaterial(m_Resources.ScreenSpaceFogOfWarShader);
		m_BlurMat = CoreUtils.CreateEngineMaterial(m_Resources.BlurShader);
		m_BlitMat = CoreUtils.CreateEngineMaterial(m_Resources.BlitShader);
		...
		m_PostProcessPass = new FogOfWarPostProcessPass((RenderPassEvent)402, m_ScreenSpaceMat);
		m_LastFrameId = -1;
	...
	protected override void Dispose(bool disposing)
	{
		...
		if (m_DefaultFogOfWarMask != null)
		{
			Object.DestroyImmediate(m_DefaultFogOfWarMask);
