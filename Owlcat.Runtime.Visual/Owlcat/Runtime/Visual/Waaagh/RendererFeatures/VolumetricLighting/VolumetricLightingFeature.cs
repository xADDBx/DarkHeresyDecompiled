...
public class VolumetricLightingFeature : ScriptableRendererFeature
{
	...
	public override void AddRenderPasses(ScriptableRenderer renderer, ContextContainer frameData)
	{
		...
		if (!waaaghCameraData.IsLightingEnabled || !(renderer is WaaaghRenderer waaaghRenderer))
		{
			return;
		}
	...
	internal override void StartSetupJobs(ContextContainer frameData)
	{
		...
		if (waaaghCameraData.IsLightingEnabled)
		{
			...
			Matrix4x4 viewProjMatrix = CoreMatrixUtils.MultiplyProjectionMatrix(waaaghCameraData.GetGPUProjectionMatrix(), viewMatrix, camera.orthographic);
			...
			for (int i = 0; i < num; i++)
			{
				...
				localVolumetricFog.PrepareParameters(waaaghRenderingData.TimeData.Time);
			...
			if (waaaghCameraData.renderer is WaaaghRenderer waaaghRenderer)
			{
				tileSize = waaaghRenderer.Settings.TileSize;
			}
			InitFogTilesBuffer(waaaghCameraData, tileSize);
	...
	private void InitFogTilesBuffer(WaaaghCameraData cameraData, TileSize tileSize)
	{
