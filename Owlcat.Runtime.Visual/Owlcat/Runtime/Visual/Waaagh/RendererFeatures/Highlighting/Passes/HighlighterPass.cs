...
public class HighlighterPass : ScriptableRenderPass<HighlighterPassData>
{
	...
	protected override void Setup(RenderGraphBuilder builder, HighlighterPassData data, ContextContainer frameData)
	{
		...
		TextureDesc desc = RenderingUtils.CreateTextureDesc("HighlightRT", waaaghCameraData.cameraTargetDescriptor);
		...
		case HighlightingFeature.ZTestMode.SceneBuffer:
			...
			data.DepthBuffer = builder.ReadWriteTexture(in input);
			break;
		...
		case HighlightingFeature.ZTestMode.None:
			break;
		}
	}
	...
	protected override void Render(HighlighterPassData data, RenderGraphContext context)
	{
		...
		context.cmd.Blit(flag ? data.Blur1RT : data.Blur2RT, data.HighlightRT, data.CutMaterial);
