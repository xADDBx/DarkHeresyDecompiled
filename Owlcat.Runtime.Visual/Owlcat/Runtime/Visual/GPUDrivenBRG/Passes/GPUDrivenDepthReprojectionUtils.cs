...
public sealed class GPUDrivenDepthReprojectionUtils
{
	...
	[MustUseReturnValue]
	public ReprojectionParameters Setup(ContextContainer frameData, Matrix4x4 gpuViewProjection, out TextureHandle source)
	{
		GPUDrivenBatchRendererGroup gPUDrivenBatchRendererGroup = frameData.Get<WaaaghRenderingData>().GPUDrivenBatchRendererGroup;
		...
		source = waaaghRenderingData.RenderGraph.ImportBackbuffer(cullingDepthHistory.GetTexture());
		if (waaaghCameraData.camera.TryGetComponent<WaaaghAdditionalCameraData>(out var component) && !FrameDebugger.enabled)
		{
		...
		result2.DepthReprojectionBias = gPUDrivenBatchRendererGroup.Settings.DepthReprojectionBias * (SystemInfo.usesReversedZBuffer ? (-1f) : 1f);
		result2.DepthReprojectionCS = gPUDrivenBatchRendererGroup.Resources.DepthReprojectionCS;
		return result2;
	}

	public void Reproject(in ReprojectionParameters reprojectionParameters, CommandBuffer cmd, TextureHandle source, TextureHandle packedReprojectedDepth)
	{
	...
	public void UnpackReprojectedDepth(in ReprojectionParameters reprojectionParameters, CommandBuffer cmd, TextureHandle packedReprojectedDepth, TextureHandle destination, Rect destinationViewport)
	{
		using (new ProfilingScope(cmd, Profiling.UnpackReprojectedDepth))
		{
			...
			cmd.DrawProcedural(Matrix4x4.identity, m_CopyDepthMaterial, 4, MeshTopology.Triangles, 3, 1, m_PropertyBlock);
		}
	}
}
