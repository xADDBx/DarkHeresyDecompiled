	...
	public void Play()
	{
		...
		if (Profile.Layers != null && Profile.Layers.Length != 0)
		{
			if (ValidateLocationWeatherData(Profile))
			{
				...
				m_DirectionalLightControllers = ((Profile.DirectionalLightSettings == null) ? null : (from light in UnityEngine.Object.FindObjectsByType<Light>(FindObjectsSortMode.None)?.Where((Light layer) => layer != null)
