...
public class OwlcatToggle : MonoBehaviour, IConsoleEntityProxy, IConsoleEntity
{
	...
	private void OnEnable()
	{
		...
		m_Subscriptions.Add(ObservableSubscribeExtensions.Subscribe(m_MultiButton.OnLeftClickAsObservable(), delegate
		{
			HandleClick();
		}));
		m_Subscriptions.Add(ObservableSubscribeExtensions.Subscribe(m_MultiButton.OnConfirmClickAsObservable(), delegate
		{
			HandleClick();
		}));
