...
public class VirtualListElement : IConsoleEntityReactiveProxy, IConsoleEntityProxy, IConsoleEntity, IConsoleNavigationEntity, IConfirmClickHandler, ILongConfirmClickHandler, IDeclineClickHandler, ILongDeclineClickHandler, IFunc01ClickHandler, ILongFunc01ClickHandler, IFunc02ClickHandler, ILongFunc02ClickHandler
{
	public ReactiveCommand<Unit> AttachViewCommand = new ReactiveCommand<Unit>();
	...
	public IDisposable Subscribe(Action<IConsoleEntity> onNext)
	{
		return AttachViewCommand.Select((Unit _) => ConsoleEntityProxy).Prepend(ConsoleEntityProxy).Subscribe(onNext);
	}
	...
	{
		m_SubscriptionOnData = ObservableSubscribeExtensions.Subscribe(m_Data.ContentChanged, delegate
		{
	...
	public bool IsValid()
	{
		return ConsoleEntityProxy?.IsValid() ?? m_Data.IsAvailable.CurrentValue;
	}
