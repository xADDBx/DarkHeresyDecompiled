...
public class ConsoleHint : MonoBehaviour, IDisposable
{
	...
	public IDisposable Bind(InputBindStruct inputBindStruct)
	{
		...
		ReadOnlyReactiveProperty<bool> property = null;
		foreach (BindDescription bind2 in inputBindStruct.Binds)
		{
			if (bind2.Enabled != null)
			{
				property = bind2.Enabled;
				break;
			}
		}
		compositeDisposable.Add(inputLayer.LayerBinded.And(property).Subscribe(SetActive));
	...
	public IDisposable BindCustomAction(int actionId, InputLayer inputLayer, ReadOnlyReactiveProperty<bool> hintIsActive = null)
	{
		return BindCustomAction(new List<int> { actionId }, inputLayer, hintIsActive);
	}

	public IDisposable BindCustomAction(List<int> actionIds, InputLayer inputLayer, ReadOnlyReactiveProperty<bool> hintIsActive = null)
	{
		PartialDispose();
		m_IsBinded = true;
		CanvasGroup.alpha = 1f;
		...
		UpdateIcon();
		CompositeDisposable compositeDisposable = new CompositeDisposable();
		compositeDisposable.Add(inputLayer.LayerBinded.And(hintIsActive).Subscribe(SetActive));
		compositeDisposable.Add(Disposable.Create(delegate
		{
			m_IsBinded = false;
			CanvasGroup.alpha = 0f;
	...
	private void PlayComplete()
	{
		if (m_PlayTween != null)
		{
			StopCoroutine(m_PlayTween);
			m_PlayTween = null;
			HintContainer.localScale = Vector3.one;
		}
	}
	...
	private void PlayScale()
	{
		...
		{
			m_PlayTween = StartCoroutine(PlayScaleCoroutine(m_ScaleFactor, m_AnimTime));
		}
	}

	private IEnumerator PlayScaleCoroutine(float amplitude, float duration)
	{
		float t = 0f;
		while (t < duration)
		{
			HintContainer.localScale = Vector3.one + Vector3.one * amplitude * Mathf.Sin(t / duration * MathF.PI);
