...
public class UnityModManager
{
	...
	public class ModSettings
	{
		...
		public static void Save<T>(T data, ModEntry modEntry, XmlAttributeOverrides attributes) where T : ModSettings, new()
		{
			...
			try
			{
				...
				new XmlSerializer(typeof(T), attributes).Serialize(textWriter, data);
			}
			catch (Exception e)
			{
	...
	private static class Textures
	{
		...
		public static void Init()
		{
			...
			{
				array2[i].SetValue(null, (object)new Texture2D(2, 2, (TextureFormat)5, false, true));
			}
			if (unityVersion.Major >= 2017)
			{
				MethodInfo method = Assembly.Load("UnityEngine.ImageConversionModule").GetType("UnityEngine.ImageConversion").GetMethod("LoadImage", new Type[2]
				{
					typeof(Texture2D),
					typeof(byte[])
				});
	...
	public class UI : MonoBehaviour
	{
		...
		private sealed class _003C_003Ec
		{
			...
			public static UnityAction _003C_003E9__65_0;

			public static Func<Column, bool> _003C_003E9__67_2;

			public static Func<Column, float> _003C_003E9__67_3;

			public static Func<Column, bool> _003C_003E9__67_4;

			public static Func<Column, float> _003C_003E9__67_5;

			public static Action<int> _003C_003E9__67_0;

			public static Action<int> _003C_003E9__67_1;

			public static Func<ModEntry, bool> _003C_003E9__79_0;

			public static Converter<object, float> _003C_003E9__117_6;

			public static Converter<object, int> _003C_003E9__117_7;

			public static Converter<object, long> _003C_003E9__117_8;

			public static Converter<object, double> _003C_003E9__117_9;
			...
			internal void _003CWindowFunction_003Eb__65_0()
			{
			}

			internal bool _003CDrawTab_003Eb__67_2(Column x)
			{
				return !x.skip;
			}

			internal float _003CDrawTab_003Eb__67_3(Column x)
			{
				return x.width;
			}

			internal bool _003CDrawTab_003Eb__67_4(Column x)
			{
				if (x.expand)
				{
					return !x.skip;
			...
			internal float _003CDrawTab_003Eb__67_5(Column x)
			{
				return x.width;
			}

			internal void _003CDrawTab_003Eb__67_0(int i)
			{
				Params.CheckUpdates = i;
			}

			internal void _003CDrawTab_003Eb__67_1(int i)
			{
				Params.ShowOnStart = i;
			}

			internal bool _003CFirstLaunch_003Eb__79_0(ModEntry x)
			{
				return !x.ErrorOnLoading;
			}

			internal float _003CDraw_003Eb__117_6(object x)
			{
				return (float)x;
			}

			internal int _003CDraw_003Eb__117_7(object x)
			{
				return (int)x;
			}

			internal long _003CDraw_003Eb__117_8(object x)
			{
				return (long)x;
			}

			internal double _003CDraw_003Eb__117_9(object x)
			{
				return (double)x;
			}
		}
		...
		private void DrawTab(int tabId, ref UnityAction buttons)
		{
			...
			GUILayoutOption[] array = mColumns.Select((Column x) => (!x.expand) ? GUILayout.Width(x.width) : GUILayout.Width(x.width / expandWidth * (mWindowSize.x - 60f + expandWidth - amountWidth))).ToArray();
			GUILayout.BeginVertical(GUIStyle.op_Implicit("box"), Array.Empty<GUILayoutOption>());
		...
		private static bool DependsOn(string str, object container, Type type, ModEntry mod)
		{
			...
			if (!str.StartsWith("#"))
			{
				...
				return field.GetValue(container).GetHashCode() == obj.GetHashCode();
			}
			...
			return property.GetValue(container, null).GetHashCode() == obj2.GetHashCode();
		}
		...
		private static bool Draw(object container, Type type, ModEntry mod, DrawFieldMask defaultMask, int unique)
		{
			...
			{
				drawFieldMask = ((DrawFieldsAttribute)customAttributes[i]).Mask;
			}
			...
			foreach (FieldInfo f in fields)
			{
				...
				if (customAttributes2.Length != 0)
				{
					...
					{
						drawAttribute = (DrawAttribute)customAttributes[j];
				...
				else
				{
					...
					customAttributes = f.GetCustomAttributes(typeof(RangeAttribute), inherit: false);
					int j = 0;
					if (j < customAttributes.Length)
					{
						RangeAttribute val = (RangeAttribute)customAttributes[j];
						drawAttribute.Type = DrawType.Slider;
						drawAttribute.Min = val.min;
						drawAttribute.Max = val.max;
					}
				}
				customAttributes = f.GetCustomAttributes(typeof(SpaceAttribute), inherit: false);
				for (int j = 0; j < customAttributes.Length; j++)
				{
					GUILayout.Space((float)Scale((int)((SpaceAttribute)customAttributes[j]).height));
				}
				customAttributes = f.GetCustomAttributes(typeof(HeaderAttribute), inherit: false);
				for (int j = 0; j < customAttributes.Length; j++)
				{
					GUILayout.Label(((HeaderAttribute)customAttributes[j]).header, bold, (GUILayoutOption[])(object)new GUILayoutOption[1] { GUILayout.ExpandWidth(false) });
				}
				...
				if (((f.FieldType.IsClass && !f.FieldType.IsArray) || (f.FieldType.IsValueType && !f.FieldType.IsPrimitive && !f.FieldType.IsEnum)) && !Array.Exists(specialTypes, (Type x) => x == f.FieldType))
				{
					...
					customAttributes = f.GetCustomAttributes(typeof(DrawFieldsAttribute), inherit: false);
					for (int j = 0; j < customAttributes.Length; j++)
					{
						defaultMask = ((DrawFieldsAttribute)customAttributes[j]).Mask;
					}
					...
					if (flag3)
					{
						...
						if (typeof(Object).IsAssignableFrom(f.FieldType))
						{
							...
							if (val2 != null)
							{
								GUILayout.Label(val2.name, (GUILayoutOption[])(object)new GUILayoutOption[1] { GUILayout.ExpandWidth(false) });
								goto IL_0595;
							}
						}
				...
				if (drawAttribute.Type == DrawType.Field)
				{
					...
					if (f.FieldType.IsArray)
					{
						...
						{
							array = source.Cast<object>().ToArray();
							type2 = value2.GetType().GetElementType();
						}
					}
					else
					{
						array = new object[1] { value2 };
					...
					if (f.FieldType.IsArray)
					{
						...
						{
							Array.Resize(ref array, Math.Min(array.Length + 1, int.MaxValue));
							array[array.Length - 1] = Convert.ChangeType("0", type2);
							flag4 = true;
							result = true;
						}
						if (GUILayout.Button("-", (GUILayoutOption[])(object)new GUILayoutOption[1] { GUILayout.ExpandWidth(false) }))
						{
							Array.Resize(ref array, Math.Max(array.Length - 1, 0));
							if (array.Length != 0)
							{
								array[array.Length - 1] = Convert.ChangeType("0", type2);
							}
					...
					if (array.Length != 0)
					{
						...
						for (int k = 0; k < array.Length; k++)
						{
							...
							{
								GUILayout.BeginHorizontal(Array.Empty<GUILayoutOption>());
								GUILayout.Label($"  [{k}] ", (GUILayoutOption[])(object)new GUILayoutOption[1] { GUILayout.ExpandWidth(false) });
							}
							...
							array[k] = Convert.ChangeType(text3, type2);
							result = true;
							flag4 = true;
						}
					}
					if (flag4)
					{
						if (f.FieldType.IsArray)
						{
							...
							{
								f.SetValue(container, Array.ConvertAll(array, (object x) => (float)x));
							}
							else if (type2 == typeof(int))
							{
								f.SetValue(container, Array.ConvertAll(array, (object x) => (int)x));
							}
							else if (type2 == typeof(long))
							{
								f.SetValue(container, Array.ConvertAll(array, (object x) => (long)x));
							}
							else if (type2 == typeof(double))
							{
								f.SetValue(container, Array.ConvertAll(array, (object x) => (double)x));
							}
						}
						else
						{
							f.SetValue(container, array[0]);
						}
					}
					if (drawAttribute.Vertical)
					{
				...
				else if (drawAttribute.Type == DrawType.Slider)
				{
					...
					if (!double.TryParse(f.GetValue(container).ToString(), NumberStyles.Float, NumberFormatInfo.CurrentInfo, out var result4))
					{
						result4 = 0.0;
					}
					if (drawAttribute.Vertical)
					...
					float num = (float)result4;
					float num2 = GUILayout.HorizontalSlider(num, (float)drawAttribute.Min, (float)drawAttribute.Max, list.ToArray());
					if (!drawAttribute.Vertical)
					{
						GUILayout.Space((float)Scale(5));
					}
					GUILayout.Label(num2.ToString(), (GUILayoutOption[])(object)new GUILayoutOption[2]
					{
						GUILayout.ExpandWidth(false),
					...
					if (num2 != num)
					{
						...
						{
							num2 = (float)Math.Round(num2, drawAttribute.Precision);
						}
						f.SetValue(container, Convert.ChangeType(num2, f.FieldType));
						result = true;
					}
				}
	...
	[CompilerGenerated]
	private sealed class _003CDownloadString_003Ed__53 : IEnumerator<object>, IDisposable, IEnumerator
	{
		...
		public UnityAction<string, string> handler;
		...
		[DebuggerHidden]
		public _003CDownloadString_003Ed__53(int _003C_003E1__state)
		{
		...
		private bool MoveNext()
		{
			...
			case 0:
				...
				_003C_003E2__current = _003Cwww_003E5__2.Send();
				_003C_003E1__state = 1;
				return true;
			case 1:
			{
				_003C_003E1__state = -1;
				MethodInfo methodInfo = ((ParseVersion(Application.unityVersion).Major < 2017) ? typeof(UnityWebRequest).GetMethod("get_isError") : typeof(UnityWebRequest).GetMethod("get_isNetworkError"));
				if (methodInfo == null || (bool)methodInfo.Invoke(_003Cwww_003E5__2, null))
				{
					PFLog.UnityModManager.Log(_003Cwww_003E5__2.error);
					PFLog.UnityModManager.Log($"Error downloading '{url}'.");
					return false;
				}
				handler.Invoke(_003Cwww_003E5__2.downloadHandler.text, url);
				return false;
			}
			}
		}
	...
	[CompilerGenerated]
	private sealed class _003CDownloadString_5_3_003Ed__54 : IEnumerator<object>, IDisposable, IEnumerator
	{
		private int _003C_003E1__state;
		...
		[DebuggerHidden]
		public _003CDownloadString_5_3_003Ed__54(int _003C_003E1__state)
		{
			this._003C_003E1__state = _003C_003E1__state;
		}
		...
		private bool MoveNext()
		{
			...
			case 1:
				...
				handler.Invoke(_003Cwww_003E5__2.text, url);
				return false;
			}
		}
	...
	private static void _Start(string passedGameVersion)
	{
		...
		if (!UI.Load())
		{
			Debug.LogError((object)"UMM can't load UI");
	...
	private static void ParseGameVersion(string passedGameVersion)
	{
		...
		{
			Debug.LogException(ex);
			PFLog.UnityModManager.Exception(ex, "while parsing game version");
			OpenUnityFileLog();
		}
	}
	...
	[IteratorStateMachine(typeof(_003CDownloadString_5_3_003Ed__54))]
	private static IEnumerator DownloadString_5_3(string url, UnityAction<string, string> handler)
	{
		//yield-return decompiler failed: Unexpected instruction in Iterator.Dispose()
		return new _003CDownloadString_5_3_003Ed__54(0)
		{
			url = url,
			handler = handler
		};
	...
	public static Version ParseVersion(string str)
	{
		...
		if (array.Length >= 1)
		{
			return new Version(int.Parse(new Regex("\\D").Replace(array[0], "")), 0);
		}
