...
public static class LinqExtensions
{
	...
	public static int FindIndex<T>(this IEnumerable<T> list, Func<T, bool> pred)
	{
		int num = 0;
		foreach (T item in list)
		{
	...
	[CanBeNull]
	public static TValue FirstItemOfType<TValue>([CanBeNull] this IList source) where TValue : class
	{
		if (source == null)
		{
			return null;
	...
	[CanBeNull]
	public static TValue FirstItemOfType<TValue>([CanBeNull] this IList source, Func<TValue, bool> pred) where TValue : class
	{
		if (source == null)
		{
			return null;
		...
		for (int i = 0; i < source.Count; i++)
		{
			if (source[i] is TValue val && pred(val))
			{
				return val;
			}
		}
	...
	[CanBeNull]
	public static TValue LastItem<TValue>([CanBeNull] this IList<TValue> source, Func<TValue, bool> pred, TValue @default = default(TValue))
	{
		if (source == null || source.Count <= 0)
		{
			return @default;
	...
	[CanBeNull]
	public static TValue LastOrDefault<TValue>([CanBeNull] this IList<TValue> source, Func<TValue, bool> pred, TValue @default = default(TValue))
	{
		return source.LastItem(pred, @default);
	}
