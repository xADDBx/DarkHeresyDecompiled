...
public class CharInfoCharacteristicsTabView : CharInfoComponentView<CharInfoCharacteristicsTabVM>
{
	...
	{
		return (scoresVM?.Stats.OrderBy((CharInfoStatVM s) => (!s.SourceStatType.HasValue) ? StatTypeHelper.DisplayOrder.IndexOf(s.StatType) : StatTypeHelper.DisplayOrder.IndexOf(s.SourceStatType.Value)))?.ToList() ?? new List<CharInfoStatVM>();
	}
}
