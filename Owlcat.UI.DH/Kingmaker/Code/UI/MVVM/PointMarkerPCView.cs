...
public class PointMarkerPCView : View<PointMarkerVM>
{
	[Header("General")]
	[SerializeField]
	...
	{
		base.ViewModel.Portrait.Subscribe(SetPortrait).AddTo(this);
		base.ViewModel.IsVisible.Subscribe(SetVisibility).AddTo(this);
		base.ViewModel.Relation.Subscribe(delegate(UnitRelation r)
		{
		...
		base.ViewModel.AnotherPointMarkObjectType.Subscribe(delegate(EntityPointMarkObjectType r)
		{
			if (base.ViewModel.Unit == null)
			{
		...
		if ((bool)m_LOSView)
		{
			base.ViewModel.LineOfSight.Subscribe(m_LOSView.Bind).AddTo(this);
		}
	}
	...
	protected virtual void ApplyRelationParams(PointMarkerRelationParams relationParams)
	{
		...
		{
			SetPortrait(relationParams.IsAnotherEntity ? relationParams.Icon : base.ViewModel.Portrait.CurrentValue);
	...
	protected virtual void HandleUpdate()
	{
		...
		{
			CalculateMarkerPosition(_: false);
		}
	}
	...
	protected virtual void SetVisibility(bool isVisible)
	{
		...
		if (isVisible)
		{
			CalculateMarkerPosition(_: true);
	...
	private void CalculateMarkerPosition(bool _)
	{
