...
public class NewGamePhaseSaveInjectorVM : NewGamePhaseBaseVm
{
	...
	private readonly ReactiveCommand<Unit> m_CollectionWasUpdated = new ReactiveCommand<Unit>();
	...
	private bool NeedToShow
	{
		get
		{
			if (Game.NewGamePreset == ConfigRoot.Instance.NewGameSettings.NewGamePreset)
			{
				return false;
			}
			bool flag = false;
			foreach (BlueprintCampaign storyCampaign in ConfigRoot.Instance.NewGameSettings.StoryCampaigns)
			{
	...
	{
		MainMenuChargenUnits.Instance.CurrentPregenUnit.Subscribe(SetUnitProperties).AddTo(this);
		m_SaveSlotCollectionVm = new SaveSlotCollectionVM(new ReactiveProperty<SaveLoadMode>(SaveLoadMode.Load)).AddTo(this);
	}
	...
	private void SetUnitProperties([CanBeNull] ChargenUnit chargenUnit)
	{
		if (chargenUnit == null)
		{
			m_CharacterName.Value = ConfigRoot.Instance.LocalizedTexts.UserInterfacesText.NewGameWin.CreateNewCharacter;
