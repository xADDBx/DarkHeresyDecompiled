...
public class GroupChangerCommonVM : GroupChangerVM
{
	private bool RemoteCompanionsAvailable
	{
		get
		{
			if (!AllowRemoteCompanionsEverywhere && !IsCapital)
			{
				return Game.Instance.CurrentlyLoadedArea.IsGlobalmapArea;
			}
			return true;
		}
	}

	private bool AreAllRequiredWithUs => !base.RemoteCharacter.Any((GroupChangerCharacterVM v) => v.IsLock.CurrentValue);

	public GroupChangerCommonVM(Action go, Action close, List<UnitReference> lastUnits, List<BlueprintUnit> requiredUnits, bool isCapital = false, bool sameFinishActions = false, bool canCancel = true, bool allowRemoteCompanionsEverywhere = false)
		: base(go, close, lastUnits, requiredUnits, isCapital, sameFinishActions, canCancel, allowRemoteCompanionsEverywhere)
	{
		...
		{
			if (item.IsInParty.CurrentValue)
			{
			...
			item.Click.Subscribe(delegate(GroupChangerCharacterVM value)
			{
