...
public class SurfaceMainInputLayer : InputLayer, IDisposable, IAbilityTargetSelectionUIHandler, ISubscriber
{
	...
	public SurfaceMainInputLayer()
	{
		...
		m_FrameUpdate = ObservableSubscribeExtensions.Subscribe(Observable.EveryUpdate(UnityFrameProvider.PreLateUpdate), delegate
		{
			OnUpdate();
		});
	...
	public static void MoveCursor(Vector2 vector)
	{
		...
		if ((object)consoleCursor == null || !consoleCursor.IsActive || (!(Game.Instance.CurrentModeType == GameModeType.Default) && !(Game.Instance.CurrentModeType == GameModeType.Pause)))
		{
			return;
		}
		Game.Instance.Controllers.CameraController.Follower.Release();
	...
	{
		if (Game.Instance.CurrentModeType != GameModeType.Default || ConsoleCursor.Instance.IsActive || CutsceneLock.Active)
		{
			return;
		}
	...
	public void StopMovement()
	{
		...
		if (CutsceneLock.Active && Game.Instance.CurrentModeType == GameModeType.Cutscene)
		{
			return;
		}
		try
	...
	protected virtual void UpdateLeftStickMovement()
	{
		...
		else
		{
			...
			if (!(instance == null))
			{
				...
				Game.Instance.Controllers.SynchronizedDataController.PushLeftStickMovement(value, moveDirection, magnitude);
	...
	private void UpdateInteractions()
	{
		...
		{
			if (!base.CursorEnabled && (Game.Instance.CurrentModeType == GameModeType.Default || Game.Instance.CurrentModeType == GameModeType.Pause))
			{
	...
	private void UpdateRightStickMovement()
	{
	...
	private bool TryRefreshInteractableObjectsList()
	{
		...
		for (int i = 0; i < m_PotentialInteractables.Count; i++)
		{
			...
			if (baseUnitEntity != null)
			{
				...
				IUnitInteraction unitInteraction = baseUnitEntity.GetOptional<PartUnitInteractions>()?.SelectClickInteraction(value);
			...
			if (lightweightUnitEntity != null)
			{
				...
				IUnitInteraction unitInteraction2 = lightweightUnitEntity.GetOptional<PartUnitInteractions>()?.SelectClickInteraction(value);
			...
			if (component != null)
			{
				...
				num3 += component.ProximityRadius;
	...
	private bool TryGetEntityView(GameObject interactionObject, out EntityViewBase view)
	{
		...
		if (interactionObject.TryGetComponent<EntityViewBase>(out view))
		{
			return true;
		}
		if (interactionObject.TryGetComponent<GridObstacle>(out var component))
		{
			view = component.EntityView;
			return view != null;
		}
		view = null;
		return false;
	...
	{
		if ((!(Game.Instance.CurrentModeType != GameModeType.Default) || !(Game.Instance.CurrentModeType != GameModeType.Pause)) && !CutsceneLock.Active)
		{
	...
	{
		if (Game.Instance.CurrentModeType == GameModeType.Default && !Game.Instance.Player.IsInCombat && Game.Instance.Player.IsCameraRotateMode && Game.Instance.Controllers.SelectionCharacter.SelectedUnit.Value == null && Game.Instance.Controllers.SelectionCharacter.SelectedUnits.Count == 0)
		{
			Game.Instance.Controllers.CameraController?.Follower?.ScrollTo(Game.Instance.Player.MainCharacterEntity);
		}
	}
