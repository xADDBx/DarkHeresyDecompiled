...
public class LineOfSightView : View<LineOfSightVM>
{
	[SerializeField]
	...
	{
		base.ViewModel.StartPos.CombineLatest(base.ViewModel.EndPos, base.ViewModel.BestStartPos, (Vector3 start, Vector3 end, Vector3? bestStartPos) => new { start, end, bestStartPos }).Subscribe(value =>
		{
	...
	{
		Color color = m_LineRenderer.sharedMaterial.color;
		color.a = startAlpha ?? color.a;
		m_LineRenderer.sharedMaterial.color = color;
		return m_LineRenderer.sharedMaterial.DOFade(visible ? 1f : 0f, m_FadeTime);
	}
