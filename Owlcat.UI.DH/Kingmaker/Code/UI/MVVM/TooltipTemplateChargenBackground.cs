...
public class TooltipTemplateChargenBackground : TooltipBaseTemplate
{
	...
	private void AddStatBonuses(List<ITooltipBrick> bricks, StatType[] types, string title)
	{
		...
		foreach (AddStatModifier item in enumerable)
		{
			...
			string text2 = UIUtilityText.AddSign(item.Value.Value);
			...
			bricks.Add(new TooltipBrickIconStatValue(text, text2, null, type: tooltipBrickIconStatValueType, tooltip: tooltipTemplateGlossary, iconSize: 40f, iconText: statShortLabel, iconColor: value, icon: UIConfig.Instance.UIIcons.StatBackground));
		}
		bricks.Add(new TooltipBricksGroupEnd());
	}
