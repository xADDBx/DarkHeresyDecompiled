...
public class RankEntryItemCommonView : View<CareerPathRankEntryVM>
{
	[SerializeField]
	...
	private void DrawItems()
	{
		...
		{
			RankEntrySelectionVM source = base.ViewModel.Selections[num2];
		...
		{
			RankEntryFeatureItemVM source2 = base.ViewModel.Features[num3];
	...
	public IFloatConsoleNavigationEntity TryGetItemByViewModel(IRankEntrySelectItem currentItemVM)
	{
		foreach (IRankEntryElement allItem in m_AllItems)
		{
			...
			if (!(allItem is RankEntryFeatureItemCommonView rankEntryFeatureItemCommonView))
			{
				...
				{
					rankEntrySelectItem = rankEntrySelectionItemCommonView.GetViewModel();
				}
			}
			else
			{
