...
public class TooltipBricksGroupView : TooltipBaseBrickView<TooltipBricksGroupVM>, IConsoleTooltipBrick
{
	...
	[SerializeField]
	private GameObject m_VerticaLine;
	...
	protected override void OnBind()
	{
		...
		m_Title?.transform.parent.gameObject.SetActive(!string.IsNullOrEmpty(base.ViewModel.Title));
		m_VerticaLine?.SetActive(!string.IsNullOrEmpty(base.ViewModel.Title));
		if (!string.IsNullOrEmpty(base.ViewModel.Title) && m_Title != null)
		{
		...
		if (base.ViewModel.LayoutParams != null)
		{
			...
			m_VerticalLayoutGroup.spacing = base.ViewModel.LayoutParams.Spacing.x;
			m_HorizontalLayoutGroup.spacing = base.ViewModel.LayoutParams.Spacing.x;
		...
		Observable.TimerFrame(2).Subscribe(FixCellWidth).AddTo(this);
	}
