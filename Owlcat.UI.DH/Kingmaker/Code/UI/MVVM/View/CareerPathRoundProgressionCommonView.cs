...
public class CareerPathRoundProgressionCommonView : View<CareerPathVM>, IUIHighlighter, ISubscriber
{
	[Header("Progress Bar")]
	[SerializeField]
	...
	protected override void OnBind()
	{
		...
		base.ViewModel.PointerItem.Subscribe(delegate
		{
			UpdateCurrentProgressBar();
		}).AddTo(this);
	...
	private void UpdateCurrentProgressBar()
	{
		...
		if (!(base.ViewModel.UnitProgressionVM?.CurrentCareer?.CurrentValue.IsInLevelupProcess).GetValueOrDefault())
		{
			return;
		}
		(int, int) currentLevelupRange = base.ViewModel.GetCurrentLevelupRange();
		if (base.ViewModel.PointerItem.CurrentValue == null)
		{
		...
		float num2 = (float)(base.ViewModel.PointerItem.CurrentValue.EntryRank - 1) / (float)base.ViewModel.MaxRank;
