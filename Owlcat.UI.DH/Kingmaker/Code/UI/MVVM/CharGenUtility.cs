...
public static class CharGenUtility
{
	...
	public static CharacterColorsProfile GetClothesColorsProfile(List<EquipmentEntityLink> clothes, out RampColorPreset colorPreset, bool secondary = false)
	{
		for (int i = 0; i < clothes.Count; i++)
		{
			...
			if (!(equipmentEntity == null) && !(equipmentEntity.ColorPresets == null) && equipmentEntity.ColorPresets.HasFlag(ColorPresetFeatureFlag.ClothingPreset))
			{
				CharacterColorsProfile characterColorsProfile = (secondary ? equipmentEntity.SecondaryColorsProfile : equipmentEntity.PrimaryColorsProfile);
				if ((bool)characterColorsProfile && (bool)equipmentEntity.ColorPresets)
				{
	...
	public static CharacterColorsProfile GetClothesColorsProfile(IEnumerable<EquipmentEntity> equipment, out RampColorPreset colorPreset, bool secondary = false)
	{
		foreach (EquipmentEntity item in equipment)
		{
			if (!(item.ColorPresets == null) && item.ColorPresets.HasFlag(ColorPresetFeatureFlag.ClothingPreset))
			{
				CharacterColorsProfile characterColorsProfile = (secondary ? item.SecondaryColorsProfile : item.PrimaryColorsProfile);
				if ((bool)characterColorsProfile)
				{
					colorPreset = item.ColorPresets;
					return characterColorsProfile;
				}
			}
		}
