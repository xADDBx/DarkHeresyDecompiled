...
public class TooltipHandler : IDisposable
{
	...
	{
		m_Config = TooltipHelper.EnsureTooltipPlace(m_Component, config);
		m_SingleData = null;
		m_SingleTemplate = template;
		m_TooltipDatas = null;
	}
	...
	private void Subscribe(TooltipConfig config)
	{
		...
		{
			m_Disable = ObservableSubscribeExtensions.Subscribe(m_Component.OnDisableAsObservable(), delegate
			{
				if (m_Show)
				{
					m_Show = false;
		...
		using (ProfileScope.New("right click bind"))
		{
			...
			if (component != null)
			{
				m_RightClick = ObservableSubscribeExtensions.Subscribe(component.OnRightClickAsObservable(), delegate
				{
					RightClickAction();
				});
				return;
	...
	{
		if ((!Cursor.visible && UIKitRewiredCursorController.HasController && !UIKitRewiredCursorController.Enabled) || DragNDropManager.IsDraggingSomething)
		{
			return;
		}
		m_Show = true;
