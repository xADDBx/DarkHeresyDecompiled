...
public class SplashScreenController : MonoBehaviour
{
	...
	public void ShowSplashScreen()
	{
		...
		{
			if (FirstLaunchSettings.HasShown || !screen.HideInFirstLaunch)
			{
	...
	private void Complete()
	{
		...
		foreach (ScreenUnit screen in m_Screens)
		{
			if (!FirstLaunchSettings.HasShown && screen.FirstLaunchAnotherSoundEvent)
			{
	...
	private void PrepareEvents()
	{
		if (!m_EventsPrepared)
		{
			...
			if (array.Length != 0)
			{
				AkUnitySoundEngine.PrepareEvent(AkPreparationType.Preparation_LoadAndDecode, array, (uint)array.Length);
			}
