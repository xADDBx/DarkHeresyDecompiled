...
public class CustomUIVideoPlayerBaseView : View<CustomUIVideoPlayerVM>
{
	[Header("Main Content")]
	[SerializeField]
	...
	protected readonly ReactiveProperty<bool> VideoIsPlaying = new ReactiveProperty<bool>();
	...
	{
		ObservableSubscribeExtensions.Subscribe(base.ViewModel.ChangeVideo, delegate
		{
			if (!(m_Video.Or(null)?.VideoClip == base.ViewModel.Video))
			{
				m_Video.Or(null)?.Stop();
				m_Video.Or(null)?.SetClip(base.ViewModel.Video, SoundStateType.VideoDLC, prepareVideo: true, base.ViewModel.SoundStart, base.ViewModel.SoundStop);
				ResetVideo();
			}
		}).AddTo(this);
		ObservableSubscribeExtensions.Subscribe(base.ViewModel.ResetVideoCommand, delegate
		{
			ResetVideo();
		}).AddTo(this);
