...
public sealed class SelectionManagerConsole : SelectionManagerBase, IPartyHandler, ISubscriber<IBaseUnitEntity>, ISubscriber, IAreaLoadingStagesHandler
{
	...
	{
		Game.Instance.EntityPools.AllBaseUnits.ForEach(delegate(BaseUnitEntity u)
		{
			u.IsLink = false;
		});
	...
	{
		if (Game.Instance.EntityPools.AllBaseUnits.Count((BaseUnitEntity u) => u.IsDirectlyControllable() && u.IsLink) != m_LinkedUnits.Count || force)
		{
			m_LinkedUnits.Clear();
			m_LinkedUnits.AddRange(Game.Instance.EntityPools.AllBaseUnits.Where((BaseUnitEntity u) => u.IsDirectlyControllable() && u.IsLink));
			SetSelectedUnitsToController();
			UpdateUnitMarks();
			m_UnitMarks.ForEach(delegate(BaseUnitMark m)
			{
	...
	public override void MultiSelect(IEnumerable<UnitEntityView> views, bool canAddToSelection = true)
	{
	}
	...
	public override void ChangeNetRole(string entityId)
	{
		...
		{
			Game.Instance.Controllers.SynchronizedDataController.PushLeftStickMovement(baseUnitEntity, Vector2.zero, 0f);
		}
	}
