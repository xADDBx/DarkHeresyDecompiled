...
public class SettingsEntityDropdownConsoleView : SettingsEntityWithValueConsoleView<SettingsEntityDropdownVM>
{
	...
	public override bool HandleLeft()
	{
		...
		{
			m_Dropdown.SetIndex((m_Dropdown.Index.CurrentValue - 1 + m_Dropdown.VMCollectionCount) % m_Dropdown.VMCollectionCount);
		}
		else
		{
	...
	public override bool HandleRight()
	{
		if (base.ViewModel.ModificationAllowed.CurrentValue)
		{
			m_Dropdown.SetIndex((m_Dropdown.Index.CurrentValue + 1) % m_Dropdown.VMCollectionCount);
		}
		else
		{
			CallNotAllowedNotification();
