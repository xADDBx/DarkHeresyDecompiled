...
public class BaseJournalItemPCView : BaseJournalItemBaseView
{
	...
	protected override void UpdateView()
	{
		...
		Observable.NextFrame().Subscribe(CheckQuestIsViewed).AddTo(this);
		base.UpdateView();
	}
	...
	private void CheckQuestIsViewed()
	{
		...
		base.ViewModel?.Quest?.Objectives?.Where(ObjCanBeViewed).ToList().ForEach(delegate(QuestObjective x)
		{
			if (!x.IsViewed)
			{
		...
		static bool ObjCanBeViewed(QuestObjective objective)
		{
			if (objective == null)
			{
				return false;
			}
			if (!objective.IsActive && objective.State != QuestObjectiveState.Completed)
			{
