...
public class SurfaceCombatInputLayer : SurfaceMainInputLayer
{
	...
	public SurfaceCombatInputLayer()
	{
		...
		m_CurrentUnitSubscription = Game.Instance.Controllers.SelectionCharacter.SelectedUnit.Subscribe(delegate
		{
			m_EnemyIndex = -1;
		});
	}
	...
	private void UpdateEnemyList()
	{
		...
		m_EnemyList.AddRange(Game.Instance.EntityPools.AllBaseUnits.Where((BaseUnitEntity unit) => IsValidEnemy(unit) && !m_EnemyList.Contains(unit)));
		m_EnemyList.Sort(delegate(BaseUnitEntity e1, BaseUnitEntity e2)
		{
			BaseUnitEntity baseUnitEntity = Game.Instance.Controllers.SelectionCharacter.SelectedUnit.Value ?? Game.Instance.Controllers.SelectionCharacter.ActualGroup.FirstOrDefault((BaseUnitEntity u) => u == Game.Instance.Controllers.TurnController.CurrentUnit);
