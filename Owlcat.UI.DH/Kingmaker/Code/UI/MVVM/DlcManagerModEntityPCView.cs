using Kingmaker.Blueprints.Root.Strings;
using Owlcat.UI;
using R3;
using R3.Triggers;
using UnityEngine;

namespace Kingmaker.Code.UI.MVVM;

public class DlcManagerModEntityPCView : DlcManagerModEntityBaseView
{
	[Header("PC Part")]
	[SerializeField]
	private OwlcatButton m_ModSettingsButton;

	protected override void BindViewImplementation()
	{
		base.BindViewImplementation();
		m_ModSettingsButton.gameObject.SetActive(base.ViewModel.ModSettingsAvailable.CurrentValue);
		if (base.ViewModel.ModSettingsAvailable.CurrentValue)
		{
			AddDisposable(ObservableSubscribeExtensions.Subscribe(m_ModSettingsButton.OnLeftClickAsObservable(), delegate
			{
				OpenSettings();
			}));
			AddDisposable(m_ModSettingsButton.SetHint(UIStrings.Instance.DlcManager.ModSettings));
		}
		AddDisposable(m_MultiButton.OnPointerClickAsObservable().Subscribe(delegate
		{
			SwitchValue();
		}));
	}
}
