...
public class TransitionVM_OBSOLETE : ViewModel, IAreaTransitionHandler, ISubscriber, IScreenUIHandler
{
	public readonly string Name;

	public readonly BlueprintMultiEntranceMap Map;

	public readonly List<TransitionEntryVM_OBSOLETE> EntryVms = new List<TransitionEntryVM_OBSOLETE>();

	private readonly Action m_Close;

	public TransitionVM_OBSOLETE(BlueprintMultiEntrance entrance, Action close)
	{
		m_Close = close;
		Map = entrance.Map;
		Name = entrance.Name;
		...
		{
			EntryVms.Add(new TransitionEntryVM_OBSOLETE(e, Close).AddTo(this));
		});
	...
	{
		base.OnDispose();
		EntryVms.ForEach(delegate(TransitionEntryVM_OBSOLETE vm)
		{
			vm.Dispose();
		});
		EntryVms.Clear();
