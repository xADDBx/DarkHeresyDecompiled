...
public class AdditionalSlider : BaseRangedSlider
{
	...
	public void SetAddition(float addition, float mainCorrection = 0f)
	{
		m_Addition = addition;
		if (m_Addition == 0f)
		{
		...
		if (m_Disposable == null)
		{
			...
			{
				OnSliderValueChanged(mainCorrection);
			});
		}
		OnSliderValueChanged(mainCorrection);
	}

	private void OnSliderValueChanged(float mainCorrection)
	{
		float num = m_MainSlider.maxValue - m_MainSlider.minValue;
		float from = (m_MainSlider.value + mainCorrection - m_MainSlider.minValue) / num;
		float to = (m_MainSlider.value + mainCorrection + m_Addition - m_MainSlider.minValue) / num;
