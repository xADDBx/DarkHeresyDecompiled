using Owlcat.Runtime.Core.Utility;
using Owlcat.UI;
using TMPro;
using UnityEngine;

namespace Kingmaker.Code.View.UI.MVVM.HUDNotification.New;

public class NotificationTitleWithLabelBlockView : View<NotificationTitleWithLabelBlockVM>
{
	[Header("Elements")]
	[SerializeField]
	private TMP_Text m_Title;

	[SerializeField]
	private TMP_Text m_LabelDesc;

	[SerializeField]
	private OwlcatMultiSelectable m_StateSelectable;

	protected override void OnBind()
	{
		m_Title.text = base.ViewModel.Title;
		m_LabelDesc.Or(null)?.SetText(base.ViewModel.LabelDescription);
		m_StateSelectable.SetActiveLayer(base.ViewModel.State.ToString());
	}
}
