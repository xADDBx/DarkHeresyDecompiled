...
public class IntentionLine : MonoBehaviour
{
	...
	private LineRenderer Line => m_LineRenderer ?? (m_LineRenderer = GetComponent<LineRenderer>());
	...
	public void Show(BaseUnitEntity startUnit, BaseUnitEntity endUnit)
	{
		if (!m_IsActive || startUnit != m_StartUnit || m_EndUnit != endUnit)
		{
			...
			if (startUnit != null && startUnit.Faction.IsPlayer && !SelectionManagerFacade.IsSelected(startUnit))
			{
				Color startColor = Line.startColor;
				startColor.a = 0f;
				Color endColor = Line.endColor;
